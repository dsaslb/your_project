...
def send_notification_enhanced(
    user_id, content, category="공지", link=None, is_admin_only=False
):
    """향상된 알림 발송 함수"""
    try:
        n = Notification(
    ...
    try:
        ...
        for admin in admins:
            send_notification_enhanced(
                admin.id, content, category, link, is_admin_only=True
            )
        return True
    except Exception as e:
...
def send_notification_to_role(
    ...
    try:
        ...
        for user in users:
            send_notification_enhanced(user.id, content, category, link, is_admin_only)
        return True
    except Exception as e:
