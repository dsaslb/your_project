...
class PluginConfig:
    ...
    required: bool = False
    description: str = ""
    options: Optional[List[Any]] = None  # select 타입일 때 사용
    validation: Optional[Dict[str, Any]] = None  # 유효성 검사 규칙
...
class BasePlugin(ABC):
    ...
    def on_config_change(self, old_config: Dict[str, Any], new_config: Dict[str, Any]) -> bool:
        """설정 변경 시 호출"""
        return True
    
    def on_dependency_loaded(self, dependency_id: str, dependency_plugin: 'BasePlugin') -> bool:
        """의존성 플러그인 로드 시 호출"""
        return True
    ...
    def register(self, plugin_id: str, plugin: BasePlugin) -> bool:
        ...
        try:
            ...
            if not metadata.validate_version():
                print(f"플러그인 {plugin_id}의 버전 형식이 잘못되었습니다: {metadata.version}")
                return False
