...
class PluginFeedbackSystem:
        ...
        if not self.templates_file.exists():
            templates = {
                "plugin_request": {
                    ...
                    "fields": [
                        {"name": "plugin_name", "label": "플러그인 이름", "type": "text", "required": True},
                        {"name": "category", "label": "카테고리", "type": "select", "required": True},
                        {"name": "description", "label": "상세 설명", "type": "textarea", "required": True},
                        {"name": "use_case", "label": "사용 사례", "type": "textarea", "required": False},
                        {"name": "priority", "label": "우선순위", "type": "select", "required": True}
                    ]
                },
                "feature_request": {
                    "title": "기능 요청",
                    ...
                    "fields": [
                        {"name": "plugin_id", "label": "플러그인", "type": "select", "required": True},
                        {"name": "feature_name", "label": "기능 이름", "type": "text", "required": True},
                        {"name": "description", "label": "기능 설명", "type": "textarea", "required": True},
                        {"name": "benefit", "label": "기대 효과", "type": "textarea", "required": False},
                        {"name": "priority", "label": "우선순위", "type": "select", "required": True}
                    ]
                },
                "bug_report": {
                    ...
                    "fields": [
                        {"name": "plugin_id", "label": "플러그인", "type": "select", "required": True},
                        {"name": "bug_title", "label": "버그 제목", "type": "text", "required": True},
                        {"name": "description", "label": "버그 설명", "type": "textarea", "required": True},
                        {"name": "steps", "label": "재현 단계", "type": "textarea", "required": True},
                        {"name": "expected", "label": "예상 동작", "type": "textarea", "required": False},
                        {"name": "actual", "label": "실제 동작", "type": "textarea", "required": False},
                        {"name": "severity", "label": "심각도", "type": "select", "required": True}
                    ]
                }
            }
    ...
    def create_feedback(self, feedback_data: Dict) -> Optional[str]:
        """피드백 생성"""
        try:
            ...
            # 워크플로우 생성
            self._create_workflow(feedback_id, feedback["type"])
    ...
    def get_feedback_list(self, 
        ...
        try:
            ...
            if sort_by == 'created_at':
                ...
                feedbacks.sort(key=lambda x: priority_order.get(x.get('priority', 'medium'), 0), reverse=reverse)
    ...
    def update_feedback_status(self, feedback_id: str, status: str, user_id: str, comment: str = "") -> bool:
        """피드백 상태 업데이트"""
        try:
            feedbacks = self._load_feedback()
            feedback = next((f for f in feedbacks if f['id'] == feedback_id), None)
            ...
            # 댓글 추가
            if comment:
                self.add_comment(feedback_id, user_id, "Admin", "admin", comment, is_internal=True)
    ...
    def assign_feedback(self, feedback_id: str, assigned_to: str, estimated_completion: Optional[str] = None) -> bool:
        """피드백 할당"""
        try:
            feedbacks = self._load_feedback()
            feedback = next((f for f in feedbacks if f['id'] == feedback_id), None)
    ...
    def add_comment(self, feedback_id: str, user_id: str, user_name: str, user_role: str, content: str, is_internal: bool = False) -> Optional[str]:
        """댓글 추가"""
        try:
    ...
    def get_comments(self, feedback_id: str, include_internal: bool = False) -> List[Dict]:
        """댓글 목록 조회"""
        try:
            comments = self._load_comments()
            ...
            if not include_internal:
                feedback_comments = [c for c in feedback_comments if not c.get('is_internal', False)]
            
            # 시간순 정렬
            feedback_comments.sort(key=lambda x: x.get('created_at', ''))
    ...
    def follow_feedback(self, feedback_id: str, user_id: str, follow: bool = True) -> bool:
        """피드백 팔로우"""
        try:
            feedbacks = self._load_feedback()
            feedback = next((f for f in feedbacks if f['id'] == feedback_id), None)
    ...
    def get_workflow(self, feedback_id: str) -> Optional[Dict]:
        """워크플로우 조회"""
        try:
            workflows = self._load_workflows()
            workflow = next((w for w in workflows if w['feedback_id'] == feedback_id), None)
            return workflow
        except Exception as e:
    ...
    def _update_workflow(self, feedback_id: str, status: str):
        ...
        try:
            ...
            new_step = status_to_step.get(status, workflow['current_step'])
            workflow['current_step'] = new_step
            workflow['updated_at'] = datetime.now().isoformat()
            
            # 완료된 경우
            if status in [FeedbackStatus.COMPLETED.value, FeedbackStatus.REJECTED.value, FeedbackStatus.CANCELLED.value]:
    ...
    def _save_workflows(self, workflows: List[Dict]):
        """워크플로우 목록 저장"""
        with open(self.workflows_file, 'w', encoding='utf-8') as f:
