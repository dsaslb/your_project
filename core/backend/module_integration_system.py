...
class IntegrationRule:
    ...
    data_mapping: Dict[str, str]  # 소스 필드 -> 타겟 필드 매핑
    conditions: Dict[str, Any]    # 연동 조건
    schedule: Optional[str] = None  # 배치 스케줄 (cron 형식)
    enabled: bool = True
...
class ModuleIntegrationSystem:
    ...
    def get_integration_rules(self, source_module: str = None, target_module: str = None) -> List[IntegrationRule]:
        """연동 규칙 조회"""
        rules = list(self.integration_rules.values())
    ...
    def enable_integration_rule(self, rule_id: str) -> bool:
        ...
        if rule_id in self.integration_rules:
            self.integration_rules[rule_id].enabled = True
            self.integration_rules[rule_id].updated_at = datetime.now()
    ...
    def disable_integration_rule(self, rule_id: str) -> bool:
        """연동 규칙 비활성화"""
        if rule_id in self.integration_rules:
            self.integration_rules[rule_id].enabled = False
            self.integration_rules[rule_id].updated_at = datetime.now()
            logger.info(f"연동 규칙 비활성화: {rule_id}")
            return True
        return False
    
    async def process_data_integration(self, source_data: Dict[str, Any], 
                                     source_module: str, event_type: str) -> List[Dict[str, Any]]:
        """데이터 연동 처리"""
        try:
            results = []
            
            # 해당 모듈과 관련된 연동 규칙 찾기
            rules = [r for r in self.integration_rules.values() 
            ...
            for rule in rules:
                try:
                    ...
                    if event_type in self.event_handlers:
                        result = await self.event_handlers[event_type](source_data, rule)
                        if result:
                            results.append({
                                'rule_id': rule.id,
                                'rule_name': rule.name,
    ...
    async def _process_realtime_integration(self, source_data: Dict[str, Any], 
                                          rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """실시간 연동 처리"""
        try:
            # 데이터 매핑 적용
            mapped_data = {}
    ...
    def _validate_integration_conditions(self, data: Dict[str, Any], 
    ...
    # 이벤트 핸들러들
    async def _handle_attendance_created(self, data: Dict[str, Any], 
                                       rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """출퇴근 생성 이벤트 처리"""
        try:
    ...
    async def _handle_attendance_updated(self, data: Dict[str, Any], 
                                       rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """출퇴근 수정 이벤트 처리"""
        try:
    ...
    async def _handle_sales_created(self, data: Dict[str, Any], 
                                  rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """매출 생성 이벤트 처리"""
        try:
    ...
    async def _handle_payroll_calculated(self, data: Dict[str, Any], 
                                       rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """급여 계산 이벤트 처리"""
        try:
    ...
    async def _handle_qsc_evaluated(self, data: Dict[str, Any], 
                                  rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """QSC 평가 이벤트 처리"""
        try:
    ...
    async def _handle_inventory_updated(self, data: Dict[str, Any], 
                                      rule: IntegrationRule) -> Optional[Dict[str, Any]]:
        """재고 업데이트 이벤트 처리"""
        try:
    ...
    # 모듈별 데이터 처리 메서드들
    async def _process_payroll_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """급여 데이터 처리"""
        try:
            # 급여 계산 로직
    ...
    async def _process_sales_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """매출 데이터 처리"""
        try:
            # 매출 효율 분석 로직
            employee_id = data.get('employee_id')
    ...
    # 트리거 메서드들
    async def _trigger_payroll_calculation(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """급여 계산 트리거"""
        return await self._process_payroll_data(data)
    
    async def _trigger_payroll_recalculation(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """급여 재계산 트리거"""
        return await self._process_payroll_data(data)
    
    async def _trigger_sales_efficiency_analysis(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """매출 효율 분석 트리거"""
        return await self._process_sales_data(data)
    
    async def _trigger_inventory_update(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """재고 업데이트 트리거"""
        return await self._process_inventory_data(data)
    
    async def _trigger_labor_cost_analysis(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """인건비 분석 트리거"""
        return await self._process_sales_data(data)
    
    async def _trigger_labor_cost_ratio_analysis(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """인건비 비율 분석 트리거"""
        return await self._process_sales_data(data)
    
    async def _trigger_employee_performance_update(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """직원 성과 업데이트 트리거"""
        return await self._process_employee_data(data)
    
    async def _trigger_auto_reorder(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """자동 발주 트리거"""
        return await self._process_inventory_data(data)
    ...
    def get_integration_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            enabled_rules = len([r for r in self.integration_rules.values() if r.enabled])
    ...
    def get_integration_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for rule_id, rule in self.integration_rules.items():
                if not rule.enabled:
                    health_status["issues"].append(f"비활성화된 규칙: {rule.name}")
            
            # 캐시 상태 확인
            cache_size = len(self.data_cache)
            if cache_size > 1000:  # 임계값
                health_status["issues"].append(f"캐시 크기 초과: {cache_size}")
            
            # 오래된 동기화 확인
            for sync_key, sync_time in self.last_sync_times.items():
                if datetime.now() - sync_time > timedelta(hours=24):
                    health_status["issues"].append(f"오래된 동기화: {sync_key}")
    ...
    # 배치 처리
    async def run_batch_integration(self) -> Dict[str, Any]:
        """배치 연동 실행"""
        try:
            results = []
            
            # 배치 타입 규칙들 찾기
            batch_rules = [r for r in self.integration_rules.values() 
            ...
            for rule in batch_rules:
                try:
                    ...
                    self.last_sync_times[sync_key] = datetime.now()
    ...
    async def _collect_batch_data(self, rule: IntegrationRule) -> List[Dict[str, Any]]:
        """배치 데이터 수집"""
        # 실제 구현에서는 데이터베이스에서 배치 데이터 조회
        return []
    
    async def _process_batch_integration(self, batch_data: List[Dict[str, Any]], 
                                       rule: IntegrationRule) -> Dict[str, Any]:
        """배치 연동 처리"""
        # 실제 구현에서는 배치 데이터 처리
