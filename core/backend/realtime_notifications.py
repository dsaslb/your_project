...
class RealtimeNotificationManager:
    ...
    def create_channel(self, channel_id: str, name: str, description="") -> 'NotificationChannel':
        """채널 생성"""
    ...
    def send_notification(self, channel_id: str, notification: 'Notification', target_users: Optional[List[str]] = None) -> bool:
        """알림 전송"""
        try:
            channel = self.get_channel(channel_id)
            if not channel:
    ...
    def get_user_notifications(self, user_id: str, channel_id=None, limit=50) -> List['Notification']:
        """사용자 알림 조회"""
        notifications = []
        if channel_id:
            channel = self.get_channel(channel_id)
            ...
            for channel in self.channels.values():
                channel_notifications = channel.get_notifications(user_id, limit // max(1, len(self.channels)))
