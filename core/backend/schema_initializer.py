from .dynamic_schema import schema_manager, IndustrySchema, DynamicField  # pyright: ignore
    ...
    try:
        ...
        for schema_file in schemas_dir.glob("*_industry.json"):
            try:
                ...
                fields = []
                for field_data in (schema_data.get('fields', []) if schema_data else []):
                    field = DynamicField(**field_data)
                    fields.append(field)

                # 날짜 파싱 (Z 형식과 +00:00 형식 모두 지원)
                created_at_str = schema_data['created_at'].replace('Z', '+00:00') if schema_data and schema_data.get('created_at') else ''
                updated_at_str = schema_data['updated_at'].replace('Z', '+00:00') if schema_data and schema_data.get('updated_at') else ''
                ...
                # 스키마 등록
                if schema_manager.register_industry_schema(
                    schema_data['schema_id'] if schema_data and schema_data.get('schema_id') else '',
                    schema
                ):
                    logger.info(f"업종 스키마 로드 완료: {schema_data['schema_name'] if schema_data and schema_data.get('schema_name') else ''}")
                else:
                    logger.error(f"업종 스키마 등록 실패: {schema_data['schema_name'] if schema_data and schema_data.get('schema_name') else ''}")
...
def create_sample_brand_schema():
    """샘플 브랜드 스키마 생성"""
    try:
        from .dynamic_schema import BrandSchema  # pyright: ignore
