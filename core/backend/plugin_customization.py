...
from typing import Dict, List, Optional, Any, Union  # pyright: ignore
import json
...
class CustomizationRule:
    ...
    value: Any
    conditions: Dict[str, Any] = field(default_factory=dict)  # pyright: ignore
    created_at: str = ""
    updated_at: str = ""
    approved: bool = False
...
class PluginCustomizationManager:
    ...
    def __init__(self, base_dir="customizations"):
        ...
        if self.rules_file.exists():
            try:
                with open(self.rules_file, "r", encoding="utf-8") as f:
                    ...
                    self._rules_cache = [
                        CustomizationRule(**rule) for rule in rules_data
                    ]
            ...
            try:
                with open(self.requests_file, "r", encoding="utf-8") as f:
                    ...
                    self._requests_cache = [
                        CustomizationRequest(**req) for req in requests_data
                    ]
            except Exception as e:
                print(f"요청 로드 실패: {e}")
                self._requests_cache = []
    ...
    def _save_data(self):
        ...
        try:
            with open(self.rules_file, "w", encoding="utf-8") as f:
                json.dump(
                    [asdict(rule) for rule in self._rules_cache],
                    f,
                    indent=2,
                    ensure_ascii=False,
                )
        ...
        try:
            with open(self.requests_file, "w", encoding="utf-8") as f:
                json.dump(
                    [asdict(req) for req in self._requests_cache],
                    f,
                    indent=2,
                    ensure_ascii=False,
                )
    ...
    def update_customization_rule(
        self, old_rule: CustomizationRule, new_rule: CustomizationRule
    ) -> bool:
        """커스터마이즈 규칙 업데이트"""
        try:
            ...
            self._add_to_history(
                "update_rule", {"old": asdict(old_rule), "new": asdict(new_rule)}
            )
            return True
    ...
    def _find_existing_rule(
        self, rule: CustomizationRule
    ) -> Optional[CustomizationRule]:
        """기존 규칙 찾기"""
        for existing_rule in self._rules_cache:
            if (
                existing_rule.plugin_name == rule.plugin_name
                and existing_rule.target_type == rule.target_type
                and existing_rule.target_path == rule.target_path
            ):
    ...
    def get_customization_rules(
        ...
        if conditions:
            rules = [
                r for r in rules if self._check_conditions(r.conditions, conditions)
            ]

        return rules

    def _check_conditions(
        self, rule_conditions: Dict[str, Any], user_conditions: Dict[str, Any]
    ) -> bool:
        """조건 검사"""
        for key, value in (
            rule_conditions.items() if rule_conditions is not None else []
        ):
    ...
    def apply_customizations(
        self,
        plugin_config: Union[Dict[str, Any], List[Any]],
        industry: str,
        brand: str,
    ...
    def _apply_rule(
        self, config: Union[Dict[str, Any], List[Any]], rule: CustomizationRule
    ) -> Union[Dict[str, Any], List[Any]]:
    ...
    def _apply_extend(
        ...
        last_part = path_parts[-1]
        if isinstance(current, dict):
            if isinstance(rule.value, dict) and isinstance(
                current.get(last_part), dict
            ):
                current[last_part].update(rule.value)
            elif isinstance(rule.value, list) and isinstance(
                current.get(last_part), list
            ):
            ...
            idx = int(last_part)
            if isinstance(rule.value, dict) and isinstance(current[idx], dict):
                current[idx].update(rule.value)
            elif isinstance(rule.value, list) and isinstance(current[idx], list):
                current[idx].extend(rule.value)
            else:
                current[idx] = rule.value
        return config
    ...
    def _apply_optional(
        ...
        if last_part.isdigit():
            idx = int(last_part)
            if isinstance(current, list) and isinstance(current[idx], dict):
                current[idx]["optional"] = True
        else:
            if isinstance(current, dict) and isinstance(current.get(last_part), dict):
    ...
    def approve_customization_request(
        ...
        self._add_to_history(
            "approve_request",
            {"request_id": request_id, "reviewer": reviewer, "comment": comment},
        )
        return True

    def reject_customization_request(
        self, request_id: str, reviewer: str, comment: str = ""
    ) -> bool:
        """커스터마이즈 요청 거부"""
        request = self._find_request_by_id(request_id)
    ...
    def get_customization_requests(
        self, status: Optional[str] = None, plugin_name: Optional[str] = None
