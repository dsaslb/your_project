...
from .plugin_interface import plugin_registry
...
class AutoRouter:
    ...
    def register_plugin_routes(self) -> bool:
        ...
        for plugin_id, plugin in loaded_plugins.items():
            try:
                ...
                self.registered_blueprints[plugin_id] = blueprint
    ...
    def _register_plugin_route(self, blueprint: Blueprint, route, plugin_id: str):
        ...
        try:
            ...
            if hasattr(plugin, handler_name):
                ...
                logger.warning(f"플러그인 {plugin_id}에 핸들러 {handler_name}이 없습니다")
        ...
        for module_path, blueprint_name in legacy_blueprints:
            try:
                ...
                if hasattr(module, blueprint_name):
                    ...
                    # 블루프린트 등록
                    self.app.register_blueprint(blueprint, url_prefix=url_prefix)
                    self.registered_blueprints[f"{module_path}.{blueprint_name}"] = blueprint
                    
                    success_count += 1
                    logger.info(f"블루프린트 등록 완료: {module_path}.{blueprint_name}")
                else:
                    logger.warning(f"블루프린트를 찾을 수 없습니다: {module_path}.{blueprint_name}")
                    
            except ImportError as e:
                logger.warning(f"모듈을 로드할 수 없습니다: {module_path} - {e}")
            except Exception as e:
                logger.error(f"블루프린트 등록 실패: {module_path}.{blueprint_name} - {e}")
        
        logger.info(f"기존 블루프린트 등록 완료: {success_count}/{total_count}")
        return success_count > 0
    
    def _determine_url_prefix(self, module_path: str, blueprint_name: str) -> Optional[str]:
        """URL 프리픽스 결정"""
        # API 모듈
    ...
    def get_plugin_status(self) -> Dict:
        ...
        for plugin_id, plugin in plugin_registry.get_all_plugins().items():
            ...
            plugin_status['metadata'] = plugin.get_metadata().__dict__ if plugin.get_metadata() else None
