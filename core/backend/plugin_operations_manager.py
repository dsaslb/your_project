...
class PluginOperationsManager:
    ...
    def start_operations(self) -> Dict[str, Any]:
        """운영 모니터링 시작"""
        ...
        try:
            ...
            return {
                ...
                'start_time': self.operations_status['start_time'].isoformat()
            }
    ...
    def stop_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'uptime': self.operations_status['uptime']
            }
    ...
    def record_operation(self, operation_type: str, success: bool, 
                        response_time: float = 0.0, details: 'Optional[Dict[str, Any]]' = None) -> None:
        """운영 로그 기록"""
        timestamp = datetime.now()
        
        # 운영 통계 업데이트
        self.operations_status['total_operations'] += 1
        if success:
            self.operations_status['successful_operations'] += 1
        else:
            self.operations_status['failed_operations'] += 1
    ...
    def get_performance_metrics(self, metric_type: str = 'all') -> Dict[str, Any]:
        ...
        if metric_type == 'all':
            return {
                ...
                'plugin_load_times': {
                    plugin: list(times) 
                    for plugin, times in self.performance_metrics['plugin_load_times'].items()
                }
            }
        elif metric_type in self.performance_metrics:
            return {
                metric_type: list(self.performance_metrics[metric_type])
            }
        else:
            return {}
    
    def get_alerts(self, alert_level: str = 'all') -> List[Dict[str, Any]]:
        """알림 조회"""
        if alert_level == 'all':
            all_alerts = []
            for level in ['critical', 'warning', 'info']:
                all_alerts.extend(self.alerts[level])
            return all_alerts
        elif alert_level in self.alerts:
            return self.alerts[alert_level]
        else:
            return []
    ...
    def _monitoring_loop(self) -> None:
        """모니터링 루프"""
        while self.operations_status['running']:
            try:
                # 시스템 리소스 모니터링
    ...
    def _monitor_system_resources(self) -> None:
        ...
        try:
            ...
            self.performance_metrics['memory_usage'].append(50.0)
            self.performance_metrics['cpu_usage'].append(30.0)
        except Exception as e:
    ...
    def _calculate_performance_metrics(self) -> None:
        ...
        try:
            # 에러율 계산
            if self.operations_status['total_operations'] > 0:
                error_rate = (self.operations_status['failed_operations'] / 
                            self.operations_status['total_operations']) * 100
    ...
    def _check_system_alerts(self) -> None:
        ...
        try:
            ...
            if self.performance_metrics['memory_usage']:
                current_memory = self.performance_metrics['memory_usage'][-1]
            ...
            if self.performance_metrics['cpu_usage']:
                current_cpu = self.performance_metrics['cpu_usage'][-1]
            ...
            if self.performance_metrics['error_rates']:
                current_error_rate = self.performance_metrics['error_rates'][-1]
    ...
    def _check_thresholds(self, operation_type: str, response_time: float, success: bool) -> None:
        ...
        try:
            ...
            if operation_type == 'plugin_load':
                if response_time > self.thresholds['max_plugin_load_time']:
    ...
    def _perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            # 기본 시스템 상태 체크
            if not self.operations_status['running']:
            ...
            # 에러율 체크
            if self.performance_metrics['error_rates']:
                error_rate = self.performance_metrics['error_rates'][-1]
    ...
    def _get_performance_summary(self) -> Dict[str, Any]:
        """성능 요약 조회"""
        summary = {
            'total_operations': self.operations_status['total_operations'],
            'success_rate': 0.0,
        ...
        try:
            ...
            if self.operations_status['total_operations'] > 0:
                summary['success_rate'] = (
                    self.operations_status['successful_operations'] / 
                    self.operations_status['total_operations']
                ) * 100
            
            # 평균 응답 시간
            if self.performance_metrics['response_times']:
                summary['avg_response_time'] = sum(self.performance_metrics['response_times']) / len(self.performance_metrics['response_times'])
            
            # 현재 메모리 사용량
            if self.performance_metrics['memory_usage']:
                summary['current_memory_usage'] = self.performance_metrics['memory_usage'][-1]
            
            # 현재 CPU 사용량
            if self.performance_metrics['cpu_usage']:
                summary['current_cpu_usage'] = self.performance_metrics['cpu_usage'][-1]
            
            # 현재 에러율
            if self.performance_metrics['error_rates']:
                summary['current_error_rate'] = self.performance_metrics['error_rates'][-1]
    ...
    def _cleanup_old_logs(self) -> None:
        ...
        try:
            ...
            while (self.operation_logs and 
                   datetime.fromisoformat(self.operation_logs[0]['timestamp']) < cutoff_time):
            ...
            for level in self.alerts:
                self.alerts[level] = [
                    alert for alert in self.alerts[level]
