...
class PluginPerformanceOptimizer:
    ...
    def _cleanup_old_cache(self):
        """오래된 캐시 정리"""
        # 가장 오래된 캐시 20% 제거
        if hasattr(self, 'cache') and len(self.cache) > 50:
            if hasattr(self, 'cache_timestamps'):
                sorted_keys = sorted(self.cache_timestamps.items(), key=lambda x: x[1] if x and x[1] is not None else 0)
    ...
    def get_cached_result(self, key: str) -> Optional[Any]:
        ...
        if key in self.cache:
            timestamp = self.cache_timestamps.get(key) if self.cache_timestamps else None
            if timestamp and (datetime.now() - timestamp).total_seconds() < self.cache_ttl:
                return self.cache[key]
            else:
                if self.cache is not None:
                    del self.cache[key]
                if key in self.cache_timestamps and self.cache_timestamps is not None:
                    del self.cache_timestamps[key]
        return None
    ...
    def _calculate_error_rate(self,  plugin_name: str) -> float:
        """오류율 계산"""
        total_errors = sum(count for key, count in self.performance_metrics['error_counts'].items() if key.startswith(plugin_name))
        total_operations = sum(count for key, count in self.performance_metrics['success_counts'].items() if key.startswith(plugin_name))
    ...
    def _calculate_success_rate(self, plugin_name: str) -> float:
        """성공율 계산"""
        total_success = sum(count for key, count in self.performance_metrics['success_counts'].items() if key.startswith(plugin_name))
        total_errors = sum(count for key, count in self.performance_metrics['error_counts'].items() if key.startswith(plugin_name))
...
def cache_result(ttl=300):
    ...
    def decorator(func):
        ...
        def wrapper(*args,  **kwargs):
            # 캐시 키 생성
            cache_key = f"{func.__name__}_{hash(str(args) + str(sorted(kwargs.items() if kwargs is not None else [])))}"
