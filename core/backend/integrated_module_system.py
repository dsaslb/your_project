from core.backend.central_data_layer import CentralDataLayer  # pyright: ignore
...
class IntegrationEventData:
    ...
    branch_id: Optional[int] if Optional is not None else None = None
...
class IntegratedModuleSystem:
    ...
    def __init__(self):
        ...
        self.event_handlers: Dict[IntegrationEvent, List[Callable] if Dict is not None else None] = {}
        self.module_registry: Dict[str, Dict[str, Any] if Dict is not None else None] = {}
        self.analysis_cache: Dict[str, Dict[str, Any] if Dict is not None else None] = {}
    ...
    def _handle_attendance_recorded(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            employee_id = attendance_data.get('employee_id') if attendance_data else None
            branch_id = attendance_data.get('branch_id') if attendance_data else None

            # 1. 매출 데이터와 연동하여 효율성 분석
            sales_data = self.central_data.get_sales_data(
                employee_id=employee_id,
                branch_id=branch_id,
                date_from=attendance_data.get('date') if attendance_data else None,
                date_to=attendance_data.get('date') if attendance_data else None
            )

            if sales_data:
                # 근무 시간 대비 매출 효율성 계산
                work_hours = attendance_data.get('work_hours', 0) if attendance_data else 0
                total_sales = sum(sale['amount'] if sale is not None else 0 for sale in sales_data)
                efficiency = total_sales / work_hours if work_hours > 0 else 0

                # 효율성 데이터 저장
                self.analysis_cache[f"efficiency_{employee_id}_{attendance_data.get('date') if attendance_data else None}"] = {
                    "employee_id": employee_id,
                    "branch_id": branch_id,
                    "date": attendance_data.get('date') if attendance_data else None,
    ...
    def _handle_sales_recorded(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            employee_id = sales_data.get('employee_id') if sales_data else None
            branch_id = sales_data.get('branch_id') if sales_data else None
    ...
    def _handle_payroll_calculated(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            employee_id = payroll_data.get('employee_id') if payroll_data else None
            branch_id = payroll_data.get('branch_id') if payroll_data else None
    ...
    def _handle_inventory_updated(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            branch_id = inventory_data.get('branch_id') if inventory_data else None
    ...
    def _handle_analytics_generated(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            branch_id = analytics_data.get('branch_id') if analytics_data else None
    ...
    def _trigger_analytics_update(self,  event_data: IntegrationEventData):
        """분석 업데이트 트리거"""
        try:
            branch_id = event_data.data.get('branch_id') if event_data.data else None
            if branch_id:
                # 통합 분석 실행
    ...
    def _check_efficiency_alerts(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            # 지각 알림
            if attendance_data.get('is_late') if attendance_data else False:
                self.central_data.create_notification(
                    user_id=employee_id,
                    title="지각 알림",
                    message=f"{attendance_data.get('date') if attendance_data else None} 지각하셨습니다.",
                    notification_type="warning",
                    priority="normal"
                )

            # 초과근무 알림
            if attendance_data.get('is_overtime') if attendance_data else False:
                self.central_data.create_notification(
                    user_id=employee_id,
                    title="초과근무 알림",
                    message=f"{attendance_data.get('date') if attendance_data else None} 초과근무하셨습니다.",
                    notification_type="info",
                    priority="normal"
                )

            # 효율성 저하 알림 (매출 대비 근무시간)
            efficiency_key = f"efficiency_{employee_id}_{attendance_data.get('date') if attendance_data else None}"
    ...
    def _analyze_labor_cost(self,  event_data: IntegrationEventData):
        ...
        try:
            ...
            sales_data = self.central_data.get_sales_data(
                branch_id=branch_id,
                date_from=f"{payroll_data.get('year') if payroll_data else None}-{payroll_data.get('month') if payroll_data else None:02d}-01",
                date_to=f"{payroll_data.get('year') if payroll_data else None}-{payroll_data.get('month') if payroll_data else None:02d}-31"
            )

            total_sales = sum(sale['amount'] if sale is not None else 0 for sale in sales_data)
            total_salary = payroll_data.get('net_salary', 0) if payroll_data else 0
            ...
            if total_sales > 0:
                ...
                # 분석 결과 저장
                self.analysis_cache[f"labor_cost_{branch_id}_{payroll_data.get('year') if payroll_data else None}_{payroll_data.get('month') if payroll_data else None}"] = {
                    "branch_id": branch_id,
                    "year": payroll_data.get('year') if payroll_data else None,
                    "month": payroll_data.get('month') if payroll_data else None,
                    "total_sales": total_sales,
    ...
    def get_integrated_data(self, branch_id: int, data_types: List[str] if List is not None else None = None) -> Dict[str, Any] if Dict is not None else None:
        """통합 데이터 조회"""
        try:
            result = {}
    ...
    def get_efficiency_analysis(self, branch_id: int, period: str = "month") -> Dict[str, Any] if Dict is not None else None:
        """효율성 분석 조회"""
        try:
            ...
            analytics_result['efficiency'] = analytics_result.get('efficiency', {})
    ...
    def get_management_dashboard(self, user_id: int) -> Dict[str, Any] if Dict is not None else None:
        """관리자 대시보드 데이터"""
        try:
            # 사용자 권한에 따른 매장 목록 조회
