from .plugin_customization import PluginCustomizationManager  # pyright: ignore
from .plugin_schema import PluginSchemaValidator, PluginManifest  # pyright: ignore
...
class PluginManager:
    ...
    def __init__(self, app: Flask, plugins_dir: str = "plugins"):
        ...
        self.plugins: Dict[str, Any] = {}  # pyright: ignore
        self.blueprints: Dict[str, Any] = {}  # pyright: ignore
        self.plugin_configs: Dict[str, Any] = {}  # pyright: ignore
        self.loaded_plugins: List[str] = []  # pyright: ignore
        ...
        try:
            ...
            if hasattr(module, "register_blueprint"):
                ...
                if hasattr(plugin_instance, "blueprint") and plugin_instance.blueprint:
                    ...
                    logger.warning(
                        f"플러그인 {plugin_name}에서 블루프린트를 찾을 수 없습니다"
                    )
                    return False
            else:
                logger.warning(
    ...
    def validate_plugin_dependencies(self, plugin_name: str, config: Dict) -> bool:
        """플러그인 의존성 검증"""
        dependencies = config and config.get("dependencies", [])
        for dep in dependencies:
            if dep not in self.loaded_plugins:
                logger.error(
                    f"플러그인 {plugin_name}의 의존성 {dep}이 로드되지 않았습니다"
                )
                return False
        return True
    ...
    def load_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            if not config:
                return False
            # 플러그인이 비활성화된 경우 스킵
            if not (config and config.get("enabled", True)):
            ...
            if not self.register_plugin_blueprint(plugin_name, config):
                return False
            # 플러그인 정보 저장
            self.plugins[plugin_name] = config  # pyright: ignore
            self.plugin_configs[plugin_name] = config.get(
                "config", {}
            )  # pyright: ignore
    ...
    def get_plugin_menus(self) -> List[Dict[str, Any]]:
        """모든 플러그인의 메뉴 정보 반환"""
        menus = []
    ...
    def update_plugin_config(self, plugin_name: str, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not is_valid:
                logger.error(
                    f"플러그인 {plugin_name} 설정 검증 실패: {', '.join(errors)}"
                )
                return False
            # 매니페스트 업데이트
            plugin_dir = Path(self.plugins_dir) / plugin_name
