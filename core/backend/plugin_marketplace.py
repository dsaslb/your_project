...
class PluginMarketplace:
    ...
    def _init_marketplace(self):
        ...
        if not self.categories_file.exists():
            categories = [
                {"id": "restaurant", "name": "레스토랑", "description": "레스토랑 업종 전용"},
                {"id": "retail", "name": "소매", "description": "소매 업종 전용"},
                {"id": "service", "name": "서비스", "description": "서비스 업종 전용"},
                {"id": "manufacturing", "name": "제조", "description": "제조 업종 전용"},
    ...
    def add_plugin_to_marketplace(self, plugin_data: Dict) -> bool:
        """마켓플레이스에 플러그인 추가"""
        try:
            plugins = self._load_plugins()
            
            # 중복 검사
            existing_plugin = next((p for p in plugins if p['id'] == plugin_data['id']), None)
            if existing_plugin:
                # 기존 플러그인 업데이트
    ...
    def install_plugin_from_marketplace(self, plugin_id: str, target_dir: str = "plugins") -> bool:
        """마켓플레이스에서 플러그인 설치"""
        try:
            ...
            with open(plugin_dir / "config" / "plugin.json", 'w', encoding='utf-8') as f:
...
def register_blueprint(bp):
    ...
    def index():
        """플러그인 메인 페이지"""
        return jsonify({{
            "message": "Hello from {plugin['name']} plugin!",
            "plugin": "{plugin_id}",
            "version": "{plugin['version']}"
        }})
...
''')
    ...
    def add_review(self, plugin_id: str, user_id: str, user_name: str, rating: int, comment: str) -> bool:
        """플러그인 리뷰 추가"""
        try:
    ...
    def get_plugin_reviews(self, plugin_id: str, limit: int = 50) -> List[Dict]:
        """플러그인 리뷰 목록 조회"""
        try:
            reviews = self._load_reviews()
            plugin_reviews = [r for r in reviews if r['plugin_id'] == plugin_id]
    ...
    def _save_plugins(self, plugins: List[Dict]):
        """플러그인 목록 저장"""
        with open(self.plugins_file, 'w', encoding='utf-8') as f:
