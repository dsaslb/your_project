...
class IndustrySchema:
    ...
    industry_id: str
    industry_name: str
    fields: Optional[List[DynamicField]]
    created_at: datetime
    updated_at: datetime
    version: str = "1.0.0"
...
class DynamicSchemaManager:
    ...
    def get_combined_schema(self, industry_id: str, brand_id: str) -> Optional[List[DynamicField]]:
        """업종 + 브랜드 스키마 결합"""
        fields = []
        ...
        if industry_schema:
            fields.extend(industry_schema.fields or [])
        ...
        if brand_schema:
            ...
            for brand_field in (brand_schema.fields or []):
    ...
    def validate_data(self, data: Dict[str, Any], schema: List[DynamicField]) -> Dict[str, Any]:
        ...
        for field in schema or []:
            ...
            if field_value is not None:
                validator = self.field_types.get(field.type) if hasattr(self, 'field_types') else None
                if validator:
                    try:
    ...
    def _validate_boolean(self,  value: Any,  field: DynamicField) -> bool:
        ...
        if isinstance(value, bool):
            ...
            if value.lower() if value is not None else '' in ['true', '1', 'yes', 'on']:
                return True
            elif value.lower() if value is not None else '' in ['false', '0', 'no', 'off']:
                return False
            else:
    ...
    def _validate_json(self, value: Any, field: DynamicField) -> Optional[Dict[str, Any]]:
        """JSON 검증"""
    ...
    def create_form_schema(self, fields: Optional[List[DynamicField]]) -> Optional[Dict[str, Any]]:
        """폼 스키마 생성 (프론트엔드용)"""
    ...
    def export_schema(self, schema: IndustrySchema) -> Optional[Dict[str, Any]]:
        """스키마 내보내기"""
        return {
            ...
            'fields': [asdict(field) for field in (schema.fields or [])],
    ...
    def import_schema(self, schema_data: Dict[str, Any]) -> IndustrySchema:
        ...
        schema = IndustrySchema(
            industry_id=schema_data['schema_id'] if schema_data and schema_data.get('schema_id') else '',
            industry_name=schema_data['schema_name'] if schema_data and schema_data.get('schema_name') else '',
            fields=fields,
            version=schema_data.get('version', '1.0.0') if schema_data else '1.0.0',
            created_at=datetime.fromisoformat(schema_data['created_at']) if schema_data and schema_data.get('created_at') else datetime.utcnow(),
            updated_at=datetime.fromisoformat(schema_data['updated_at']) if schema_data and schema_data.get('updated_at') else datetime.utcnow()
        )
