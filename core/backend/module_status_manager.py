...
class ModuleStatusManager:
    ...
    def __init__(self, status_file: str = "marketplace/module_status.json"):
    ...
    def get_module_status(self, module_id: str, user_id: int = None, 
        ...
        try:
            ...
            base_status = {
                ...
                "health": ModuleHealthStatus.UNKNOWN.value,
            ...
            if module_id in self.status_data:
                base_status.update(self.status_data[module_id])
            
            # 사용자별/범위별 상태 확인
            if user_id and scope_id:
                user_key = f"{user_id}_{scope_id}"
                if user_key in base_status.get("user_status", {}):
                    base_status.update(base_status["user_status"][user_key])
            ...
            return {
                ...
                "health": ModuleHealthStatus.CRITICAL.value,
                "error": str(e)
            }
    
    def update_module_status(self, module_id: str, status: ModuleStatus, 
                           user_id: int = None, scope_id: int = None,
                           **kwargs) -> bool:
        """모듈 상태 업데이트"""
        try:
            if module_id not in self.status_data:
                self.status_data[module_id] = {}
            
            # 기본 상태 업데이트
            self.status_data[module_id].update({
                "status": status.value,
            ...
            # 사용자별/범위별 상태 업데이트
            if user_id and scope_id:
                if "user_status" not in self.status_data[module_id]:
                    self.status_data[module_id]["user_status"] = {}
                
                user_key = f"{user_id}_{scope_id}"
                self.status_data[module_id]["user_status"][user_key] = {
                    "status": status.value,
                    "last_updated": datetime.now().isoformat(),
                    **kwargs
                }
    ...
    def set_module_health(self, module_id: str, health: ModuleHealthStatus,
        ...
        try:
            ...
            if metrics:
                self.status_data[module_id]["performance_metrics"] = metrics
    ...
    def record_module_error(self, module_id: str, error_message: str,
        ...
        try:
            ...
            current_errors = self.status_data[module_id].get("errors", [])
    ...
    def set_maintenance_mode(self, module_id: str, enabled: bool,
                           message: str = None, scheduled_end: str = None) -> bool:
        """유지보수 모드 설정"""
        try:
            if module_id not in self.status_data:
                self.status_data[module_id] = {}
            ...
            self.status_data[module_id].update({
                ...
                "status": ModuleStatus.MAINTENANCE.value if enabled else ModuleStatus.ACTIVATED.value
            })
    ...
    def get_modules_by_status(self, status: ModuleStatus = None,
                             health: ModuleHealthStatus = None) -> List[Dict[str, Any]]:
        """상태별 모듈 목록 조회"""
        try:
            modules = []
            
            for module_id, module_data in self.status_data.items():
                if status and module_data.get("status") != status.value:
                    continue
                if health and module_data.get("health") != health.value:
                    continue
    ...
    def get_modules_needing_attention(self) -> List[Dict[str, Any]]:
        """주의가 필요한 모듈 목록"""
        try:
            attention_modules = []
            ...
            for module_id, module_data in self.status_data.items():
                ...
                if (module_data.get("status") == ModuleStatus.ERROR.value or
                    module_data.get("health") in [ModuleHealthStatus.WARNING.value, ModuleHealthStatus.CRITICAL.value]):
    ...
    def check_module_dependencies(self, module_id: str) -> Dict[str, Any]:
        """모듈 의존성 상태 확인"""
        try:
            if module_id not in self.status_data:
                return {"status": "unknown", "dependencies": []}
            
            module_data = self.status_data[module_id]
            ...
            for dep in dependencies:
                ...
                dependency_status.append({
                    ...
                    "health": dep_status.get("health"),
                    "available": dep_status.get("status") == ModuleStatus.ACTIVATED.value
                })
            
            # 모든 의존성이 활성화되어 있는지 확인
            all_available = all(dep["available"] for dep in dependency_status)
    ...
    def get_module_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_modules = len([m for m in self.status_data.values() 
                                if m.get("status") == ModuleStatus.ACTIVATED.value])
            error_modules = len([m for m in self.status_data.values() 
                               if m.get("status") == ModuleStatus.ERROR.value])
            maintenance_modules = len([m for m in self.status_data.values() 
                                     if m.get("maintenance_mode", False)])
            
            avg_uptime = 0
            avg_response_time = 0
            total_metrics = 0
            
            for module_data in self.status_data.values():
                metrics = module_data.get("performance_metrics", {})
                if metrics:
                    avg_uptime += metrics.get("uptime", 0)
    ...
    def cleanup_old_data(self, days: int = 30) -> bool:
        ...
        try:
            ...
            for module_id, module_data in list(self.status_data.items()):
                # 오래된 오류 기록 정리
                if "errors" in module_data:
                    ...
                    if len(old_errors) != len(module_data["errors"]):
                        module_data["errors"] = old_errors
                        cleaned_count += 1
