...
from .plugin_interface import BasePlugin, plugin_registry
...
class PluginLoader:
        ...
        for plugin_dir in self.plugins_dir.iterdir():
            if plugin_dir.is_dir() and not plugin_dir.name.startswith('.'):
                ...
                if config_file.exists():
                    ...
                    logger.warning(f"플러그인 {plugin_id}에 config/plugin.json이 없습니다")
    ...
    def load_plugin(self, plugin_id: str) -> Optional[BasePlugin]:
        ...
        try:
            ...
            if hasattr(module, 'create_plugin'):
                ...
                logger.error(f"플러그인 {plugin_id}에 create_plugin 함수나 Plugin 클래스가 없습니다")
                return None
    ...
    def load_all_plugins(self) -> Dict[str, BasePlugin]:
        ...
        logger.info(f"플러그인 로드 완료: {len(loaded_plugins)}/{len(discovered_plugins)}")
    ...
    def get_plugin_status(self, plugin_id: str) -> Dict[str, Any]:
        ...
        status = {
            ...
            "config": config,
            "health": plugin.get_health_status() if plugin else None
        }
