...
class ModuleInstallationSystem:
    ...
    def install_module(self, module_id: str, installed_by: int,
                       installed_for_type: str, installed_for_id: int) -> Dict[str, Any] if Dict is not None else None:
        """모듈 설치"""
        try:
            with self.lock:
                # 마켓플레이스에서 모듈 정보 조회
                ...
                # 의존성 확인
                dependencies = module_info.get('dependencies', []) if module_info else []
                ...
                with sqlite3.connect(self.installations_db_path) as conn:
                    ...
                    """, (
                        module_id,
                        module_info['name'] if module_info is not None else None,
                        module_info['version'] if module_info is not None else None,
                        installed_by,
                        installed_for_type,
                        installed_for_id,
                        'installed',
                        json.dumps(module_info.get('default_settings', {}) if module_info else {}),
                        json.dumps(module_info.get('permissions', []) if module_info else []),
                        json.dumps(module_info.get('menu_config', {}) if module_info else {})
                    ))
                ...
                return {
                    ...
                    "message": f"모듈 {module_info['name'] if module_info is not None else None}이(가) 성공적으로 설치되었습니다."
                }
    ...
    def activate_module(self, module_id: str, installed_for_type: str,
                        installed_for_id: int, activated_by: int) -> Dict[str, Any] if Dict is not None else None:
        """모듈 활성화"""
        try:
            with self.lock:
                ...
                return {
                    "success": True,
                    "message": f"모듈 {installation['module_name'] if installation is not None else None}이(가) 성공적으로 활성화되었습니다."
                }
    ...
    def deactivate_module(self, module_id: str, installed_for_type: str,
                          installed_for_id: int, deactivated_by: int) -> Dict[str, Any] if Dict is not None else None:
        """모듈 비활성화"""
        try:
            with self.lock:
                installation = self.get_installation(module_id, installed_for_type, installed_for_id)
                ...
                return {
                    "success": True,
                    "message": f"모듈 {installation['module_name'] if installation is not None else None}이(가) 비활성화되었습니다."
                }
    ...
    def uninstall_module(self, module_id: str, installed_for_type: str,
                         installed_for_id: int, uninstalled_by: int) -> Dict[str, Any] if Dict is not None else None:
        """모듈 제거"""
        try:
            with self.lock:
                installation = self.get_installation(module_id, installed_for_type, installed_for_id)
                ...
                with sqlite3.connect(self.installations_db_path) as conn:
                    ...
                    # 관련 데이터 삭제
                    cursor.execute("DELETE FROM module_usage_stats WHERE installation_id = ?", (installation['id'] if installation is not None else None,))
                    cursor.execute("DELETE FROM module_settings WHERE installation_id = ?", (installation['id'] if installation is not None else None,))
                    cursor.execute("DELETE FROM module_permissions WHERE installation_id = ?", (installation['id'] if installation is not None else None,))
                    cursor.execute("DELETE FROM module_installations WHERE id = ?", (installation['id'] if installation is not None else None,))
                ...
                return {
                    "success": True,
                    "message": f"모듈 {installation['module_name'] if installation is not None else None}이(가) 제거되었습니다."
                }
    ...
    def get_installation(self, module_id: str, installed_for_type: str,
                         installed_for_id: int) -> Optional[Dict[str, Any] if Optional is not None else None]:
        """설치 정보 조회"""
        try:
            with sqlite3.connect(self.installations_db_path) as conn:
    ...
    def get_installations(self, installed_for_type: Optional[str] = None,
                          installed_for_id: int = None) -> List[Dict[str, Any] if List is not None else None]:
        """설치 목록 조회"""
        try:
            with sqlite3.connect(self.installations_db_path) as conn:
                conn.row_factory = sqlite3.Row
    ...
    def get_activated_modules(self, installed_for_type: str, installed_for_id: int) -> List[Dict[str, Any] if List is not None else None]:
        """활성화된 모듈 목록 조회"""
        try:
            with sqlite3.connect(self.installations_db_path) as conn:
                conn.row_factory = sqlite3.Row
    ...
    def update_module_settings(self, module_id: str, installed_for_type: str,
                               installed_for_id: int, settings: Dict[str, Any] if Dict is not None else None) -> Dict[str, Any] if Dict is not None else None:
        """모듈 설정 업데이트"""
        try:
            with self.lock:
                installation = self.get_installation(module_id, installed_for_type, installed_for_id)
                ...
                with sqlite3.connect(self.installations_db_path) as conn:
                    ...
                    # 새 설정 저장
                    for key, value in settings.items() if settings is not None else []:
                        cursor.execute("""
                            INSERT INTO module_settings (installation_id, setting_key, setting_value, setting_type)
                            VALUES (?, ?, ?, ?)
                        """, (installation['id'] if installation is not None else None, key, str(value), type(value).__name__))
                    ...
                    cursor.execute("""
                        ...
                        WHERE id = ?
                    """, (json.dumps(settings), installation['id'] if installation is not None else None))
    ...
    def get_module_settings(self, module_id: str, installed_for_type: str,
                            installed_for_id: int) -> Dict[str, Any] if Dict is not None else None:
        """모듈 설정 조회"""
        try:
            installation = self.get_installation(module_id, installed_for_type, installed_for_id)
            if not installation:
            ...
            with sqlite3.connect(self.installations_db_path) as conn:
                ...
                """, (installation['id'] if installation is not None else None,))
    ...
    def get_dependent_modules(self, module_id: str, installed_for_type: str,
                              installed_for_id: int) -> List[str] if List is not None else None:
        """의존하는 모듈 목록 조회"""
        try:
    ...
    def get_installation_statistics(self, installed_for_type: Optional[str] = None,
                                    installed_for_id: Optional[int] = None) -> Dict[str, Any] if Dict is not None else None:
        """설치 통계 조회"""
        try:
            with sqlite3.connect(self.installations_db_path) as conn:
                cursor = conn.cursor()
                ...
                try:
                    ...
                    for module_id, module_info in modules.items() if modules is not None else []:
                        total_downloads += module_info.get('downloads', 0) if module_info else 0
                        if 'rating' in module_info:
                            total_rating += module_info['rating'] if module_info is not None else 0
                            rating_count += 1
