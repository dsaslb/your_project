...
from models import Notice, NoticeComment, NoticeHistory, CommentHistory, Report, User, NoticeRead
from extensions import db
from utils.decorators import owner_or_admin, admin_required
from utils.logger import log_action, log_error, log_action_consistency
from utils.decorators import safe_remove, validate_file_size, sanitize_filename, escape_html, MAX_PREVIEW_SIZE
import os
...
def comment_hide(comment_id):
    ...
    try:
        ...
        log_action_consistency(current_user.id, 'COMMENT_HIDDEN', 
...
def comment_unhide(comment_id):
    ...
    try:
        ...
        log_action_consistency(current_user.id, 'COMMENT_UNHIDDEN', 
...
def report(target_type, target_id):
    ...
    try:
        ...
        log_action(current_user.id, 'REPORT_SUBMITTED', f'Reported {target_type} {target_id}')
...
def admin_report_review(report_id):
    ...
    try:
        ...
        log_action(current_user.id, 'REPORT_REVIEWED', f'Reviewed report {report_id}: {action}')
        return redirect(url_for('admin_reports'))
...
def admin_hidden_comments():
    """관리자: 숨김 처리된 댓글 조회"""
    try:
        comments = NoticeComment.query.filter_by(is_hidden=True).order_by(NoticeComment.created_at.desc()).all()
...
def admin_comment_history(comment_id):
    ...
    try:
        comment = NoticeComment.query.get_or_404(comment_id)
        history = CommentHistory.query.filter_by(comment_id=comment_id).order_by(CommentHistory.edited_at.desc()).all()
        return render_template('admin/comment_history.html', comment=comment, history=history)
    except Exception as e:
        log_error(e, current_user.id)
