...
from models import db, Notification, ErrorLog, Order
...
def get_admin_monitor():
    ...
    traffic = db.session.execute('SELECT COUNT(*) FROM access_log WHERE date = :today', {'today': today}).scalar() if db.engine.has_table('access_log') else 123
    # 최근 1시간 알림
    one_hour_ago = datetime.datetime.now() - datetime.timedelta(hours=1)
    notifications = Notification.query.filter(Notification.created_at >= one_hour_ago).count() if db.engine.has_table('notification') else 15
    # 최근 24시간 에러
    one_day_ago = datetime.datetime.now() - datetime.timedelta(days=1)
    errors = ErrorLog.query.filter(ErrorLog.created_at >= one_day_ago).count() if db.engine.has_table('error_log') else 2
    # DB 상태
    try:
    ...
    if db.engine.has_table('order'):
        for o in Order.query.order_by(Order.created_at.desc()).limit(3):
            logs.append({'time': o.created_at.strftime('%H:%M'), 'event': '발주 생성', 'user': o.created_by})
    if not logs:
        logs = [
    ...
    chart = {
        'labels': [(datetime.datetime.now()-datetime.timedelta(minutes=5*i)).strftime('%H:%M') for i in range(12)][::-1],
    ...
    return jsonify({'stats': {
        ...
        'errors': errors,
        'db_status': db_status,
    }, 'logs': logs, 'chart': chart, 'bar': bar, 'pie': pie, 'doughnut': doughnut})
...
def monitor_stream():
    def event_stream():
        while True:
            data = {'message': '새 알림', 'time': datetime.datetime.now().strftime('%H:%M:%S')}
