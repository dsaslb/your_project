    ...
    try:
        ...
        for attendance in attendances:
            ...
            if attendance.check_in and attendance.check_out:
                ...
                staff_stats[user_id]['total_hours'] += hours
                staff_stats[user_id]['attendance_days'] += 1
            
            if attendance.status == 'late':
                staff_stats[user_id]['late_count'] += 1
            elif attendance.status == 'early_leave':
                staff_stats[user_id]['early_leave_count'] += 1
        
        # 평균 근무 시간
        total_hours = sum(stats['total_hours'] for stats in staff_stats.values())
        total_attendance_days = sum(stats['attendance_days'] for stats in staff_stats.values())
        ...
        result = {
            ...
            'attendance_summary': {
                'total_late': sum(stats['late_count'] for stats in staff_stats.values()),
                'total_early_leave': sum(stats['early_leave_count'] for stats in staff_stats.values())
            }
        }
...
def create_custom_report(current_user):
    ...
    try:
        ...
        if 'error' in report_data:
            return jsonify({'message': report_data['error']}), 400
...
def get_analytics_dashboard(current_user):
    ...
    try:
        ...
        # KPI 계산
        kpis = {
            'total_revenue': sales_data.get('total_sales', 0) if sales_data else 0,
            'total_orders': sales_data.get('total_orders', 0) if sales_data else 0,
            'avg_order_value': sales_data.get('average_order_value', 0) if sales_data else 0,
            'active_staff': staff_data.get('total_staff', 0) if staff_data else 0,
            'low_stock_items': inventory_data.get('low_stock_count', 0) if inventory_data else 0,
            'out_of_stock_items': inventory_data.get('out_of_stock_count', 0) if inventory_data else 0
        }
