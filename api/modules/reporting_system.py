...
class ReportingSystem:
    ...
    def generate_sales_report(self, start_date: datetime, end_date: datetime, branch_id: Optional[int] = None) -> Dict:
        """매출 보고서 생성"""
        try:
            # 기본 쿼리
            ...
            for order in orders:
                ...
                if hour not in hourly_sales:
                    hourly_sales[hour] = 0
                hourly_sales[hour] += order.total_amount
            ...
            return {
                ...
                'data': {
                    ...
                    'sales_by_hour': [
                        ...
                        for hour, sales in sorted(hourly_sales.items())
                    ],
    ...
    def generate_staff_report(self, start_date: datetime, end_date: datetime, branch_id: Optional[int] = None) -> Dict:
        """직원 보고서 생성"""
        try:
            # 직원 기본 정보
            query = User.query
            ...
            for user in users:
                if user.role not in role_stats:
                    role_stats[user.role] = {'count': 0, 'avg_attendance': 0}
                role_stats[user.role]['count'] += 1
                    ...
                    'role_distribution': [
                        {
                            'role': role,
                            'count': stats['count'],
                            'percentage': (stats['count'] / len(users) * 100) if users else 0
                        }
    ...
    def generate_inventory_report(self, branch_id: Optional[int] = None) -> Dict:
        """재고 보고서 생성"""
        try:
            # 재고 데이터 (더미)
            inventory_items = [
    ...
    def generate_operational_report(self, start_date: datetime, end_date: datetime, branch_id: Optional[int] = None) -> Dict:
        """운영 보고서 생성"""
        try:
            # 운영 지표 (더미 데이터)
            operational_metrics = {
    ...
    def create_chart_image(self, data: Dict, chart_type: str, title: str) -> str:
        ...
        try:
            ...
            if chart_type == 'line':
                ...
                values = [item['sales'] for item in data]
                plt.bar(labels, values, color=self.chart_styles['primary_color'])
                plt.xticks(rotation=45)
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            for key, value in report_data['data'].items():
                if isinstance(value, list) and value:
                    ...
                    sheet_name = key[:31]  # Excel 시트명 제한
