from models_main import db, Order, Attendance, InventoryItem
...
ai_assistant_api = Blueprint(
    "ai_assistant_api", __name__, url_prefix="/api/ai/assistant"
)
...
def ai_report():
    ...
    # 2. AI 진단/예측/개선점(예시)
    diagnosis = (
        "매출이 전주 대비 10% 감소했습니다. 인건비 비율이 높아 개선이 필요합니다."
    )
...
def ai_alerts():
    ...
    sales_last_7 = (
        db.session.query(db.func.sum(Order.total_amount))
        .filter(Order.created_at >= seven_days_ago, Order.created_at < today)
        .scalar()
        or 0
    )
