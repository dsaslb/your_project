...
def calculate_address_similarity(address1, address2):
    ...
    # 주소 정규화 (공백 제거, 소문자 변환)
    addr1 = re.sub(r'\s+', '', address1.lower())
    addr2 = re.sub(r'\s+', '', address2.lower())
...
def check_address_duplicate():
    ...
    try:
        ...
        entity_type = data.get('entity_type', 'brand')  # brand, store, employee
        ...
        logger.info(f"주소 중복 체크 완료: {address} -> {duplicate_result['duplicate']}")
        
        return jsonify({
            'success': True,
            'duplicate': duplicate_result['duplicate'],
            'similar_addresses': duplicate_result['similar_addresses'],
...
def check_duplicate_address(address, exclude_id=None, entity_type='brand', similarity_threshold=0.8):
    ...
    for addr_record in all_addresses:
        existing_address = addr_record[0]
        if existing_address:
...
def geocode_address():
    ...
    try:
        ...
        if coordinates:
            logger.info(f"주소 좌표 변환 성공: {address} -> ({coordinates['latitude']}, {coordinates['longitude']})")
            return jsonify({
                'success': True,
                'latitude': coordinates['latitude'],
                'longitude': coordinates['longitude'],
                'formatted_address': coordinates.get('formatted_address', normalized_address),
                'normalized_address': normalized_address
            })
        else:
        ...
        if response.status_code == 200:
            ...
            if data.get('documents') and len(data['documents']) > 0:
                ...
                return {
                    ...
                    'formatted_address': document.get('address_name', address)
                }
            else:
...
def get_address_suggestions():
    ...
    try:
        ...
        entity_type = data.get('entity_type', 'all')  # all, brand, store, employee
...
def get_similar_addresses(query, limit=10, entity_type='all'):
    ...
    if entity_type in ['all', 'brand']:
        ...
        all_addresses.update([addr[0] for addr in brand_addresses if addr[0]])
    
    if entity_type in ['all', 'store']:
        store_addresses = Store.query.with_entities(Store.address).filter(Store.address.isnot(None)).all()
        all_addresses.update([addr[0] for addr in store_addresses if addr[0]])
    ...
    if entity_type in ['all', 'employee']:
        ...
        all_addresses.update([addr[0] for addr in employee_addresses if addr[0]])
    ...
    # 상위 결과 반환
    suggestions = [item['address'] for item in address_scores[:limit]]
