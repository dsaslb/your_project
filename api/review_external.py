...
def analyze_reviews():
    ...
    positive = sum(1 for r in DUMMY_REVIEWS if r["score"] >= 4)
    negative = sum(1 for r in DUMMY_REVIEWS if r["score"] <= 2)
    neutral = total - positive - negative
    avg_score = round(sum(r["score"] for r in DUMMY_REVIEWS) / total, 2) if total else 0
    return jsonify({
        "total_reviews": total,
