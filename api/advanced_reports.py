...
def generate_pdf_report(report_data, report_type):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_file:
            ...
            # 보고서 제목
            title = Paragraph(f"{REPORT_TEMPLATES[report_type]['name']}", title_style)
            story.append(title)
...
def generate_sales_pdf_content(data, styles, header_style):
    ...
    summary_data = [
        ['지표', '값', '전월 대비'],
        ['총 매출', f"{data['total_sales']:,}원", f"{data['sales_growth']:+.1f}%"],
        ['주문 수', f"{data['total_orders']:,}건", f"{data['order_growth']:+.1f}%"],
        ['평균 주문 금액', f"{data['avg_order_value']:,}원", f"{data['avg_order_growth']:+.1f}%"],
        ['고객 수', f"{data['total_customers']:,}명", f"{data['customer_growth']:+.1f}%"]
    ]
    ...
    daily_sales_text = ""
    for day_data in data['daily_sales'][:7]:  # 최근 7일
        daily_sales_text += f"• {day_data['date']}: {day_data['sales']:,}원 ({day_data['orders']}건)\n"
    
    story.append(Paragraph(daily_sales_text, styles['Normal']))
    story.append(Spacer(1, 20))
...
def generate_staff_pdf_content(data, styles, header_style):
    ...
    for staff in data['staff_performance']:
        staff_data.append([
            staff['name'],
            f"{staff['work_days']}일",
            f"{staff['total_hours']:.1f}시간",
            f"{staff['attendance_rate']:.1f}%",
            f"{staff['avg_hours']:.1f}시간"
        ])
...
def generate_inventory_pdf_content(data, styles, header_style):
    ...
    for item in data['inventory_status']:
        status = "정상" if item['current_stock'] > item['min_stock'] else "부족"
        inventory_data.append([
            item['name'],
            f"{item['current_stock']}개",
            f"{item['min_stock']}개",
            status,
            f"{item['days_until_stockout']:.1f}일" if item['days_until_stockout'] > 0 else "N/A"
        ])
...
def generate_operational_pdf_content(data, styles, header_style):
    ...
    operational_data = [
        ['지표', '현재값', '목표값', '달성률'],
        ['주문 처리 시간', f"{data['avg_order_time']:.1f}분", "15분", f"{data['order_time_achievement']:.1f}%"],
        ['고객 만족도', f"{data['customer_satisfaction']:.1f}점", "4.5점", f"{data['satisfaction_achievement']:.1f}%"],
        ['재고 회전율', f"{data['inventory_turnover']:.1f}회", "12회", f"{data['turnover_achievement']:.1f}%"],
        ['직원 생산성', f"{data['employee_productivity']:.1f}%", "85%", f"{data['productivity_achievement']:.1f}%"]
    ]
...
def create_sales_excel_worksheet(workbook, data, header_format, data_format, number_format, currency_format):
    ...
    # 제목
    worksheet.merge_range('A1:E1', f"{REPORT_TEMPLATES['sales_summary']['name']}", header_format)
    ...
    summary_data = [
        ['총 매출', data['total_sales'], f"{data['sales_growth']:+.1f}%"],
        ['주문 수', data['total_orders'], f"{data['order_growth']:+.1f}%"],
        ['평균 주문 금액', data['avg_order_value'], f"{data['avg_order_growth']:+.1f}%"],
        ['고객 수', data['total_customers'], f"{data['customer_growth']:+.1f}%"]
    ]
    ...
    for i, day_data in enumerate(data['daily_sales'][:7]):
        row = 14 + i
        worksheet.write(f'A{row}', day_data['date'], data_format)
        worksheet.write(f'B{row}', day_data['sales'], currency_format)
        worksheet.write(f'C{row}', day_data['orders'], number_format)
...
def create_staff_excel_worksheet(workbook, data, header_format, data_format, number_format):
    ...
    # 제목
    worksheet.merge_range('A1:F1', f"{REPORT_TEMPLATES['staff_performance']['name']}", header_format)
    worksheet.write('A2', f"생성일: {datetime.now().strftime('%Y년 %m월 %d일 %H:%M')}", data_format)
    ...
    for i, staff in enumerate(data['staff_performance']):
        row = 5 + i
        worksheet.write(row, 0, staff['name'], data_format)
        worksheet.write(row, 1, staff['work_days'], number_format)
        worksheet.write(row, 2, staff['total_hours'], number_format)
        worksheet.write(row, 3, staff['attendance_rate'], number_format)
        worksheet.write(row, 4, staff['avg_hours'], number_format)
...
def create_inventory_excel_worksheet(workbook, data, header_format, data_format, number_format):
    ...
    # 제목
    worksheet.merge_range('A1:F1', f"{REPORT_TEMPLATES['inventory_analysis']['name']}", header_format)
    worksheet.write('A2', f"생성일: {datetime.now().strftime('%Y년 %m월 %d일 %H:%M')}", data_format)
    ...
    for i, item in enumerate(data['inventory_status']):
        row = 5 + i
        status = "정상" if item['current_stock'] > item['min_stock'] else "부족"
        worksheet.write(row, 0, item['name'], data_format)
        worksheet.write(row, 1, item['current_stock'], number_format)
        worksheet.write(row, 2, item['min_stock'], number_format)
        worksheet.write(row, 3, status, data_format)
        worksheet.write(row, 4, item['days_until_stockout'], number_format)
...
def create_operational_excel_worksheet(workbook, data, header_format, data_format, number_format):
    ...
    # 제목
    worksheet.merge_range('A1:E1', f"{REPORT_TEMPLATES['operational_efficiency']['name']}", header_format)
    worksheet.write('A2', f"생성일: {datetime.now().strftime('%Y년 %m월 %d일 %H:%M')}", data_format)
    ...
    operational_data = [
        ['주문 처리 시간', data['avg_order_time'], 15, data['order_time_achievement']],
        ['고객 만족도', data['customer_satisfaction'], 4.5, data['satisfaction_achievement']],
        ['재고 회전율', data['inventory_turnover'], 12, data['turnover_achievement']],
        ['직원 생산성', data['employee_productivity'], 85, data['productivity_achievement']]
    ]
...
def generate_report(report_type):
    ...
    try:
        ...
        return send_file(
            file_path,
            as_attachment=True,
            download_name=f"{REPORT_TEMPLATES[report_type]['name']}_{datetime.now().strftime('%Y%m%d_%H%M')}.{file_extension}",
            mimetype=mime_type
        )
