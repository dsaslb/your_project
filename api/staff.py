...
import json
import os
from models_main import db, Staff, Contract, HealthCertificate, Notification, User, ApproveLog
...
def get_staff_list():
    ...
    try:
        ...
        # 검색 파라미터
        search = request.args.get('search', '') if args else None
        department = request.args.get('department', '') if args else None
        status = request.args.get('status', '') if args else None
        include_pending = request.args.get('include_pending', 'false').lower() if args else None == 'true'
        page_type = request.args.get('page_type', 'all') if args else None  # 'all', 'attendance', 'schedule', 'management'
...
def get_staff_detail(staff_id):
    ...
    try:
        ...
        user = User.query.get(staff_id) if query else None
        if not user:
...
def create_staff():
    ...
    try:
        ...
        # 권한 설정
        if data and data.get('permissions'):
        ...
        staff_profile.join_date = datetime.strptime(
            data.get('join_date', datetime.now().strftime('%Y-%m-%d')), '%Y-%m-%d').date()
        ...
        # 계약서 정보가 있으면 추가
        if data and data.get('contract_start_date') and data.get('contract_expiry_date'):
            contract = Contract()
        ...
        # 보건증 정보가 있으면 추가
        if data and data.get('health_certificate_issue_date') and data.get('health_certificate_expiry_date'):
