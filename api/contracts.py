...
def generate_contract():
    try:
        ...
        basic_info_data = [
            ['구분', '내용'],
            ['성명', data['employee_name'] if data is not None else None],
            ['직책', data['position'] if data is not None else None],
            ['부서', data['department'] if data is not None else None],
            ['이메일', data['email'] if data is not None else None],
            ['연락처', data['phone'] if data is not None else None]
        ]
        ...
        contract_period_data = [
            ['구분', '날짜'],
            ['계약 시작일', data['start_date'] if data is not None else None],
            ['계약 종료일', data.get('end_date', '무기한') if data else None]
        ]
        ...
        # 근무일
        work_days = ', '.join(data.get('work_days', [])) if data else None
        work_hours = data.get('work_hours', {}) if data else None
        work_time = f"{work_hours.get('start', '')} ~ {work_hours.get('end', '')}" if work_hours else ''
        ...
        salary = data.get('salary', {}) if data else None
        salary_data = [
            ['구분', '금액'],
            ['기본급', f"{salary.get('base', 0):,}원" if salary else '0원'],
            ['수당', f"{salary.get('allowance', 0):,}원" if salary else '0원'],
            ['상여금', f"{salary.get('bonus', 0):,}원" if salary else '0원'],
            ['총 급여', f"{salary.get('total', 0):,}원" if salary else '0원']
        ]
        ...
        benefits = data.get('benefits', []) if data else None
        ...
        story.append(Paragraph(f"수습기간: {data.get('probation_period', 0) if data else 0}개월", normal_style))
        story.append(Paragraph(f"사전통지기간: {data.get('notice_period', 0) if data else 0}개월", normal_style))

        if data and data.get('responsibilities'):
            story.append(Paragraph(f"주요 업무: {data['responsibilities']}", normal_style))

        if data and data.get('terms'):
            story.append(Paragraph(f"특별 조건: {data['terms']}", normal_style))

        # 계약서 생성일
        story.append(Spacer(1, 30))
        generated_date = data.get('generated_date', datetime.now().isoformat()) if data else datetime.now().isoformat()
