from core.backend.module_development_system import ModuleDevelopmentSystem  # pyright: ignore
import sqlite3
...
def create_project():
    ...
    try:
        ...
        module_type = data.get('module_type', 'general') if data else None
        created_by = data.get('created_by', 'default_user') if data else None
...
def update_component(project_id,  component_id):
    ...
    try:
        ...
        updates = {
            'name': data.get('name') if data else None,
            'position_x': data.get('position', {}).get('x') if data else None,
            'position_y': data.get('position', {}).get('y') if data else None,
            'width': data.get('size', {}).get('width') if data else None,
            'height': data.get('size', {}).get('height') if data else None,
            'properties': json.dumps(data.get('properties', {})),
            'styles': json.dumps(data.get('styles', {}))
        }
...
def create_version(project_id):
    ...
    try:
        data = request.get_json()
        version_name = data.get('version_name') if data else None
        description = data.get('description', '') if data else None
        created_by = data.get('created_by', 'default_user') if data else None
...
def deploy_project(project_id):
    ...
    try:
        ...
        environment = data.get('environment', 'marketplace') if data else None
        deployed_by = data.get('deployed_by', 'default_user') if data else None
...
def generate_test_data(project_id):
    ...
    try:
        data = request.get_json()
        data_type = data.get('data_type', 'sample') if data else None
...
def export_project(project_id):
    ...
    try:
        data = request.get_json()
        export_format = data.get('format', 'zip') if data else None
...
def reset_sandbox():
    ...
    try:
        data = request.get_json()
        reset_type = data.get('type', 'data') if data else None  # data, components, all
