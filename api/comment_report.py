...
from models import Report, NoticeComment, db
from api.utils import admin_required

comment_report_bp = Blueprint('comment_report', __name__, url_prefix='/api/admin/comment-reports')
...
def get_comment_reports(current_user):
    ...
    query = Report.query.filter_by(target_type='comment').order_by(Report.created_at.desc())
