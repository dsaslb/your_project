        ...
        return jsonify({
            ...
            'data': {
                ...
                'channels_configured': {
                    ...
                    for channel, config in enhanced_alert_system.channel_configs.items()
                }
            }
        })
...
def get_alert_rules():
    ...
    try:
        rules = []
        for rule in enhanced_alert_system.alert_rules.values():
            rules.append({
                'id': rule.id,
                'name': rule.name,
...
def create_alert_rule():
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                'success': False,
                'message': f'유효하지 않은 심각도입니다: {data["severity"]}'
            }), 400
        ...
        # 규칙 생성
        rule = AlertRule(
            id=data.get('id', f"custom_{int(datetime.utcnow().timestamp())}"),
            name=data['name'],
...
def update_alert_rule(rule_id: str):
    ...
    try:
        ...
        data = request.get_json()
        rule = enhanced_alert_system.alert_rules[rule_id]
        ...
        for channel, config in enhanced_alert_system.channel_configs.items():
            ...
            if channel == AlertChannel.EMAIL:
                safe_config.update({
                    ...
                    'to_emails_count': len(config.get('to_emails', []))
                })
                ...
                safe_config.update({
                    ...
                    'chat_configured': bool(config.get('chat_id'))
                })
            elif channel == AlertChannel.SMS:
                safe_config.update({
                    'provider': config.get('provider', ''),
                    'from_number': config.get('from_number', ''),
                    'to_numbers_count': len(config.get('to_numbers', []))
                })
            
            configs[channel.value] = safe_config
...
def update_channel_config(channel_name: str):
    ...
    try:
        ...
        # 기존 설정과 병합
        current_config = enhanced_alert_system.channel_configs[channel].copy()
...
def get_alert_statistics():
    ...
    try:
        ...
        for alert in alerts_24h:
            ...
            plugin_stats[plugin_id]['total_alerts'] += 1
            plugin_stats[plugin_id]['by_severity'][alert.severity.value] += 1
