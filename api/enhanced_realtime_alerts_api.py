from core.backend.plugin_monitoring_integration import PluginMonitoringIntegration  # pyright: ignore
from models_main import User, db
from api.gateway import token_required, role_required, admin_required  # pyright: ignore
from core.backend.enhanced_realtime_alerts import (  # pyright: ignore
...
def alert_callback(alert):
    ...
    try:
        ...
        # 모든 연결된 클라이언트에게 전송
        for client_id, client_info in connected_clients.items() if connected_clients is not None else []:
            try:
...
def get_alerts():
    ...
    try:
        ...
        # 알림 필터링
        alerts = []
        for alert in getattr(alert_system, 'alerts', []):
            if hasattr(alert, 'resolved') and alert.resolved != resolved:
                continue
            if severity and hasattr(alert, 'severity') and getattr(alert.severity, 'value', None) != severity:
                continue
            if alert_type and hasattr(alert, 'type') and getattr(alert.type, 'value', None) != alert_type:
                continue
            if plugin_id and getattr(alert, 'plugin_id', None) != plugin_id:
                continue
            if hasattr(alert, 'to_dict'):
                alerts.append(alert.to_dict())

        # 시간순 정렬 및 제한
        alerts.sort(key=lambda x: x.get('timestamp', '') if isinstance(x, dict) else '', reverse=True)
        alerts = alerts[:limit]
...
def get_alert_detail(alert_id):
    ...
    try:
        ...
        alert = alert_system.alerts[alert_id] if alerts is not None else None
        return jsonify({
            'success': True,
            'alert': alert.to_dict()
        })
...
def bulk_resolve_alerts():
    ...
    try:
        ...
        alert_ids = data.get('alert_ids', []) if data else []
...
def get_plugin_metrics(plugin_id):
    ...
    try:
        ...
        metrics = alert_system.plugin_metrics[plugin_id] if plugin_metrics is not None else None
...
def get_all_plugin_metrics():
    ...
    try:
        all_metrics = {}
        for plugin_id, metrics in alert_system.plugin_metrics.items() if plugin_metrics is not None else []:
...
def get_notification_config():
    ...
    try:
        ...
        if user_id not in alert_system.notification_configs:
            ...
            config_obj = alert_system.notification_configs[user_id] if notification_configs is not None else None
            config = {
                'user_id': config_obj.user_id,
                'channels': [c.value if c is not None else None for c in config_obj.channels],
                'alert_types': [t.value if t is not None else None for t in config_obj.alert_types],
                'severity_levels': [s.value if s is not None else None for s in config_obj.severity_levels],
