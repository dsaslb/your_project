...
from core.backend.plugin_loader import plugin_loader  # pyright: ignore
from core.backend.plugin_interface import plugin_registry  # pyright: ignore
...
# 플러그인 설치 이력
plugin_installation_history: List[Dict[str, Any]] = []
...
def get_plugins():
    ...
    try:
        ...
        for plugin_id, plugin in plugin_registry.get_all_plugins().items():
            ...
            # 필터링 적용
            if filters and filters.get('category') and metadata.category != filters['category']:
                continue
            if filters and filters.get('enabled') and str(metadata.enabled).lower() != filters['enabled'].lower():
                continue
            if filters and filters.get('status') and health_status.get('status') != filters['status']:
                continue

            # 성능 메트릭 추가
            metrics = plugin_metrics.get(plugin_id, {}) if plugin_metrics else {}
        ...
        if sort_by in ['name', 'version', 'category', 'enabled']:
            plugins = [p for p in plugins if p is not None and sort_by in p]
            plugins.sort(key=lambda x: x[sort_by], reverse=(sort_order == 'desc'))  # pyright: ignore
    ...
    try:
        ...
        if plugin_loader.save_plugin_config(plugin_id, new_config):
            ...
            plugin.on_config_change(
                old_config.get('config', {}) if old_config else {},
                new_config.get('config', {}) if new_config else {}
            )
...
def get_plugins_status():
    ...
    try:
        ...
        for plugin_id, plugin in plugin_registry.get_all_plugins().items():
            ...
            # 타입 강제 초기화(안전)
            if not isinstance(status['enabled_plugins'], int):
                status['enabled_plugins'] = 0
            if not isinstance(status['disabled_plugins'], int):
                status['disabled_plugins'] = 0
            if not isinstance(status['plugins_by_category'], dict):
                status['plugins_by_category'] = {}
            if not isinstance(status['health_summary'], dict):
                status['health_summary'] = {'healthy': 0, 'warning': 0, 'error': 0}
            for key in ['healthy', 'warning', 'error']:
                if key not in status['health_summary'] or not isinstance(status['health_summary'][key], int):
                    status['health_summary'][key] = 0
            # 카테고리별 카운트
            category = metadata.category
            if category not in status['plugins_by_category'] or not isinstance(status['plugins_by_category'][category], list):
...
def update_plugin_schema(plugin_id: str):
    ...
    try:
        ...
        config['config_schema'] = data['schema'] if data is not None else None
        config['updated_at'] = datetime.utcnow().isoformat()
        # 저장
        if plugin_loader.save_plugin_config(plugin_id, config):
            return jsonify({'success': True, 'message': '스키마가 업데이트되었습니다', 'schema': data['schema'] if data is not None else None})
        else:
...
def install_plugin(plugin_id: str):
    ...
    try:
        ...
        install_options = data.get('options', {}) if data else None
        ...
        # 버전 호환성 검사
        compatibility_check = check_plugin_compatibility(
            plugin_id, install_options.get('version') if install_options else None)
        if not compatibility_check['compatible'] if compatibility_check is not None else None:
            return jsonify({
                'success': False,
                'error': f'버전 호환성 문제: {compatibility_check["error"] if compatibility_check is not None else None}'
            }), 400
            ...
            if plugin_loader.save_plugin_config(plugin_id, config):
                ...
                return jsonify({
                    ...
                    'install_info': {
                        ...
                        'installed_at': config['installed_at'] if config is not None else None,
...
def uninstall_plugin(plugin_id: str):
    ...
    try:
        data = request.get_json() or {}
        uninstall_options = data.get('options', {}) if data else None
        ...
        if dependent_plugins and not uninstall_options.get('force', False) if uninstall_options else False:
            return jsonify({
                'success': False,
        ...
        if plugin.on_disable():
            ...
            if plugin_loader.save_plugin_config(plugin_id, config):
                ...
                return jsonify({
                    ...
                    'uninstall_info': {
                        'plugin_id': plugin_id,
                        'uninstalled_at': config['uninstalled_at'] if config is not None else None,
...
def update_plugin(plugin_id: str):
    ...
    try:
        ...
        # 현재는 버전 정보만 업데이트
        config = plugin_loader.get_plugin_config(plugin_id) or {}
        old_version = config.get('version', '1.0.0') if config else '1.0.0'

        if new_version:
            config['version'] = new_version
        config['updated_at'] = datetime.utcnow().isoformat()
        config['update_history'] = config.get('update_history', []) if config else []
...
def rollback_plugin(plugin_id: str):
    ...
    try:
        ...
        # 현재는 버전 정보만 변경
        config = plugin_loader.get_plugin_config(plugin_id) or {}
        current_version = config.get('version', '1.0.0') if config else '1.0.0'

        config['version'] = target_version
        config['updated_at'] = datetime.utcnow().isoformat()
        config['rollback_history'] = config.get('rollback_history', []) if config else []
...
def get_plugin_versions(plugin_id: str):
    ...
    try:
        ...
        versions = {
            'current_version': config.get('version', '1.0.0') if config else '1.0.0',
            'update_history': config.get('update_history', []) if config else [],
            'rollback_history': config.get('rollback_history', []) if config else [],
            'available_versions': [
                '1.0.0',
                '1.1.0',
                '1.2.0',
...
def get_plugin_logs(plugin_id: str):
    """플러그인 로그 조회 (새로운 기능)"""
    try:
        logs = plugin_logs.get(plugin_id, []) if plugin_logs else []
        ...
        paginated_logs = filtered_logs[start_idx:end_idx] if filtered_logs is not None else None
...
def validate_plugin_structure(plugin_dir: str,  metadata: dict) -> dict:
    ...
    # 메타데이터 필드 확인
    for field in required_metadata_fields or []:
...
def check_plugin_compatibility(plugin_id: str, version=None) -> dict:
    """플러그인 호환성 검사"""
...
def generate_dummy_metrics(plugin_id: str, time_range: str) -> dict:
    ...
    return {
        'cpu_usage': {
            'current': cpu_data[-1] if cpu_data is not None else None,
        ...
        'memory_usage': {
            'current': memory_data[-1] if memory_data is not None else None,
        ...
        'response_time': {
            'current': response_time_data[-1] if response_time_data is not None else None,
...
def check_plugin_access(plugin_id, user):
    # 브랜드, 매장, 직원별 권한 우선순위로 체크
    checks = []
    if getattr(user, 'brand_id', None):
        checks.append(PluginAccessControl.query.filter_by(plugin_id=plugin_id, target_type='brand', target_id=str(user.brand_id), access_type='use', is_allowed=True).first())
    if getattr(user, 'branch_id', None):
        checks.append(PluginAccessControl.query.filter_by(plugin_id=plugin_id, target_type='store', target_id=str(user.branch_id), access_type='use', is_allowed=True).first())
    checks.append(PluginAccessControl.query.filter_by(plugin_id=plugin_id, target_type='user', target_id=str(user.id), access_type='use', is_allowed=True).first())
    return any(checks)

@plugin_management_bp.route('/plugins/attendance/clock-in', methods=['POST'])
@login_required
def plugin_attendance_clock_in():
    if not check_plugin_access('attendance', current_user):
        return jsonify({'success': False, 'error': '플러그인 사용 권한이 없습니다.'}), 403
    return jsonify({'success': True, 'message': '출근 기록 성공(샘플)'}), 200

@plugin_management_bp.route('/plugins/inventory/list', methods=['GET'])
@login_required
def plugin_inventory_list():
    if not check_plugin_access('inventory', current_user):
        return jsonify({'success': False, 'error': '플러그인 사용 권한이 없습니다.'}), 403
    return jsonify({'success': True, 'data': [{'item': '콜라', 'stock': 10}, {'item': '사이다', 'stock': 5}], 'message': '재고 조회 성공(샘플)'}), 200

@plugin_management_bp.route('/plugins/purchase/order', methods=['POST'])
@login_required
def plugin_purchase_order():
    if not check_plugin_access('purchase', current_user):
        return jsonify({'success': False, 'error': '플러그인 사용 권한이 없습니다.'}), 403
    return jsonify({'success': True, 'message': '구매 발주 성공(샘플)'}), 200

@plugin_management_bp.route('/plugins/schedule/list', methods=['GET'])
@login_required
def plugin_schedule_list():
    if not check_plugin_access('schedule', current_user):
        return jsonify({'success': False, 'error': '플러그인 사용 권한이 없습니다.'}), 403
    return jsonify({'success': True, 'data': [{'staff': '홍길동', 'date': '2024-07-17', 'shift': '오전'}], 'message': '스케줄 조회 성공(샘플)'}), 200

@plugin_management_bp.route('/plugins/ai-analytics/report', methods=['GET'])
@login_required
def plugin_ai_analytics_report():
    if not check_plugin_access('ai-analytics', current_user):
        return jsonify({'success': False, 'error': '플러그인 사용 권한이 없습니다.'}), 403
    return jsonify({'success': True, 'data': {'summary': 'AI 분석 리포트(샘플)'}, 'message': 'AI 분석 성공(샘플)'}), 200

@plugin_management_bp.route('/plugins/<plugin_id>/access-control/brand', methods=['POST'])
def set_plugin_access_brand(plugin_id):
    """브랜드별 플러그인 접근 권한 설정"""
    data = request.get_json()
    brand_id = data.get('brand_id')
    is_allowed = data.get('is_allowed', True)
    access_type = data.get('access_type', 'use')
    from models_main import PluginAccessControl
    pac = PluginAccessControl.query.filter_by(plugin_id=plugin_id, target_type='brand', target_id=str(brand_id), access_type=access_type).first()
    if not pac:
        pac = PluginAccessControl(plugin_id=plugin_id, target_type='brand', target_id=str(brand_id), access_type=access_type, is_allowed=is_allowed)
        db.session.add(pac)
    else:
        pac.is_allowed = is_allowed
    db.session.commit()
    return jsonify({'success': True, 'message': '브랜드별 플러그인 접근 권한이 설정되었습니다.'})

@plugin_management_bp.route('/plugins/<plugin_id>/access-control/branch', methods=['POST'])
def set_plugin_access_branch(plugin_id):
    """매장별 플러그인 접근 권한 설정"""
    data = request.get_json()
    ...
    access_type = data.get('access_type', 'use')
    from models_main import PluginAccessControl
    pac = PluginAccessControl.query.filter_by(plugin_id=plugin_id, target_type='store', target_id=str(branch_id), access_type=access_type).first()
    if not pac:
        pac = PluginAccessControl(plugin_id=plugin_id, target_type='store', target_id=str(branch_id), access_type=access_type, is_allowed=is_allowed)
        db.session.add(pac)
    else:
        pac.is_allowed = is_allowed
    db.session.commit()
    return jsonify({'success': True, 'message': '매장별 플러그인 접근 권한이 설정되었습니다.'})

@plugin_management_bp.route('/plugins/<plugin_id>/access-control/user', methods=['POST'])
def set_plugin_access_user(plugin_id):
    """직원별 플러그인 접근 권한 설정"""
    data = request.get_json()
    ...
    access_type = data.get('access_type', 'use')
    from models_main import PluginAccessControl
    pac = PluginAccessControl.query.filter_by(plugin_id=plugin_id, target_type='user', target_id=str(user_id), access_type=access_type).first()
    if not pac:
        pac = PluginAccessControl(plugin_id=plugin_id, target_type='user', target_id=str(user_id), access_type=access_type, is_allowed=is_allowed)
        db.session.add(pac)
    else:
        pac.is_allowed = is_allowed
    db.session.commit()
    return jsonify({'success': True, 'message': '직원별 플러그인 접근 권한이 설정되었습니다.'})
