...
from .models import {name.capitalize()}Model, db
import logging
    ...
    def deploy_to_marketplace(self, project_path, metadata):
        ...
        try:
            ...
            module_info = {
                ...
                "author": metadata.get("author", "Your Program"),
...
def create_project():
    ...
    try:
        ...
        project_type = data.get('type', 'module')  # module 또는 plugin
        name = data.get('name')
...
def convert_project():
    ...
    try:
        ...
        target_type = data.get('target_type')  # module 또는 plugin
            ...
            if plugin_dir.is_dir():
                ...
                if plugin_meta.exists():
                    ...
                    projects.append({
                        ...
                        "enabled": meta.get("settings", {}).get("enabled", True)
                    })
