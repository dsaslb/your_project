...
def start_security_scan():
    ...
    try:
        ...
        plugin_id = data.get('plugin_id')  # 특정 플러그인 또는 None (전체 스캔)
    ...
    try:
        ...
        for row in rows:
            profile = {
                ...
                'risk_level': row[1],
                'last_scan': row[2],
                'vulnerabilities_count': row[3],
                'malware_count': row[4],
                'security_events_count': row[5],
                'permissions': row[6],
                'network_access': row[7],
                'file_access': row[8],
                'api_calls': row[9],
