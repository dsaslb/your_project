...
def create_backup():
    ...
    try:
        ...
        include_files = request.json.get('include_files', False)
        ...
        if backup_type == 'full':
            backup_info['includes'] = ['database', 'files', 'configurations']
        elif backup_type == 'database':
            backup_info['includes'] = ['database']
        elif backup_type == 'files':
            backup_info['includes'] = ['files']
...
def manage_feature_flags():
    ...
    try:
        if request.method == 'GET':
            ...
            updated_flags = data.get('feature_flags', {})
...
def get_system_logs():
    ...
    try:
        ...
        # 레벨 필터링
        if log_level:
            sample_logs = [log for log in sample_logs if log['level'] == log_level.upper()]
