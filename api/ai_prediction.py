...
from core.backend.ai_prediction import (
...
def predict_inventory():
    ...
    try:
        ...
        # 결과 변환
        prediction_results = {}
        for item_name, prediction in predictions.items():
            prediction_results[item_name] = {
                'prediction_type': prediction.prediction_type.value,
                'model_type': prediction.model_type.value,
                'predicted_value': prediction.predicted_value,
                'confidence': prediction.confidence,
...
def get_model_performance():
    """모델 성능 조회"""
    try:
        model_type_str = request.args.get('model_type')
...
def get_prediction_history():
    """예측 히스토리 조회"""
    try:
        prediction_type_str = request.args.get('prediction_type')
