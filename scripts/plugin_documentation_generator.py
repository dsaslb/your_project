...
""",
    ...
    def _extract_plugin_info(
        self, plugin_path: Path, plugin_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """플러그인 정보 추출"""
    ...
    def _generate_examples(
        self, modules: List[ModuleDoc], plugin_info: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """예제 코드 생성"""
        examples = []
...
print(result)""",
    ...
    def _generate_examples_doc(
        self, documentation: PluginDocumentation, output_dir: Path
    ):
        """examples.md 생성"""
        try:
            ...
            content = template.render(
                plugin_name=documentation.plugin_name, examples=documentation.examples
            )
    ...
    def _generate_changelog_doc(
        ...
        try:
            ...
            content = template.render(
                plugin_name=documentation.plugin_name, changelog=documentation.changelog
            )
    ...
    def _generate_metadata(self, documentation: PluginDocumentation, output_dir: Path):
        ...
        try:
            metadata = {
                ...
                "generated_at": (
                    documentation.generated_at.isoformat()
                    if documentation.generated_at
                    else None
                ),
                "files": ["README.md", "API.md", "examples.md", "CHANGELOG.md"],
...
'''
                ...
                # 문서 생성
                generator.generate_documentation(str(plugin_path))

                print(f"플러그인 문서 생성 완료: {plugin_info['config']['name']}")

            except Exception as e:
                logger.error(
                    f"플러그인 {plugin_info['config']['name']} 문서 생성 실패: {e}"
                )
