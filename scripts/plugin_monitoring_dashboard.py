...
class PluginMonitoringDashboard:
    ...
    def __init__(self, config: DashboardConfig):
        ...
        self.performance_data: Dict[str, deque] = defaultdict(lambda: deque(maxlen=config.max_data_points))
    ...
    def setup_routes(self):
        ...
        self.app.router.add_post('/api/alert/resolve/{alert_id}', self.api_resolve_alert)
    ...
    async def api_performance(self, request):
        ...
        try:
            ...
            if self.monitor:
                ...
                return web.json_response({'status': 'error', 'message': '모니터링 시스템이 초기화되지 않았습니다'}, status=500)
    ...
    async def api_logs(self, request):
        ...
        try:
            ...
            if self.log_manager:
                logs = self.log_manager.get_logs(plugin_id=plugin_id, level=level, limit=limit)
                log_data = []
                ...
                return web.json_response({'status': 'error', 'message': '로그 시스템이 초기화되지 않았습니다'}, status=500)
    ...
    async def api_chart(self, request):
        ...
        try:
            ...
            if self.monitor:
                ...
                return web.Response(text="모니터링 시스템이 초기화되지 않았습니다", status=500)
    ...
    def update_plugin_status(self, plugin_id: str, metrics=None):
        ...
        if plugin_id not in self.plugin_status:
            self.plugin_status[plugin_id] = PluginStatus(plugin_id=plugin_id, is_active=True)
        ...
        if metrics:
            ...
            if metrics.success:
                status.success_rate = (status.success_rate * (status.execution_count - 1) + 1) / status.execution_count
            else:
                status.success_rate = (status.success_rate * (status.execution_count - 1)) / status.execution_count
            ...
            if status.execution_count == 1:
                ...
                status.avg_execution_time = (status.avg_execution_time * (status.execution_count - 1) + metrics.execution_time) / status.execution_count
                status.avg_memory_usage = (status.avg_memory_usage * (status.execution_count - 1) + metrics.memory_usage) / status.execution_count
                status.avg_cpu_usage = (status.avg_cpu_usage * (status.execution_count - 1) + metrics.cpu_usage) / status.execution_count
    ...
    async def start_dashboard(self):
        ...
        logger.info(f"플러그인 모니터링 대시보드가 시작되었습니다: http://{self.config.host}:{self.config.port}")
    ...
    async def _background_update_loop(self):
        ...
        while self.dashboard_running:
            try:
                ...
                if self.monitor:
                    for plugin_id in self.plugin_status.keys():
                        summary = self.monitor.get_performance_summary(plugin_id, 1)  # 최근 1시간
...
def main():
    ...
    parser.add_argument('--refresh-interval', type=int, default=30, help='새로고침 간격 (초)')
    parser.add_argument('--monitor-db', default='plugin_monitoring.db', help='모니터링 데이터베이스 경로')
    parser.add_argument('--log-db', default='plugin_logs.db', help='로그 데이터베이스 경로')
    parser.add_argument('--disable-websocket', action='store_true', help='웹소켓 비활성화')
    parser.add_argument('--disable-notifications', action='store_true', help='알림 비활성화')
