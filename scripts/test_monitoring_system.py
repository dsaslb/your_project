...
class MonitoringSystemTest:
    ...
    def test_monitoring_system(self):
        ...
        try:
            ...
            for plugin_id in ["test_plugin_0", "test_plugin_1", "test_plugin_2"]:
                ...
                assert summary['total_executions'] > 0, f"플러그인 {plugin_id}의 실행 기록이 없습니다"
            ...
            assert self.monitor.thresholds['execution_time'] == 10.0, "임계값 업데이트가 실패했습니다"
    ...
    def test_dashboard_system(self):
        ...
        try:
            ...
            assert self.dashboard.monitor is not None, "모니터링 컴포넌트가 연결되지 않았습니다"
            assert self.dashboard.log_manager is not None, "로깅 컴포넌트가 연결되지 않았습니다"
    ...
    def test_integration(self):
        ...
        try:
            ...
            plugin_logger.info("통합 테스트 실행 완료", context={'execution_time': 3.0, 'memory_usage': 100.0})
    ...
    def test_performance(self):
        ...
        try:
            ...
            assert processing_time < 10.0, f"메트릭 처리 시간이 너무 깁니다: {processing_time}초"
            ...
            assert processing_time < 15.0, f"로그 처리 시간이 너무 깁니다: {processing_time}초"
            ...
            assert memory_usage < 500, f"메모리 사용량이 너무 높습니다: {memory_usage}MB"
...
def main():
    ...
    parser.add_argument('--monitoring-only', action='store_true', help='모니터링 테스트만 실행')
    parser.add_argument('--logging-only', action='store_true', help='로깅 테스트만 실행')
