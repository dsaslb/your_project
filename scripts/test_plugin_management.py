...
def test_plugin_list():
    ...
    try:
        ...
        # 필터링 테스트
        params = {"category": "management", "enabled": "true", "page": 1, "per_page": 5}
...
def test_plugin_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"활성화된 플러그인 수: {data.get('status', {}).get('enabled_plugins', 0)}"
            )
            print(
                f"비활성화된 플러그인 수: {data.get('status', {}).get('disabled_plugins', 0)}"
            )
        else:
            print(f"오류: {response.text}")
            ...
            if plugins:
                ...
                if response.status_code == 200:
                    ...
                    response = requests.post(
                        f"{API_BASE}/plugins/{plugin_id}/{restore_action}"
                    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if plugins:
                ...
                # 필터링된 로그 조회
                params = {"level": "info", "page": 1, "per_page": 10}
                response = requests.get(
                    f"{API_BASE}/plugins/{plugin_id}/logs", params=params
                )
        ...
        if response.status_code == 200:
            ...
            if plugins:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"CPU 사용률: {metrics.get('cpu_usage', {}).get('current', 'N/A')}%"
                    )
                    print(
                        f"메모리 사용률: {metrics.get('memory_usage', {}).get('current', 'N/A')}MB"
                    )
                    print(
                        f"응답 시간: {metrics.get('response_time', {}).get('current', 'N/A')}ms"
                    )
                ...
                response = requests.get(
                    f"{API_BASE}/plugins/{plugin_id}/metrics", params=params
                )
...
def test_installation_history():
    ...
    try:
        ...
        # 필터링된 이력 조회
        params = {"action": "install", "page": 1, "per_page": 10}
