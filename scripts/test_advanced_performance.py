        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                for suggestion in suggestions[:3]:  # 처음 3개만 출력
                    print(
                        f"  - {suggestion.get('tuning_type')}: {suggestion.get('description', 'N/A')}"
                    )
            else:
                print(f"오류: {data.get('error', 'Unknown error')}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print(
                    f"모니터링 상태: {'활성화' if dashboard_data.get('monitoring_status', {}).get('active') else '비활성화'}"
                )

                alerts_summary = dashboard_data.get("alerts_summary", {})
                print(
                    f"알림 요약: 전체 {alerts_summary.get('total', 0)}개, 심각 {alerts_summary.get('critical', 0)}개, 경고 {alerts_summary.get('warning', 0)}개"
                )

                predictions_summary = dashboard_data.get("predictions_summary", {})
                print(
                    f"예측 요약: 전체 {predictions_summary.get('total', 0)}개, CPU {predictions_summary.get('cpu_predictions', 0)}개, 메모리 {predictions_summary.get('memory_predictions', 0)}개"
                )

                tuning_summary = dashboard_data.get("tuning_summary", {})
                print(
                    f"튜닝 요약: 전체 {tuning_summary.get('total_suggestions', 0)}개, 적용됨 {tuning_summary.get('applied_suggestions', 0)}개, 대기중 {tuning_summary.get('pending_suggestions', 0)}개"
                )

                current_metrics = dashboard_data.get("current_metrics", {})
                if current_metrics:
                    print(
                        f"현재 메트릭: CPU {current_metrics.get('cpu_usage', 'N/A')}%, 메모리 {current_metrics.get('memory_usage', 'N/A')}%"
                    )
            else:
                print(f"오류: {data.get('error', 'Unknown error')}")
        else:
