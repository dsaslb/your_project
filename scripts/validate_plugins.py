...
class PluginValidator:
    ...
    def validate_plugin(self, plugin_dir: Path) -> None:
        ...
        try:
            ...
            results["checks"]["performance"] = self.validate_performance(plugin_dir)  # pyright: ignore
    ...
    def validate_code_quality(self, plugin_dir: Path) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            try:
                ...
                if len(functions) > 50:
                    result["warnings"].append(f"{py_file.name}: 함수가 너무 많습니다 ({len(functions)}개)")
                
                if len(classes) > 20:
                    result["warnings"].append(f"{py_file.name}: 클래스가 너무 많습니다 ({len(classes)}개)")
                
                # 복잡도 검사
                complexity = self.calculate_complexity(tree)
                if complexity > 10:
                    result["warnings"].append(f"{py_file.name}: 복잡도가 높습니다 ({complexity})")
                
                # 임포트 검사
                imports = [node for node in ast.walk(tree) if isinstance(node, (ast.Import, ast.ImportFrom))]
                if len(imports) > 20:
                    result["warnings"].append(f"{py_file.name}: 임포트가 많습니다 ({len(imports)}개)")
            ...
            try:
                ...
                for pattern in dangerous_patterns:
                    if pattern in content:
                        result["warnings"].append(f"{py_file.name}: 잠재적 보안 위험 - {pattern}")
                
                # 하드코딩된 비밀번호/키 검사
                if any(keyword in content.lower() for keyword in ["password", "secret", "key", "token"]):
                    if any(line.strip().startswith(("password", "secret", "key", "token")) for line in content.split('\n')):
                        result["warnings"].append(f"{py_file.name}: 하드코딩된 비밀정보 가능성")
    ...
    def validate_dependencies(self, plugin_dir: Path) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            try:
                ...
                result["warnings"].append(f"{py_file.name}: 파일 읽기 오류 - {e}")  # noqa
            ...
            # 성능 관련 패턴 검사
                ...
                for pattern, issue in performance_issues:
                    if pattern in content:
                        result["warnings"].append(f"{py_file.name}: 성능 이슈 가능성 - {issue}")
                
                # 파일 크기 검사
                file_size = len(content)
                if file_size > 10000:  # 10KB
                    if file_size > 10000:  # 10KB
                        result["warnings"].append(f"{py_file.name}: 파일이 큽니다 ({file_size} bytes)")
    ...
    def validate_documentation(self, plugin_dir: Path) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            ...
            try:
                ...
                result["warnings"].append(f"{py_file.name}: AST 파싱 오류 - {e}")  # noqa
                ...
                for node in ast.walk(tree):
                    if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                        if not node.body or not isinstance(node.body[0], ast.Expr) or not isinstance(node.body[0].value, ast.Str):
                            result["warnings"].append(f"{py_file.name}: {node.name}에 docstring이 없습니다")
        
        if files_without_docstrings > len(python_files) * 0.5:
            result["warnings"].append(f"docstring이 없는 파일이 많습니다 ({files_without_docstrings}/{len(python_files)})")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        valid_plugins = sum(1 for result in self.validation_results.values() if result["valid"])
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": (valid_plugins / total_plugins * 100) if total_plugins > 0 else 0
            },
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        # 에러가 있는 플러그인들
        invalid_plugins = [name for name, result in self.validation_results.items() if not result["valid"]]
        if invalid_plugins:
            recommendations.append(f"다음 플러그인들의 오류를 수정하세요: {', '.join(invalid_plugins)}")
        ...
        if high_warning_plugins:
            recommendations.append(f"다음 플러그인들의 경고를 검토하세요: {', '.join(high_warning_plugins)}")
        ...
        if no_test_plugins:
            recommendations.append(f"다음 플러그인들에 테스트를 추가하세요: {', '.join(no_test_plugins)}")
        ...
        if no_docs_plugins:
            recommendations.append(f"다음 플러그인들의 문서화를 개선하세요: {', '.join(no_docs_plugins)}")
...
def main():
    ...
    # 리포트 파일 저장
    report_file = f"plugin_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
