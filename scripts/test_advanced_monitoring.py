...
class AdvancedMonitoringTester:
    ...
    def test_monitoring_status(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/status"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
                    ...
                    logger.info(
                        f"   - 모니터링 활성: {data['data']['monitoring_active']}"
                    )
    ...
    def test_plugins_list(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/plugins"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
                    plugins = data["data"]
                    logger.info(
                        f"✅ 플러그인 목록 조회 성공: {len(plugins)}개 플러그인"
                    )
    ...
    def test_plugin_summary(self, plugin_id):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/plugins/{plugin_id}/summary"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data["success"]:
                        ...
                        logger.error(
                            f"   ❌ {metric_type} 메트릭 기록 실패: {data['message']}"
                        )
                else:
                    logger.error(
                        f"   ❌ {metric_type} 메트릭 기록 HTTP 오류: {response.status_code}"
                    )
        ...
        for level in self.log_levels:
            try:
                log_data = {
                    ...
                    "context": {"test_id": f"test_{int(time.time())}", "level": level},
                }
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"   ❌ {level} 로그 기록 HTTP 오류: {response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data["success"]:
                        ...
                        logger.error(
                            f"   ❌ {event_type} 이벤트 기록 실패: {data['message']}"
                        )
                else:
                    logger.error(
                        f"   ❌ {event_type} 이벤트 기록 HTTP 오류: {response.status_code}"
                    )
    ...
    def test_metrics_retrieval(self, plugin_id):
        ...
        try:
            # 모든 메트릭 조회
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/plugins/{plugin_id}/metrics?hours=1"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
    ...
    def test_logs_retrieval(self, plugin_id):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/plugins/{plugin_id}/logs?hours=1"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
    ...
    def test_events_retrieval(self, plugin_id):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/plugins/{plugin_id}/events?hours=1"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
    ...
    def test_snapshots_retrieval(self, plugin_id):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/plugins/{plugin_id}/snapshots?hours=1"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
    ...
    def test_analytics_trends(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/analytics/trends?hours=24&metric_type=cpu_usage"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
                    ...
                    logger.info(
                        f"✅ 분석 트렌드 조회 성공: {len(trends)}개 데이터 포인트"
                    )
                    return True
                else:
    ...
    def test_performance_analytics(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/advanced-monitoring/analytics/performance?hours=24"
            )
            if response.status_code == 200:
                data = response.json()
                if data["success"]:
    ...
    def run_full_test(self):
        ...
        for plugin_id in self.test_plugins[:2]:  # 처음 2개만 테스트
            ...
            # 요약 테스트
            test_results.append(
                (f"{plugin_id} 요약", self.test_plugin_summary(plugin_id))
            )

            # 데이터 기록 테스트
            test_results.append(
                (f"{plugin_id} 메트릭 기록", self.test_metrics_recording(plugin_id))
            )
            test_results.append(
                (f"{plugin_id} 로그 기록", self.test_logs_recording(plugin_id))
            )
            test_results.append(
                (f"{plugin_id} 이벤트 기록", self.test_events_recording(plugin_id))
            )
            test_results.append(
                (f"{plugin_id} 스냅샷 생성", self.test_snapshot_creation(plugin_id))
            )

            # 데이터 조회 테스트
            test_results.append(
                (f"{plugin_id} 메트릭 조회", self.test_metrics_retrieval(plugin_id))
            )
            test_results.append(
                (f"{plugin_id} 로그 조회", self.test_logs_retrieval(plugin_id))
            )
            test_results.append(
                (f"{plugin_id} 이벤트 조회", self.test_events_retrieval(plugin_id))
            )
            test_results.append(
                (f"{plugin_id} 스냅샷 조회", self.test_snapshots_retrieval(plugin_id))
            )
        ...
        if plugins:
            simulation_thread = threading.Thread(
                target=self.simulate_real_time_data, args=(plugins[0]["plugin_id"], 30)
            )
