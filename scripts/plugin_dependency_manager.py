...
class PluginDependencyManager:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 인덱스 생성
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_dependencies_plugin_id ON plugin_dependencies(plugin_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_dependencies_dependency_id ON plugin_dependencies(dependency_id)"
                )
                conn.execute(
    ...
    def resolve_dependencies(
        self, plugin_id: str, target_version: Optional[str] = None
        ...
        try:
            ...
            for dep in dependencies or []:
                ...
                if not self._check_version_compatibility(
                    dep_info["version"], version_constraint
                ):
                    conflicts.append(
                        f"{dep_id}: 요구 버전 {version_constraint}, 사용 가능 버전 {dep_info['version']}"
                    )
                    continue
    ...
    def _check_version_compatibility(
        self, available_version: str, constraint: str
    ) -> bool:
        """버전 호환성 검사"""
        try:
    ...
    def _install_python_package(self, requirement: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"Python 패키지 설치 실패: {requirement} - {result.stderr}"
                )
                return False
    ...
    def check_compatibility(
        ...
        try:
            ...
            if not self._check_version_compatibility(
                current_version, f">={target_version}"
            ):
                issues.append(
                    f"버전 호환성 문제: 현재 {current_version}, 요구 {target_version}"
                )
            ...
            for dep in dependencies:
                ...
                if dep_info:
                    if not self._check_version_compatibility(
                        dep_info["version"], dep["version_constraint"]
                    ):
    ...
    def export_dependency_report(self, output_path: str, plugin_ids: List[str] = []):
        """의존성 보고서 내보내기"""
        try:
            report = {"generated_at": datetime.utcnow().isoformat(), "plugins": []}
