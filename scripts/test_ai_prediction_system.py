...
class AIPredictionSystemTester:
            ...
            if response.status_code == 200:
                ...
                if "sales" in predictions:
                    ...
                    for pred in sales_data[:3]:  # 처음 3개만 로그
                        logger.info(
                            f"  - {pred['date']}: {pred['predicted_value']:.0f}원 (신뢰도: {pred['confidence']:.3f})"
                        )
                ...
                if "customer_flow" in predictions:
                    ...
                    logger.info(
                        f"예상 고객 수: {customer_data.get('total_predicted_customers', 'N/A')}"
                    )
                ...
                if "staffing" in predictions:
                    ...
                    logger.info(
                        f"필요 인력: {staffing_data.get('needed_staff', 'N/A')}"
                    )
                    logger.info(
                        f"현재 인력: {staffing_data.get('current_staff', 'N/A')}"
                    )
    ...
    def test_prediction_accuracy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if accuracy_trends:
                    logger.info(
                        f"전체 평균 정확도: {accuracy_trends.get('overall_accuracy', 'N/A')}"
                    )
                    logger.info(
                        f"총 예측 수: {accuracy_trends.get('total_predictions', 'N/A')}"
                    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for insight in insights:
                    logger.info(
                        f"  - {insight['title']} ({insight['priority']}): {insight['description']}"
                    )
    ...
    def test_prediction_alerts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for alert in alerts:
                    logger.info(
                        f"  - {alert['title']} ({alert['severity']}): {alert['description']}"
                    )
    ...
    def cleanup_old_test_files(self, days_to_keep: int = 7):
        ...
        try:
            ...
            pattern = "ai_prediction_test_results_*.json"
