...
class PluginLogManager:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_plugin_logs_timestamp ON plugin_logs(timestamp)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_plugin_logs_level ON plugin_logs(level)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_plugin_logs_trace_id ON plugin_logs(trace_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_log_analytics_plugin_date ON log_analytics(plugin_id, date)")
    ...
    def get_logs(self, plugin_id: Optional[str] = None, level: Optional[str] = None,
    ...
    def search_logs(self, search_term: str, plugin_id: Optional[str] = None,
                   level: Optional[str] = None, case_sensitive: bool = False) -> List[LogEntry]:
        """로그 검색"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                ...
                for row in cursor.fetchall():
                    ...
                    # 대소문자 구분 검색
                    if case_sensitive or search_term.lower() in log_entry.message.lower():
    ...
    def get_log_statistics(self, plugin_id: Optional[str] = None, 
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                plugin_stats = {row['plugin_id']: row['count'] for row in cursor.fetchall()}
    ...
    def analyze_log_patterns(self, plugin_id: Optional[str] = None, 
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                patterns = [(row['pattern'], row['description'], row['severity']) 
                           for row in cursor.fetchall()]
            
            # 로그에서 패턴 매칭
            logs = self.get_logs(plugin_id=plugin_id, start_time=start_time, limit=10000)
    ...
    def export_logs(self, output_path: str, plugin_id: Optional[str] = None,
                   start_time: Optional[datetime] = None, end_time: Optional[datetime] = None,
                   format: str = 'json'):
        """로그 내보내기"""
        try:
            logs = self.get_logs(plugin_id=plugin_id, start_time=start_time, 
            ...
            if format.lower() == 'json':
                with open(output_path, 'w', encoding='utf-8') as f:
                    ...
                    } for log in logs], f, indent=2, ensure_ascii=False, default=str)
            
            elif format.lower() == 'csv':
                import csv
                with open(output_path, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(['timestamp', 'plugin_id', 'level', 'message', 
                                   'module', 'function', 'line_number', 'trace_id'])
    ...
    def get_error_summary(self, plugin_id: Optional[str] = None, 
                         hours: int = 24) -> Dict[str, Any]:
        """오류 요약 조회"""
        try:
            start_time = datetime.now() - timedelta(hours=hours)
            
            error_logs = self.get_logs(plugin_id=plugin_id, level='ERROR', 
            ...
            for log in error_logs:
                ...
                normalized_message = re.sub(r'[\\\/][^\\\/]+[\\\/]', '/path/', normalized_message)
                error_groups[normalized_message].append(log)
            
            # 상위 오류 정렬
            top_errors = sorted(error_groups.items(), 
...
def main():
    ...
    parser.add_argument('--cleanup', type=int, metavar='DAYS', help='오래된 로그 정리 (일)')
    ...
    try:
        if args.stats:
            ...
            print(f"플러그인별 분포: {dict(list(stats.get('plugin_distribution', {}).items())[:5])}")
            ...
            log_manager.export_logs(args.export, args.plugin_id, start_time, 
                                  format=args.format)
            ...
            # 기본 로그 조회
            start_time = datetime.now() - timedelta(hours=args.hours)
            logs = log_manager.get_logs(args.plugin_id, args.level, start_time, 
                                      limit=args.limit)
            ...
            for log in logs:
                ...
                if log.exception_info:
                    print(f"  예외: {log.exception_info.split('Traceback')[-1].strip()}")
