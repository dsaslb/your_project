        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for notif in notifications[:3]:  # 처음 3개만 로그
                    logger.info(f"  - {notif.get('title', notif.get('message'))} ({notif.get('type')})")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for channel in channels:
                    logger.info(f"  - {channel.get('name')} ({channel.get('type')}): {channel.get('enabled')}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for template in templates:
                    logger.info(f"  - {template.get('name')} ({template.get('category')})")
...
def main():
    ...
    parser.add_argument('--base-url', default='http://localhost:5000', help='서버 기본 URL')
