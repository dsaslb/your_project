...
class PluginMonitoringDeployer:
    ...
    def build_docker_images(self):
        ...
        try:
            ...
            self.log_step(
                "Build Backend Image", "PASSED", "Backend image built successfully"
            )
            ...
            self.log_step(
                "Build Frontend Image", "PASSED", "Frontend image built successfully"
            )
    ...
    def push_docker_images(self):
        ...
        try:
            ...
            self.log_step(
                "Push Backend Image", "PASSED", "Backend image pushed successfully"
            )
            ...
            self.log_step(
                "Push Frontend Image", "PASSED", "Frontend image pushed successfully"
            )
    ...
    def deploy_to_kubernetes(self):
        ...
        try:
            ...
            self.log_step(
                "Create Namespace",
                "PASSED",
                f"Namespace {self.config['namespace']} ready",
            )
            ...
            self.log_step(
                "Deploy Plugin Monitoring",
                "PASSED",
                "Plugin monitoring deployed successfully",
            )
        ...
        try:
            ...
            for i in range(timeout // 10):
                ...
                if success and "Running" in output:
                    self.log_step(
                        "Wait for Deployment", "PASSED", "Deployment is ready"
                    )
                    return True
    ...
    def run_health_checks(self):
        ...
        try:
            ...
            response = requests.get(
                f"{self.config['base_url']}/api/admin/plugin-monitoring/health",
                timeout=30,
            )

            if response.status_code == 200:
                self.log_step(
                    "Health Check", "PASSED", "Plugin monitoring health check passed"
                )
            else:
                self.log_step(
                    "Health Check",
            ...
            for endpoint in endpoints:
                response = requests.get(
                    f"{self.config['base_url']}{endpoint}", timeout=10
                )

                if response.status_code == 200:
                    self.log_step(
                        f"API Check: {endpoint}", "PASSED", "API endpoint accessible"
                    )
                else:
                    self.log_step(
                        f"API Check: {endpoint}",
                        "FAILED",
    ...
    def run_smoke_tests(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.config['base_url']}/api/admin/plugin-monitoring/start",
                timeout=30,
            )

            if response.status_code == 200:
                self.log_step(
                    "Smoke Test: Start Monitoring",
                    "PASSED",
                ...
                self.log_step(
                    "Smoke Test: Start Monitoring",
                    "FAILED",
                    f"Start failed: {response.status_code}",
                )
                return False
            ...
            # 모니터링 중지 테스트
            response = requests.post(
                f"{self.config['base_url']}/api/admin/plugin-monitoring/stop",
                timeout=30,
            )

            if response.status_code == 200:
                self.log_step(
                    "Smoke Test: Stop Monitoring",
                    "PASSED",
                ...
                self.log_step(
                    "Smoke Test: Stop Monitoring",
                    "FAILED",
                    f"Stop failed: {response.status_code}",
                )
                return False
    ...
    def rollback_deployment(self):
        ...
        try:
            ...
            if success:
                self.log_step(
                    "Rollback", "PASSED", "Deployment rolled back successfully"
                )
                return True
            else:
    ...
    def deploy(self):
        ...
        try:
            ...
            if not self.wait_for_deployment():
                self.log_step(
                    "Rollback", "INFO", "Deployment failed, attempting rollback"
                )
            ...
            if not self.run_health_checks():
                self.log_step(
                    "Rollback", "INFO", "Health checks failed, attempting rollback"
                )
                self.rollback_deployment()
                self.deployment_log["status"] = "FAILED"
                self.save_deployment_log()
            ...
            if not self.run_smoke_tests():
                self.log_step(
                    "Rollback", "INFO", "Smoke tests failed, attempting rollback"
                )
                self.rollback_deployment()
                self.deployment_log["status"] = "FAILED"
                self.save_deployment_log()
...
def main():
    ...
    parser.add_argument(
        "--skip-push", action="store_true", help="Skip Docker push step"
    )
