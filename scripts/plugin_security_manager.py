...
class PluginSecurityManager:
    def __init__(self, base_path: str = "plugins"):
        ...
        self.security_config = {
            ...
            "allowed_permissions": [
                "read", "write", "execute", "network", "database"
            ]
        }
        
    def generate_security_key(self) -> str:
        """보안 키 생성"""
        return base64.b64encode(secrets.token_bytes(self.security_config["key_length"])).decode()
    ...
    def sign_plugin(self, plugin_id: str, private_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                "expires_at": (datetime.utcnow() + timedelta(days=self.security_config["signature_expiry_days"])).isoformat(),
    ...
    def verify_plugin_signature(self, plugin_id: str, public_key: str) -> Dict[str, Any]:
        """플러그인 서명 검증"""
        try:
            plugin_path = self.base_path / plugin_id
            signature_path = plugin_path / "config" / "plugin.sig"
    ...
    def scan_plugin_security(self, plugin_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for py_file in plugin_path.rglob("*.py"):
                ...
                scan_results["file_analysis"][str(py_file.relative_to(plugin_path))] = file_analysis
                
                # 취약점 점수 계산
                if file_analysis["vulnerabilities"]:
                    scan_results["vulnerabilities"].extend(file_analysis["vulnerabilities"])
                    scan_results["security_score"] -= len(file_analysis["vulnerabilities"]) * 10
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if alias.name in self.security_config["forbidden_imports"]:
                        analysis["vulnerabilities"].append(f"금지된 import: {alias.name}")
            ...
            # 함수 호출 분석
                ...
                if isinstance(node.func, ast.Name):
                    ...
                    if func_name in self.security_config["forbidden_functions"]:
                        analysis["vulnerabilities"].append(f"금지된 함수 호출: {func_name}")
                ...
                if isinstance(node.value, ast.Call):
                    if isinstance(node.value.func, ast.Name):
                        if node.value.func.id in ["eval", "exec"]:
                            analysis["vulnerabilities"].append(f"위험한 함수 사용: {node.value.func.id}")
    ...
    def manage_plugin_permissions(self, plugin_id: str, permissions: List[str]) -> Dict[str, Any]:
        """플러그인 권한 관리"""
        try:
            plugin_path = self.base_path / plugin_id
            if not plugin_path.exists():
            ...
            # 권한 검증
            invalid_permissions = [p for p in permissions if p not in self.security_config["allowed_permissions"]]
    ...
    def generate_security_report(self, plugin_id: str) -> str:
        ...
        try:
            ...
            # 권장사항 생성
            if scan_results["security_score"] < 80:
                report["recommendations"].append("보안 점수가 낮습니다. 취약점을 수정하세요.")
