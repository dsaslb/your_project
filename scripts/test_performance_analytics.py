...
class PerformanceAnalyticsTester:
    ...
    def test_analytics_start_stop(self) -> bool:
        ...
        try:
            ...
            if not data.get('success'):
                self.log_test("분석 시스템 시작", False, data.get('error', '알 수 없는 오류'))
                return False
            
            self.log_test("분석 시스템 시작", True, "분석 시스템이 성공적으로 시작되었습니다")
            ...
            if not data.get('success'):
                self.log_test("분석 시스템 중지", False, data.get('error', '알 수 없는 오류'))
                return False
            
            self.log_test("분석 시스템 중지", True, "분석 시스템이 성공적으로 중지되었습니다")
            return True
        ...
        try:
            ...
            for i, metric in enumerate(test_metrics):
                response = self.session.post(
                    f"{self.base_url}/api/performance/collect",
                    json=metric
                )
                data = response.json()
                ...
                if data.get('success'):
                    success_count += 1
                    self.log_test(f"메트릭 수집 {i+1}", True, f"{metric['metric_type']} 수집 성공")
                else:
                    self.log_test(f"메트릭 수집 {i+1}", False, data.get('error', '알 수 없는 오류'))
    ...
    def test_performance_analysis(self) -> bool:
        ...
        try:
            # 분석 수행
            response = self.session.post(
                f"{self.base_url}/api/performance/analyze",
                json={'period_hours': 1}
            )
            data = response.json()
            ...
            # 분석 결과 검증
            required_fields = ['analysis_id', 'health_score', 'metrics_summary', 'bottlenecks', 'recommendations']
            ...
            self.log_test("성능 분석", True, f"분석 완료 (건강도: {analysis_data['health_score']:.1f})")
            return True
    ...
    def test_performance_report(self) -> bool:
        ...
        try:
            ...
            if not data.get('success'):
                self.log_test("성능 리포트", False, data.get('error', '알 수 없는 오류'))
                return False
            ...
            self.log_test("성능 리포트", True, f"리포트 조회 성공 (ID: {report_data['analysis_id']})")
            return True
    ...
    def test_optimization_suggestions(self) -> bool:
        ...
        try:
            ...
            if not data.get('success'):
                self.log_test("최적화 제안사항", False, data.get('error', '알 수 없는 오류'))
                return False
            
            suggestions_data = data.get('data', {})
            suggestions = suggestions_data.get('suggestions', [])
            
            self.log_test("최적화 제안사항", True, f"제안사항 {len(suggestions)}개 조회 성공")
            return True
    ...
    def test_metrics_summary(self) -> bool:
        ...
        try:
            ...
            if not data.get('success'):
                self.log_test("메트릭 요약", False, data.get('error', '알 수 없는 오류'))
                return False
            ...
            # 주요 메트릭 검증
            expected_metrics = ['avg_response_time', 'avg_memory_usage', 'avg_cpu_usage', 'avg_error_rate']
    ...
    def test_bottlenecks(self) -> bool:
        ...
        try:
            ...
            self.log_test("병목 지점", True, f"병목 지점 {len(bottlenecks)}개 조회 성공")
            return True
    ...
    def test_trends(self) -> bool:
        ...
        try:
            ...
            if not data.get('success'):
                self.log_test("트렌드 분석", False, data.get('error', '알 수 없는 오류'))
                return False
    ...
    def test_analytics_status(self) -> bool:
        ...
        try:
            ...
            if not data.get('success'):
                self.log_test("분석 시스템 상태", False, data.get('error', '알 수 없는 오류'))
                return False
            
            status_data = data.get('data', {})
            
            # 상태 정보 검증
            required_fields = ['running', 'analysis_count', 'metrics_storage', 'config', 'thresholds']
            missing_fields = [field for field in required_fields if field not in status_data]
            
            if missing_fields:
                self.log_test("분석 시스템 상태", False, f"필수 필드 누락: {missing_fields}")
                return False
            
            self.log_test("분석 시스템 상태", True, f"상태 조회 성공 (실행중: {status_data['running']})")
            return True
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        # 결과 파일 저장
        with open('performance_analytics_test_results.json', 'w', encoding='utf-8') as f:
...
def main():
    ...
    parser.add_argument('--output', default='performance_analytics_test_results.json', help='결과 파일 경로')
