...
class PluginDeploymentTester:
    ...
    def test_plugin_upload(self, plugin_file: str) -> Dict[str, Any]:
        ...
        try:
            with open(plugin_file, 'rb') as f:
                files = {'plugin': f}
                response = requests.post(f"{self.base_url}/api/admin/plugin-management/upload", 
    ...
    def test_plugin_installation(self, plugin_name: str) -> Dict[str, Any]:
        """플러그인 설치 테스트"""
        try:
            response = requests.post(f"{self.base_url}/api/admin/plugin-management/install/{plugin_name}", 
                                   timeout=30)
    ...
    def test_plugin_activation(self, plugin_name: str) -> Dict[str, Any]:
        """플러그인 활성화 테스트"""
        try:
            response = requests.post(f"{self.base_url}/api/admin/plugin-management/activate/{plugin_name}", 
                                   timeout=30)
    ...
    def test_plugin_functionality(self, plugin_name: str) -> Dict[str, Any]:
        ...
        try:
            # 플러그인 API 엔드포인트 테스트
            response = requests.get(f"{self.base_url}/api/plugins/{plugin_name}/status", timeout=10)
    ...
    def test_plugin_deactivation(self, plugin_name: str) -> Dict[str, Any]:
        """플러그인 비활성화 테스트"""
        try:
            response = requests.post(f"{self.base_url}/api/admin/plugin-management/deactivate/{plugin_name}", 
                                   timeout=30)
    ...
    def test_plugin_uninstallation(self, plugin_name: str) -> Dict[str, Any]:
        """플러그인 제거 테스트"""
        try:
            response = requests.delete(f"{self.base_url}/api/admin/plugin-management/uninstall/{plugin_name}", 
                                     timeout=30)
    ...
    def run_full_deployment_test(self, plugin_file: str, plugin_name: str) -> Dict[str, Any]:
        """전체 배포 테스트 실행"""
        ...
        for step_name, test_func in test_sequence:
            ...
            if not result.get('success', False):
                ...
                logger.error(f"테스트 실패: {step_name} - {result.get('error', 'Unknown error')}")
                break
