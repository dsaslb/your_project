...
class PluginPerformanceMonitor:
                ...
                if key.startswith(f"{plugin_id}_"):
                    if self.metrics_history[key]:
                        ...
                        if latest_timestamp is None or entry['timestamp'] > latest_timestamp:
                            ...
                            latest_metrics = self._reconstruct_metrics(plugin_id, latest_timestamp)
    ...
    def _reconstruct_metrics(self, plugin_id: str, timestamp: str) -> PerformanceMetrics:
        """타임스탬프로부터 성능 지표 재구성"""
        metrics_data = {}
    ...
    def get_metrics_history(self, plugin_id: str, metric_name: str, 
    ...
    def _calculate_plugin_summary(self, plugin_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            metric_names = ['cpu_percent', 'memory_rss', 'memory_percent', 'thread_count']
    ...
    def generate_performance_report(self, plugin_id: str, 
        ...
        try:
            ...
            if output_format == "json":
                report_path = Path("logs") / f"performance_report_{plugin_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
...
"""
            ...
            report_path = Path("logs") / f"performance_report_{report['plugin_id']}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.html"
            with open(report_path, 'w', encoding='utf-8') as f:
    ...
    def detect_memory_leak(self, plugin_id: str, threshold: float = 0.1) -> Dict[str, Any]:
        """메모리 누수 감지"""
        try:
            memory_history = self.get_metrics_history(plugin_id, 'memory_rss', 24)  # 24시간
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            if history:
                print(json.dumps(history[-5:], indent=2, ensure_ascii=False))  # 최근 5개
        elif choice == "5":
            plugin_id = input("플러그인 ID: ").strip()
            format_choice = input("출력 형식 (json/html/text, 기본값: json): ").strip() or "json"
            ...
            if bottlenecks:
                ...
                for i, bottleneck in enumerate(bottlenecks, 1):
                    print(f"{i}. [{bottleneck['severity'].upper()}] {bottleneck['message']}")
            else:
