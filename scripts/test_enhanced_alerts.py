...
class EnhancedAlertTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    status = data["data"]
                    logger.info(
                        f"✅ 시스템 상태: 모니터링={status['monitoring_active']}"
                    )
    ...
    def test_active_alerts(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/enhanced-alerts/alerts/active"
            )
    ...
    def test_alert_history(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/enhanced-alerts/alerts/history?hours=24"
            )
    ...
    def test_channel_configs(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/enhanced-alerts/channels/config"
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    for channel, config in configs.items():
                        logger.info(
                            f"  - {channel}: 활성={config['enabled']}, 설정={config['configured']}"
                        )
                    return True
                else:
    ...
    def test_statistics(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/enhanced-alerts/statistics"
            )
    ...
    def test_create_alert_rule(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/enhanced-alerts/rules", json=rule_data
            )
    ...
    def test_channel_test(self) -> bool:
        ...
        try:
            ...
            # web 채널 테스트
            response = self.session.post(
                f"{self.base_url}/api/enhanced-alerts/test/web"
            )
        ...
        try:
            ...
            for plugin_id, plugin_name in test_plugins:
                ...
                for i in range(10):
                    ...
                    logger.info(
                        f"메트릭 업데이트: {plugin_name} - CPU: {cpu_usage:.1f}%, 메모리: {memory_usage:.1f}%"
                    )
...
def main():
    ...
    if api_success:
        ...
        simulation_thread = threading.Thread(
            target=tester.simulate_plugin_metrics, daemon=True
        )
