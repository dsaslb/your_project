...
class PluginIsolationRunner:
    ...
    def create_isolation_environment(self, plugin_id: str, config: Optional[Dict] = None) -> Dict[str, Any]:
        """격리 실행 환경 생성"""
        try:
            ...
            # 임시 격리 디렉토리 생성
            isolation_path = Path(tempfile.mkdtemp(prefix=f"plugin_isolated_{plugin_id}_"))
    ...
    def run_plugin_isolated(self, plugin_id: str, entry_point: str = "main.py", 
                           args: Optional[List[str]] = None, config: Optional[Dict] = None) -> Dict[str, Any]:
        """플러그인을 격리된 환경에서 실행"""
        try:
            # 격리 환경 생성
            ...
            monitor_thread = threading.Thread(
                target=self._monitor_plugin_execution,
                args=(process_id,),
                daemon=True
            )
    ...
    def get_plugin_status(self, process_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                "status": execution_info["status"],
                "start_time": execution_info["start_time"].isoformat(),
                "elapsed_time": (datetime.utcnow() - execution_info["start_time"]).total_seconds(),
                "pid": process.pid,
    ...
    def get_execution_history(self, plugin_id: Optional[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
        """실행 히스토리 조회"""
        try:
            log_path = Path("logs") / "plugin_execution"
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            args = [arg.strip() for arg in args_input.split(",") if arg.strip()] if args_input else None
            ...
            if running_list:
                ...
                for plugin in running_list:
                    print(f"  - {plugin['plugin_id']} (PID: {plugin['process_id']}) - {plugin['status']}")
            else:
            ...
            if history:
                ...
                for entry in history:
                    print(f"  - {entry['plugin_id']} ({entry['start_time']}) - {entry['status']}")
            else:
                print("실행 히스토리가 없습니다.")
        else:
            print("❌ 잘못된 선택입니다.")
