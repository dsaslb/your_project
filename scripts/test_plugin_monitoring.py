...
class PluginMonitoringTester:
    ...
    def generate_plugin_metrics(self, plugin: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_usage = max(0, min(100, plugin['baseline_memory'] + memory_variation))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    logger.warning(f"플러그인 {plugin_id} 메트릭 전송 실패: {data.get('error')}")
                    return False
            else:
                logger.error(f"플러그인 {plugin_id} 메트릭 전송 실패: HTTP {response.status_code}")
                return False
    ...
    def send_test_alert(self, title: str, message: str, plugin_id: Optional[str] = None) -> bool:
        """테스트 알림 발송"""
        try:
    ...
    def run_test_cycle(self, duration: int = 3600):
        ...
        try:
            while time.time() - start_time < duration:
                ...
                logger.info(f"테스트 사이클 {cycle_count} 시작 (시나리오: {self.current_scenario})")
                ...
                for plugin in self.test_plugins:
                    ...
                    if metrics:
                        ...
                        if success:
                            ...
                            logger.warning(f"플러그인 {plugin['name']} 메트릭 전송 실패")
                ...
                if cycle_count % 10 == 0:
                    ...
                    if stats.get('success'):
                        ...
                        logger.info(f"알림 통계 - 활성: {statistics.get('active_alerts', 0)}, "
    ...
    def run_scenario_test(self):
        ...
        for scenario, duration in scenarios:
            ...
            if stats.get('success'):
                statistics = stats.get('statistics', {})
                logger.info(f"시나리오 '{scenario}' 완료 - "
...
def main():
    ...
    parser.add_argument('--mode', choices=['cycle', 'scenario', 'manual'], default='manual', help='테스트 모드')
    parser.add_argument('--duration', type=int, default=3600, help='테스트 지속시간 (초)')
