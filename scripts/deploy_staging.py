...
class StagingDeployer:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(config, dict):  # pyright: ignore
                logger.error("설정 파일이 올바른 딕셔너리 형식이 아닙니다.")  # pyright: ignore
                sys.exit(1)
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            ...
            if result.stdout.strip():
                self.log_deployment("prerequisites", "warning", "작업 디렉토리에 커밋되지 않은 변경사항이 있습니다")
            else:
        ...
        if missing_vars:
            self.log_deployment("prerequisites", "error", f"필수 환경 변수 없음: {', '.join(missing_vars)}")
            return False
        else:
    ...
    def backup_current_deployment(self) -> bool:
        ...
        try:
            # 백업 디렉토리 생성
            backup_dir = Path("backups") / f"staging_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            backup_dir.mkdir(parents=True, exist_ok=True)
            
            # 현재 코드 백업
            subprocess.run(["git", "archive", "--format=tar", "--output", str(backup_dir / "code.tar"), "HEAD"], check=True)
            ...
            for config_file in config_files:
                if Path(config_file).exists():
                    subprocess.run(["cp", config_file, str(config_backup / config_file)], check=True)
        ...
        try:
            ...
            with Connection(
                ...
                if self.config['database']['type'] == 'postgresql':
                    ...
                    conn.run(f"pg_dump {self.config['database']['name']} > /tmp/{backup_file}")
    ...
    def build_docker_image(self) -> bool:
        ...
        try:
            ...
            # 이미지 태그 생성
            image_tag = f"plugin-system:staging-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            
            # Docker 빌드
            subprocess.run([
                "docker", "build", 
                "-t", image_tag,
                "--build-arg", f"ENVIRONMENT=staging",
                "--build-arg", f"BUILD_DATE={datetime.now().isoformat()}",
                "."
            ], check=True)
            
            self.log_deployment("docker_build", "success", f"이미지 빌드 완료: {image_tag}")
            return True
    ...
    def deploy_code(self, conn: Connection, deploy_dir: str):
        """코드 배포"""
        # Git 아카이브 생성
        subprocess.run(["git", "archive", "--format=tar", "--output", "deploy.tar", "HEAD"], check=True)
...
LOG_LEVEL=INFO
    ...
    def run_database_migration(self, conn: Connection, deploy_dir: str):
        ...
        try:
            ...
            self.log_deployment("migration", "success", "데이터베이스 마이그레이션 완료")
        except Exception as e:
            ...
            with Connection(
                ...
                for plugin_path in Path("plugins").iterdir():
                    if plugin_path.is_dir():
                        ...
                        # 플러그인 아카이브 생성
                        subprocess.run([
                            "tar", "-czf", f"{plugin_name}.tar.gz", 
                            "-C", "plugins", plugin_name
                        ], check=True)
                        
                        # 원격 서버로 전송
                        conn.put(f"{plugin_name}.tar.gz", f"{plugin_dir}/{plugin_name}.tar.gz")
    ...
    def setup_monitoring(self) -> bool:
        ...
        try:
            ...
            with Connection(
                ...
                # Prometheus 설정
                if self.config['monitoring'].get('prometheus', {}).get('enabled', False):
...
volumes:
  grafana-data:
        ...
        conn.put("docker-compose.grafana.yml", "/opt/plugin-system/docker-compose.grafana.yml")
        conn.run("cd /opt/plugin-system && docker-compose -f docker-compose.grafana.yml up -d")
...
volumes:
  prometheus-data:
        ...
        conn.put("docker-compose.prometheus.yml", "/opt/plugin-system/docker-compose.prometheus.yml")
        conn.run("cd /opt/plugin-system && docker-compose -f docker-compose.prometheus.yml up -d")
