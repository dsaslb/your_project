...
class PluginDebugTools:
    ...
    def enable_debug_mode(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            if not config_path.exists():
                self.logger.error(f"플러그인 설정 파일이 존재하지 않습니다: {config_path}")
                return False
    ...
    def disable_debug_mode(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            self.debug_mode = False
            self.logger.info(f"✅ {plugin_id} 플러그인 디버그 모드가 비활성화되었습니다.")
            return True
    ...
    def diagnose_plugin_health(self, plugin_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 파일 구조 검사
            diagnosis['checks']['file_structure'] = self._check_file_structure(plugin_path)
            ...
            # 전체 상태 평가
            diagnosis['overall_health'] = self._evaluate_overall_health(diagnosis['checks'])
    ...
    def _check_config_file(self, plugin_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'error',
                'error': f'설정 파일 JSON 파싱 오류: {e}'
            }
        except Exception as e:
            return {
                'status': 'error',
                'error': f'설정 파일 검사 중 오류: {e}'
            }
    ...
    def generate_debug_report(self, plugin_id: str) -> str:
        ...
        try:
            ...
            # 리포트 파일 생성
            report_path = Path("logs") / f"debug_report_{plugin_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            report = {
                ...
                'performance_data': dict(self.performance_data) if plugin_id in self.performance_data else {}
            }
