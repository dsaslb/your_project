...
class CICDSystemTester:
    ...
    def check_required_files(self) -> bool:
        ...
        if missing_files:
            self.log_test("필수 파일 확인", False, f"누락된 파일: {', '.join(missing_files)}")
            return False
    ...
    def check_environment_variables(self) -> bool:
        ...
        if missing_vars:
            self.log_test("환경 변수 확인", False, f"누락된 환경 변수: {', '.join(missing_vars)}")
            return False
    ...
    def test_unit_tests(self) -> bool:
        ...
        try:
            ...
            message = f"테스트 완료 - 커버리지: {coverage_match}" if coverage_match else "테스트 완료"
            ...
            for test_name, method, endpoint in api_tests:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.log_test(f"API 테스트 - {test_name}", False, f"상태 코드: {response.status_code}")
                        all_passed = False
    ...
    def test_docker_build(self) -> bool:
        ...
        try:
            ...
            if success:
                # 테스트 이미지 정리
                subprocess.run(["docker", "rmi", "plugin-system:test"], capture_output=True)
    ...
    def test_deployment_simulation(self) -> bool:
        ...
        try:
            ...
            required_sections = ['staging', 'production', 'database', 'plugins', 'monitoring']
            # config가 None이거나 dict가 아닐 경우를 안전하게 처리합니다.
            if not isinstance(config, dict):  # pyright: ignore
                self.log_test("배포 시뮬레이션", False, "배포 설정 파일이 올바른 YAML 객체가 아닙니다")
                return False
            missing_sections = [section for section in required_sections if section not in config]  # pyright: ignore
            
            if missing_sections:
                self.log_test("배포 시뮬레이션", False, f"누락된 설정 섹션: {', '.join(missing_sections)}")
                return False
    ...
    def save_detailed_results(self):
        ...
        report_file = f"cicd_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
