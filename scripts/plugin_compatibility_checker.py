...
class PluginCompatibilityChecker:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 인덱스 생성
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_compatibility_plugin_id ON compatibility_results(plugin_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_matrix_plugin_id ON compatibility_matrix(plugin_id)"
                )
    ...
    def check_plugin_compatibility(
        self, plugin_id: str, plugin_version: Optional[str] = None
    ) -> CompatibilityReport:
        """플러그인 호환성 종합 검사"""
        try:
            # 플러그인 정보 조회
            ...
            # 2. 의존성 호환성 검사
            dep_issues, checked_deps = self._check_dependency_compatibility(
                plugin_id, plugin_version
            )
            ...
            # 3. 시스템 요구사항 검사
            sys_issues, checked_reqs = self._check_system_requirements(
                plugin_id, plugin_version
            )
            ...
            # 결과 저장
            self._save_compatibility_result(
                plugin_id, plugin_version, compatible, issues
            )
    ...
    def _check_version_compatibility(
        self, plugin_id: str, target_version: str
    ...
    ) -> Tuple[List[CompatibilityIssue], List[str]]:
        """의존성 호환성 검사"""
        issues = []
        checked_deps = []
        ...
        try:
            ...
            for req in requirements or []:
                ...
                if req.startswith("python"):
                    ...
                    try:
                        ...
                        issues.append(
                            CompatibilityIssue(
                                ...
                                details={"requirement": req, "error": str(e)},
                    ...
                    try:
                        required_memory = int(
                            req.split(">=")[1].replace("MB", "").replace("GB", "")
                        ) * (1024**3 if "GB" in req else 1024**2)
    ...
    def _generate_recommendations(
        self, issues: List[CompatibilityIssue], plugin_info: Dict
    ) -> List[str]:
        """권장사항 생성"""
        recommendations = []
    ...
    def _save_compatibility_result(
        self,
        plugin_id: str,
        plugin_version: str,
        compatible: bool,
    ...
    def batch_compatibility_check(
        ...
        for plugin_id in plugin_ids:
            try:
                ...
                logger.info(
                    f"호환성 검사 완료: {plugin_id} - {'호환' if report.compatible else '비호환'}"
                )
            except Exception as e:
    ...
    def export_compatibility_report(
        self, output_path: str, reports: Dict[str, CompatibilityReport]
    ):
        """호환성 보고서 내보내기"""
        try:
            report_data = {
                ...
                "summary": {
                    ...
                    "compatible_plugins": len(
                        [r for r in reports.values() if r.compatible]
                    ),
                    "incompatible_plugins": len(
                        [r for r in reports.values() if not r.compatible]
                    ),
