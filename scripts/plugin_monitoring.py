...
class PluginMonitor:
    ...
    def __init__(self, db_path: str = "plugin_monitoring.db"):
        ...
        self.metrics_history: Dict[str, Deque[PluginMetrics]] = defaultdict(lambda: deque(maxlen=1000))
    ...
    def record_metrics(self, metrics: PluginMetrics):
        ...
        try:
            ...
            self.metrics_history[metrics.plugin_id].append(metrics)  # pyright: ignore[reportArgumentType]
    ...
    def _check_thresholds(self, metrics: PluginMetrics):
        ...
        if metrics.memory_usage > self.thresholds['memory_usage']:
            alerts.append(PerformanceAlert(
                ...
                severity='high' if metrics.memory_usage > self.thresholds['memory_usage'] * 1.5 else 'medium',
        ...
        if metrics.cpu_usage > self.thresholds['cpu_usage']:
            alerts.append(PerformanceAlert(
                plugin_id=metrics.plugin_id,
                alert_type='high_cpu',
                severity='high' if metrics.cpu_usage > self.thresholds['cpu_usage'] * 1.2 else 'medium',
    ...
    def _cleanup_old_data(self):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("DELETE FROM plugin_metrics WHERE timestamp < ?", (thirty_days_ago,))
                conn.execute("DELETE FROM performance_alerts WHERE timestamp < ? AND resolved = TRUE", (thirty_days_ago,))
                conn.commit()
    ...
    def get_plugin_metrics(self, plugin_id: str, hours: int = 24) -> List[PluginMetrics]:
        """플러그인 메트릭 조회"""
        try:
    ...
    def get_performance_summary(self, plugin_id: str, hours: int = 24) -> Dict[str, Any]:
        """성능 요약 정보 조회"""
    ...
    def resolve_alert(self, alert_id: int):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("UPDATE performance_alerts SET resolved = TRUE WHERE id = ?", (alert_id,))
                conn.commit()
    ...
    def export_metrics(self, output_path: str, plugin_id: Optional[str] = None, hours: int = 24):
        """메트릭 데이터 내보내기"""
        try:
            since = datetime.now() - timedelta(hours=hours)
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                cpu_usage = float(start_cpu + end_cpu) / 2.0 if isinstance(start_cpu, (int, float)) and isinstance(end_cpu, (int, float)) else 0.0
...
def main():
    ...
    parser.add_argument('--db-path', default='plugin_monitoring.db', help='데이터베이스 경로')
    ...
    parser.add_argument('--alerts', choices=['all', 'critical', 'high', 'medium', 'low'], 
                       help='알림 조회')
    parser.add_argument('--export', metavar='OUTPUT_PATH', help='메트릭 데이터 내보내기')
