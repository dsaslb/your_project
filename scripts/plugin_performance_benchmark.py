...
class PluginPerformanceBenchmark:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 인덱스 생성
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_benchmark_plugin_id ON benchmark_results(plugin_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_benchmark_test_name ON benchmark_results(test_name)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_metrics_plugin_id ON benchmark_metrics(plugin_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_history_plugin_id ON performance_history(plugin_id)"
                )
    ...
    def run_benchmark(
        ...
        try:
            ...
            concurrent_metrics = self._run_concurrent_benchmark(
                plugin_id, test_function, config
            )
            metrics_history.extend(concurrent_metrics)

            # 4. 지속성 벤치마크
            logger.info(f"지속성 벤치마크 실행 중... ({config.test_duration}초)")
            endurance_metrics = self._run_endurance_benchmark(
                plugin_id, test_function, config
            )
            ...
            result = self._analyze_benchmark_results(
                plugin_id, test_name, metrics_history
            )
    ...
    def _run_single_test(
        self, plugin_id: str, test_function: Callable, test_id: str
    ) -> BenchmarkMetrics:
        """단일 테스트 실행"""
    ...
    def _run_concurrent_benchmark(
        self, plugin_id: str, test_function: Callable, config: BenchmarkConfig
        ...
        def concurrent_test():
            return self._run_single_test(
                plugin_id,
                test_function,
                f"concurrent_{threading.current_thread().name}",
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [
                executor.submit(concurrent_test) for _ in range(config.test_runs)
            ]
    ...
    def _run_endurance_benchmark(
        ...
        while time.time() - start_time < config.test_duration:
            metrics = self._run_single_test(
                plugin_id, test_function, f"endurance_{len(metrics_list)}"
            )
            metrics_list.append(metrics)
    ...
    def _analyze_benchmark_results(
        self, plugin_id: str, test_name: str, metrics_history: List[BenchmarkMetrics]
    ) -> BenchmarkResult:
        """벤치마크 결과 분석"""
        ...
        execution_score = max(
            0.0,
            100.0 - (avg_execution_time / self.config.response_time_threshold) * 100.0,
        )
        cpu_score = max(
            0.0, 100.0 - (avg_cpu_usage / self.config.cpu_threshold) * 100.0
        )
        memory_score = max(
            0.0, 100.0 - (avg_memory_usage / self.config.memory_threshold) * 100.0
        )
        error_score = max(0.0, 100.0 - error_rate * 100.0)

        performance_score = (
            execution_score + cpu_score + memory_score + error_score
        ) / 4.0
    ...
    def _generate_recommendations(
        ...
        if avg_execution_time > self.config.response_time_threshold:
            recommendations.append(
                "응답 시간이 임계값을 초과합니다. 코드 최적화를 고려하세요."
            )

        if avg_cpu_usage > self.config.cpu_threshold:
            recommendations.append(
                "CPU 사용률이 높습니다. 비동기 처리나 스레드 풀을 고려하세요."
            )

        if avg_memory_usage > self.config.memory_threshold:
            recommendations.append(
                "메모리 사용률이 높습니다. 메모리 누수나 불필요한 객체 생성을 확인하세요."
            )
    ...
    def generate_performance_report(self, plugin_id: str, output_path: str):
        ...
        try:
            ...
            report_data = {
                ...
                "history_summary": {
                    ...
                    "avg_performance_score": statistics.mean(
                        [r.performance_score for r in history]
                    ),
                    "best_performance_score": max(
                        [r.performance_score for r in history]
                    ),
                    "worst_performance_score": min(
                        [r.performance_score for r in history]
                    ),
                },
            }
            ...
            self._generate_performance_charts(
                plugin_id, history, output_path.replace(".json", "_charts.png")
            )
    ...
    def _generate_performance_charts(
        self, plugin_id: str, history: List[BenchmarkResult], output_path: str
    ):
        """성능 차트 생성"""
        try:
        ...
        for plugin_id in sample_plugins:
            try:
                ...
                benchmark.generate_performance_report(
                    plugin_id, f"performance_report_{plugin_id}.json"
                )
