...
class PluginMonitoringReportGenerator:
    ...
    def collect_plugin_status(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/admin/plugin-monitoring/status", timeout=10
            )
    ...
    def collect_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/admin/plugin-monitoring/performance", timeout=10
            )
            if response.status_code == 200:
                return response.json()
            else:
    ...
    def collect_health_data(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/admin/plugin-monitoring/health", timeout=10
            )
            if response.status_code == 200:
                return response.json()
            else:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": self._generate_summary(
                plugin_status, performance_metrics, health_data
            ),
        }
    ...
    def _generate_summary(
        self, plugin_status: Dict, performance_metrics: Dict, health_data: Dict
    ) -> Dict[str, Any]:
        """요약 정보 생성"""
        summary = {
            "total_plugins": 0,
    ...
    def print_summary(self):
        ...
        if not self.report_data:
            logger.warning(
                "리포트 데이터가 없습니다. generate_report()를 먼저 실행하세요."
            )
            return
