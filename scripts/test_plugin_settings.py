...
class PluginSettingsTester:
        ...
        try:
            template = {
                ...
                "migrations": {
                    "1.0.0_to_2.0.0": {
                        ...
                        "changes": ["새로운 설정 필드 추가", "기존 설정 구조 변경"],
                    }
                },
            }
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 템플릿 생성", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
                self.log_test(
                    "설정 템플릿 생성", False, f"Status: {response.status_code}"
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 템플릿 조회", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
                self.log_test(
                    "설정 템플릿 조회", False, f"Status: {response.status_code}"
                )
                return False
    ...
    def test_create_initial_settings(self) -> bool:
        ...
        try:
            initial_settings = {
                ...
                "auto_reload": False,
                "permissions": ["read", "write"],
                "config": {"max_connections": 100, "timeout": 30, "cache_size": 1000},
                "version": "1.0.0",
            }
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "초기 설정 생성", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
                self.log_test(
                    "초기 설정 생성", False, f"Status: {response.status_code}"
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 조회", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
    ...
    def test_update_settings(self) -> bool:
        ...
        try:
            updated_settings = {
                ...
                "config": {"max_connections": 200, "timeout": 60, "cache_size": 2000},
                "version": "1.1.0",
            }
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 업데이트", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success" and data.get("data", {}).get(
                    ...
                    self.log_test(
                        "설정 검증 (유효한 설정)", False, "설정이 유효하지 않음"
                    )
                    return False
            else:
                self.log_test(
                    "설정 검증 (유효한 설정)", False, f"Status: {response.status_code}"
                )
                return False
            ...
            invalid_settings = {
                ...
                "config": {"max_connections": -1},  # 최소값 위반
            }
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get("status") == "success" and not data.get("data", {}).get(
                    "is_valid"
                ):
    ...
    def test_create_backup(self) -> bool:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/plugin-settings/{TEST_PLUGIN}/backup"
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "백업 생성", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    if len(backups) > 0:
                        self.log_test(
                            "백업 목록 조회", True, f"{len(backups)}개 백업 발견"
                        )
                        return True
                    else:
                    ...
                    self.log_test(
                        "백업 목록 조회", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
                self.log_test(
                    "백업 목록 조회", False, f"Status: {response.status_code}"
                )
                return False
    ...
    def test_export_settings(self) -> bool:
        ...
        try:
            # JSON 형식으로 내보내기
            response = self.session.get(
                f"{self.base_url}/api/plugin-settings/{TEST_PLUGIN}/export?format=json"
            )
            ...
            if response.status_code == 200:
                ...
                self.log_test(
                    "설정 내보내기 (JSON)", False, f"Status: {response.status_code}"
                )
                return False

            # YAML 형식으로 내보내기
            response = self.session.get(
                f"{self.base_url}/api/plugin-settings/{TEST_PLUGIN}/export?format=yaml"
            )
            ...
            if response.status_code == 200:
                ...
                self.log_test(
                    "설정 내보내기 (YAML)", False, f"Status: {response.status_code}"
                )
                return False
    ...
    def test_import_settings(self) -> bool:
        ...
        try:
            ...
            if export_response.status_code != 200:
                self.log_test(
                    "설정 가져오기", False, "내보내기 데이터를 가져올 수 없음"
                )
                return False
            ...
            # 설정 가져오기
            import_data = {"import_data": export_data["data"], "format": "json"}
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 가져오기", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
    ...
    def test_migrate_settings(self) -> bool:
        """설정 마이그레이션 테스트"""
        try:
            migration_data = {"from_version": "1.0.0", "to_version": "2.0.0"}
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 마이그레이션", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
                self.log_test(
                    "설정 마이그레이션", False, f"Status: {response.status_code}"
                )
                return False
    ...
    def test_reset_settings(self) -> bool:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/plugin-settings/{TEST_PLUGIN}/reset"
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.log_test(
                        "설정 초기화", False, data.get("error", "Unknown error")
                    )
                    return False
            else:
    ...
    def run_all_tests(self):
        ...
        for result in self.test_results:
            ...
            print(
                f"  {status} {result['test']} - {result['message'] if result['message'] else '성공'}"
            )
