...
from models import Attendance, User, AttendanceReport, db
...
def generate_attendance_report_pdf(period='week', user_id=None, admin_view=True):
    ...
    try:
        ...
        # HTML 템플릿 렌더링
        html = render_template('reports/attendance_report_pdf.html',
                              records=records,
                              stats=stats,
                              from_date=from_date,
                              to_date=to_date,
...
def calculate_attendance_stats(records, from_date, to_date):
    ...
    if stats['total_days'] > 0:
        stats['avg_hours_per_day'] = round(stats['total_hours'] / stats['total_days'], 2)
    else:
...
def send_email(to_email, subject, body, attach_path=None, html_body=None):
    ...
    try:
        ...
        if not all([smtp_username, smtp_password]):
            ...
            print(f"메일 발송 테스트용(to={to_email}, subject={subject}, attach={attach_path})")
            return True
...
def send_individual_attendance_report(user_id, period='week'):
    ...
    try:
        ...
        # PDF 생성
        pdf_path = generate_attendance_report_pdf(period=period, user_id=user_id, admin_view=False)
