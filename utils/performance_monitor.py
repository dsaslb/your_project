...
class PerformanceMonitor:
    ...
    def record_slow_query(self, query: str, execution_time: float, timestamp: Optional[datetime] = None):
        """느린 쿼리 기록"""
        ...
        if execution_time > 5.0:  # 5초 이상 걸리는 쿼리 경고
            logger.warning(f"매우 느린 쿼리 감지: {execution_time:.2f}초 - {query[:100]}...")
    
    def record_api_response_time(self, endpoint: str, method: str, response_time: float, status_code: Optional[int] = None):
        """API 응답 시간 기록"""
    ...
    def get_performance_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        if recent_cpu:
            summary['avg_cpu_usage'] = sum(m['value'] for m in recent_cpu) / len(recent_cpu)
            summary['max_cpu_usage'] = max(m['value'] for m in recent_cpu)
        
        # 메모리 사용률 평균
        recent_memory = [m for m in self.metrics['memory_usage'] if m['timestamp'] > cutoff_time]
        if recent_memory:
            summary['avg_memory_usage'] = sum(m['value'] for m in recent_memory) / len(recent_memory)
            summary['max_memory_usage'] = max(m['value'] for m in recent_memory)
    ...
    def get_alerts(self) -> List[Dict[str, Any]]:
        """성능 알림 생성"""
        alerts = []
        summary = self.get_performance_summary(hours=1)  # 최근 1시간 기준
