...
def get_dashboard_stats():
    ...
    # --- 3. 최근 10개 출퇴근 이력 ---
    recent = df.sort_values("clock_in", ascending=False).head(10) if not df.empty else pd.DataFrame()
    ...
    # --- 6. 자동 경고 (지각/결근 많은 직원) ---
    warn_users = summary[(summary.late_count >= 2) | (summary.absent_count >= 1)]["user"].tolist() if not summary.empty else []
    ...
    return {
        ...
        'top_absent': top_absent.to_dict(orient='records') if not top_absent.empty else [],
