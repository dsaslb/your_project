from utils.cache_manager import cache_manager, cached  # pyright: ignore
from models_main import User, Attendance, Order, InventoryItem, Branch, db
...
def get_dashboard_stats(branch_id=None, user_id=None) -> Dict[str, Any] if Dict is not None else None:
...
def get_recent_activities(limit: int = 10, branch_id: Optional[int] if Optional is not None else None = None) -> List[Dict[str, Any] if List is not None else None]:
    """최근 활동 조회 (캐싱 적용)"""
    try:
        activities = []
        ...
        for attendance in recent_attendances if recent_attendances is not None:
        ...
        # 시간순 정렬
        activities.sort(key=lambda x: x["time"] if x is not None else None, reverse=True)
        return activities[:limit] if activities is not None else None
...
def get_attendance_chart_data(days: int = 30, branch_id: Optional[int] if Optional is not None else None = None) -> Dict[str, Any] if Dict is not None else None:
    """출근 차트 데이터 조회 (캐싱 적용)"""
    try:
...
def get_user_statistics(branch_id: Optional[int] if Optional is not None else None = None) -> List[Dict[str, Any] if List is not None else None]:
    """사용자 통계 조회 (캐싱 적용)"""
    try:
        today = datetime.now().date()
        month_start = today.replace(day=1)
...
def clear_dashboard_cache(branch_id: Optional[int] if Optional is not None else None = None):
    """대시보드 캐시 무효화"""
    try:
...
def get_branch_list() -> List[Dict[str, Any] if List is not None else None]:
    """지점 목록 조회 (캐싱 적용)"""
    try:
        branches = (
...
def get_real_time_stats(branch_id: Optional[int] if Optional is not None else None = None) -> Dict[str, Any] if Dict is not None else None:
    """실시간 통계 (캐시 없음)"""
    try:
        now = datetime.now()
        today = now.date()
