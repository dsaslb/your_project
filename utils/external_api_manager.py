...
class ExternalAPIManager:
    ...
    def add_endpoint(self, name: str, url: str, method: str = 'GET',
                     headers: Optional[Dict[str, Any]] = None, timeout: int = 30, retry_count: int = 3):
        """API 엔드포인트 추가"""
    ...
    def _prepare_headers(self, endpoint: APIEndpoint, service: Optional[str] = None) -> Dict[str, Any]:
        """헤더 준비"""
        headers = (endpoint.headers or {}).copy()
    ...
    async def async_request(self, endpoint_name: str, params: Optional[Dict[str, Any]] = None,
                            data: Optional[Dict[str, Any]] = None, service: str = None) -> Dict[str, Any]:
        """비동기 API 요청"""
        endpoint = self.endpoints.get(endpoint_name)
        if not endpoint:
    ...
    def sync_request(self, endpoint_name: str, params: Optional[Dict[str, Any]] = None,
                     data: Optional[Dict[str, Any]] = None, service: str = None) -> Dict[str, Any]:
        """동기 API 요청"""
        endpoint = self.endpoints.get(endpoint_name)
        if not endpoint:
    ...
    def get_weather(self,  city: str, country_code='KR') -> Dict[str, Any]:
        """날씨 정보 조회"""
    ...
    def process_payment(self, amount: float, currency: str = 'KRW',
        ...
        return self.sync_request('payment', data=data)

    def create_delivery_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        """배달 주문 생성"""
        if 'delivery' not in self.api_keys:
            return {"error": "배달 API 키가 설정되지 않았습니다."}

        return self.sync_request('delivery', data=order_data)
    ...
    def get_request_history(self, limit=100) -> List[Dict[str, Any]]:
        """요청 히스토리 조회"""
