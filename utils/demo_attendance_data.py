...
class DemoAttendanceData:
    ...
    @staticmethod
    def get_demo_attendance_data(days: int = 30) -> List[Dict[str, Any]]:
        """데모용 출퇴근 데이터 (최근 N일)"""
        attendance_data = []
        today = datetime.now()
        ...
        for i in range(days):
            ...
            # 각 직원별 출퇴근 데이터 생성
            for user in DemoAttendanceData.get_demo_users():
                # 주말 제외 (토,일)
                if date.weekday() >= 5:
                    continue
    ...
    def get_demo_statistics() -> Dict[str, Any]:
        """데모용 통계 데이터"""
        attendance_data = DemoAttendanceData.get_demo_attendance_data()
        
        total_records = len(attendance_data)
        late_count = sum(1 for record in attendance_data if record["is_late"])
        early_leave_count = sum(1 for record in attendance_data if record["is_early_leave"])
        overtime_count = sum(1 for record in attendance_data if record["is_overtime"])
        
        total_work_hours = sum(record["work_hours"] for record in attendance_data)
        avg_work_hours = total_work_hours / total_records if total_records > 0 else 0
        
        # 직원별 통계
        user_stats = {}
        for user in DemoAttendanceData.get_demo_users():
            user_records = [r for r in attendance_data if r["user_id"] == user["id"]]
            if user_records:
                user_stats[user["name"]] = {
                    "total_days": len(user_records),
                    "late_count": sum(1 for r in user_records if r["is_late"]),
                    "early_leave_count": sum(1 for r in user_records if r["is_early_leave"]),
                    "overtime_count": sum(1 for r in user_records if r["is_overtime"]),
                    "avg_work_hours": sum(r["work_hours"] for r in user_records) / len(user_records)
                }
    ...
    @staticmethod
    def get_demo_realtime_data() -> Dict[str, Any]:
        """실시간 데모 데이터 (오늘)"""
        today = datetime.now()
        ...
        return {
            "date": today_str,
            "total_employees": len(DemoAttendanceData.get_demo_users()),
            "checked_in": len([a for a in today_attendance if a["status"] == "checked_in"]),
            "completed": len([a for a in today_attendance if a["status"] == "completed"]),
            "late_count": len([a for a in today_attendance if a["is_late"]]),
            "attendance_list": today_attendance
        }
    
    @staticmethod
    def get_demo_notifications() -> List[Dict[str, Any]]:
        """데모용 알림 데이터"""
        return [
            {
                "id": 1,
    ...
    @staticmethod
    def get_full_demo_data() -> Dict[str, Any]:
        """전체 데모 데이터"""
        return {
