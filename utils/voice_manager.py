...
class VoiceManager:
    ...
    def _validate_audio(self, audio_data: bytes) -> bool:
        ...
        try:
            ...
            if channels != self.audio_settings["channels"]:
                return False
    ...
    def _recognize_speech(self, audio_data: bytes) -> Tuple[str, float]:
        """음성 인식 (더미 구현)"""
    ...
    def _get_random_response(self, response_type: str) -> str:
        """랜덤 응답 반환"""
        responses = self.voice_responses.get(response_type, self.voice_responses["error"])
