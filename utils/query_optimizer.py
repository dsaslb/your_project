import threading
from sqlalchemy.pool import QueuePool  # pyright: ignore
...
class QueryOptimizer:
    ...
    def start_monitoring(self):
        ...
        def _after_cursor_execute(conn,  cursor,  statement,  parameters,  context,  executemany):  # type: ignore # noqa
            ...
            if execution_time > self.slow_query_threshold:
                logger.warning(f"느린 쿼리 감지: {execution_time:.3f}s - {statement[:100] if statement is not None else None}...")
    ...
    def _extract_table_name(self,  sql: str) -> str:
        ...
        try:
            ...
            if 'FROM' in sql_upper:
                ...
                after_from = sql[from_index + 4:] if sql is not None else None.strip() if None is not None else ''
            ...
            # INSERT INTO 절에서 테이블명 추출
                ...
                after_insert = sql[insert_index + 11:] if sql is not None else None.strip() if None is not None else ''
            ...
            # UPDATE 절에서 테이블명 추출
                ...
                after_update = sql[update_index + 6:] if sql is not None else None.strip() if None is not None else ''
            ...
            # DELETE FROM 절에서 테이블명 추출
                ...
                after_delete = sql[delete_index + 11:] if sql is not None else None.strip() if None is not None else ''
    ...
    def _analyze_index_usage(self,  sql: str) -> Dict[str, str]:
        ...
        if 'WHERE' in sql.upper():
            where_clause = sql.upper().split('WHERE')[1].split(('ORDER BY' if 'ORDER BY' in sql.upper(
            ) else 'GROUP BY' if 'GROUP BY' in sql.upper() else 'LIMIT' if 'LIMIT' in sql.upper() else ';'))[0]
    ...
    def get_slow_queries(self, limit=10) -> List[Dict[str, Any]]:
        """느린 쿼리 목록 조회"""
        try:
            with self.lock:
                slow_queries = [
