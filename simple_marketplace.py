                ...
                return [
                    {
                        ...
                        'menu_url': '/attendance',
                        'sub_menus': [
                            {'id': 11, 'menu_name': '출근 기록', 'menu_icon': 'fas fa-list', 'menu_url': '/attendance/records'},
                            {'id': 12, 'menu_name': '근무 통계', 'menu_icon': 'fas fa-chart-bar', 'menu_url': '/attendance/stats'}
                        ]
                    },
                    {
                        'id': 2,
                        ...
                        'menu_url': '/inventory',
                        'sub_menus': [
                            {'id': 21, 'menu_name': '재고 현황', 'menu_icon': 'fas fa-eye', 'menu_url': '/inventory/status'},
                            {'id': 22, 'menu_name': '입출고 관리', 'menu_icon': 'fas fa-exchange-alt', 'menu_url': '/inventory/transactions'}
                        ]
                    },
                    {
                        'id': 3,
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('simple_marketplace.log'),
...
def install_module(module_id):
    ...
    try:
        ...
        if not module:
            ...
            return jsonify({'success': False, 'message': '모듈을 찾을 수 없습니다.'}), 404
        
        # 이미 설치된 모듈인지 확인
        if module.get('installed', False):
            logger.info(f"이미 설치된 모듈: {module_id}")
            return jsonify({'success': False, 'message': '이미 설치된 모듈입니다.'}), 400
    ...
    for module in modules:
        if module['id'] == module_id:
            try:
                ...
                return jsonify({'success': True, 'message': f'{module["name"]} 모듈이 제거되었습니다.'})
            except Exception as e:
...
def menu_management():
    ...
    try:
        stats = menu_system.get_menu_statistics()
        user_menus = menu_system.get_user_menus(current_user.id, current_user.role)
        return render_template('simple_menu_management.html', stats=stats, menus=user_menus)
    except Exception as e:
...
def project_editor(project_id):
    ...
    try:
        ...
        if project:
            return render_template('simple_project_editor.html', 
                                 project=project, 
...
@app.route('/api/dev/projects/<project_id>/components/<component_id>', methods=['DELETE'])
@login_required
