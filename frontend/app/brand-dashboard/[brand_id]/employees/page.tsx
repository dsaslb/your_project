// /brand-dashboard/[brand_id]/employees/page.tsx
'use client';
...
export default function BrandEmployeesPage() {
  // URL에서 brand_id 추출
  ...
  useEffect(() => {
    async function fetchEmployees() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/admin/employees?brand_id=${brandId}`);
        if (res.ok) {
  ...
  return (
    <ProtectedRoute requiredRoles={['admin', 'brand_admin', 'store_manager']} redirectTo="/admin-dashboard">
      <main className="p-8">
        <h1 className="text-2xl font-bold mb-4" aria-label="브랜드별 직원 목록">직원 목록</h1>
        {canManage ? (
          <button
            tabIndex={0}
          ...
          >
            + 직원 등록
            <span className="sr-only">새 직원을 등록하려면 클릭하세요.</span>
          </button>
        ) : (
        ...
        <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="fixed z-50 inset-0 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded p-6 w-full max-w-md mx-auto">
            <Dialog.Title className="text-lg font-bold mb-2">직원 등록</Dialog.Title>
            <form onSubmit={handleAddEmployee} className="space-y-4">
              <div>
                <label htmlFor="employeeName" className="block text-sm font-medium">이름</label>
                <input
                  id="employeeName"
                  type="text"
              ...
              <div>
                <label htmlFor="employeeRole" className="block text-sm font-medium">역할</label>
                <input
                  id="employeeRole"
                  type="text"
        ...
        {loading ? (
          ...
          <ul className="space-y-2">
            {employees.map(emp => (
              <li key={emp.id} className="border rounded p-4 flex flex-col md:flex-row md:items-center md:justify-between">
                <span className="font-semibold">{emp.name}</span>
                <span className="text-sm text-gray-500">{emp.role}</span>
                <span className={`ml-2 text-xs ${emp.status === 'approved' ? 'text-green-600' : 'text-gray-400'}`}>{emp.status === 'approved' ? '승인됨' : '대기중'}</span>
              </li>
            ))}
          </ul>
        )}
