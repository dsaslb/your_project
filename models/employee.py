...
class UserInfo(db.Model):
    ...
    # 관계
    user = db.relationship('User', backref='user_info')
...
class EmployeeSchedule(db.Model):
    """직원 스케줄 모델 (프론트엔드에서 관리)"""
    __tablename__ = 'employee_schedules'
    
    id = db.Column(db.Integer, primary_key=True)
    employee_id = db.Column(db.Integer, db.ForeignKey('user_info.id'), nullable=False)
    ...
    # 관계
    employee = db.relationship('UserInfo', backref='schedules')
...
class EmployeeAttendance(db.Model):
    ...
    # 관계
    employee = db.relationship('UserInfo', backref='attendance_records')
