...
def add_notification_columns():
    ...
    with app.app_context():
        try:
            # 새로운 컬럼들 추가
            db.engine.execute('ALTER TABLE notifications ADD COLUMN recipient_role VARCHAR(20)')
            print("recipient_role 컬럼 추가 완료")
        except Exception as e:
            print(f"recipient_role 컬럼 추가 실패 (이미 존재할 수 있음): {e}")
        
        try:
            db.engine.execute('ALTER TABLE notifications ADD COLUMN recipient_team VARCHAR(20)')
            print("recipient_team 컬럼 추가 완료")
        except Exception as e:
            print(f"recipient_team 컬럼 추가 실패 (이미 존재할 수 있음): {e}")
        
        try:
            db.engine.execute('ALTER TABLE notifications ADD COLUMN priority VARCHAR(10) DEFAULT "일반"')
            print("priority 컬럼 추가 완료")
        except Exception as e:
            print(f"priority 컬럼 추가 실패 (이미 존재할 수 있음): {e}")
        
        try:
            db.engine.execute('ALTER TABLE notifications ADD COLUMN ai_priority VARCHAR(10)')
