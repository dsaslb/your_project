...
# 필요한 유틸리티 함수들 import
from utils.logger import setup_logger, log_error, log_action, log_security_event
...
from utils.slack_alert import send_slack_alert, test_slack_connection, send_event_alert, send_security_alert, send_slack_alert_if_prod
from models import User, Notice, NoticeComment, Attendance, ShiftRequest, Feedback, Suggestion, SystemLog
...
def setup_logging(app):
    ...
    # 파일 핸들러 (운영 환경에서만 활성화)
    if app.config['LOG_LEVEL'] in ('WARNING', 'ERROR'):
        # 로그 디렉토리 생성
            ...
            class SlackHandler(logging.Handler):
                def emit(self, record):
                    ...
                    if record.levelno >= logging.ERROR:
                        ...
                        send_slack_alert(msg, record.levelname)
        
        slack_handler = SlackHandler()
        slack_handler.setLevel(logging.ERROR)  # ERROR 이상만
        slack_handler.setFormatter(formatter)
        app.logger.addHandler(slack_handler)
    
    # 기존 핸들러 제거 후 새 핸들러 적용
    if app.logger.handlers:
        app.logger.handlers = []
    app.logger.addHandler(stream_handler)
    app.logger.setLevel(log_level)
    
    # SQLAlchemy engine log도 조정
    sqlalchemy_level = logging.WARNING if app.config['LOG_LEVEL'] != 'DEBUG' else logging.INFO
    logging.getLogger('sqlalchemy.engine').setLevel(sqlalchemy_level)
    
    app.logger.info(f"로깅 레벨: {app.config['LOG_LEVEL']} (환경: {config_name})")
...
def server_error(e):
    ...
    if app.config.get('SLACK_WEBHOOK_URL'):
        send_slack_alert(f"🚨 서버 500 에러 발생!\n에러: {str(e)}", "ERROR")
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            # Slack 이벤트 알림 (운영 환경에서만)
            send_slack_alert_if_prod(f"📝 [이벤트] 로그인\n사용자: {user.username}\nIP: {request.remote_addr}")
...
def test_slack():
    """Slack 알림 시스템 테스트"""
    from utils.slack_alert import test_slack_connection, send_slack_alert
    ...
    if test_slack_connection():
        ...
        for level in levels:
            message = f"🧪 {level} 레벨 테스트 메시지"
            success = send_slack_alert(message, level)
            print(f"{level}: {'✅ 성공' if success else '❌ 실패'}")
    else:
        print("❌ Slack 연결 실패!")
        print("SLACK_WEBHOOK_URL 환경변수를 확인해주세요.")
