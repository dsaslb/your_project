...
class LedgerService:
    ...
    def get_monthly_summary(user_id: int, year: int, month: int) -> Dict:
        ...
        try:
            ...
            income_items = LedgerItem.query.filter_by(
                user_id=user_id, item_type="income", is_active=True
            ).all()

            # 지출 항목들
            expense_items = LedgerItem.query.filter_by(
                user_id=user_id, item_type="expense", is_active=True
            ).all()
            ...
            for item in income_items:
                monthly_amount = LedgerService._calculate_monthly_amount(
                    item, year, month
                )
    ...
    def _get_next_payment_date(
        item: LedgerItem, from_date: datetime.date
    ) -> datetime.date:
        """다음 결제일 계산"""
        try:
    ...
    try:
        ...
        for field in required_fields:
            if not data.get(field):
                return (
                    jsonify({"success": False, "error": f"{field} 필드는 필수입니다."}),
                    400,
                )
...
def get_upcoming_payments():
    ...
    try:
        ...
        return jsonify({"success": True, "upcoming": upcoming})
    except Exception as e:
        logger.error(f"다가오는 결제 조회 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
