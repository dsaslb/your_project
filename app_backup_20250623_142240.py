...
def schedule():
    schedules = (
        Schedule.query.filter(Schedule.user_id.in_([current_user.id]))
...
def approve_schedule(schedule_id):
    ...
    if action in ["승인", "거절"]:
        ...
        if action == "승인":
            content = (
                f"{schedule.date.strftime('%m월 %d일')} 교대 신청이 승인되었습니다."
            )
            link = "/swap_manage"
            send_notification_enhanced(schedule.user_id, content, "교대", link)
        else:
            content = (
                f"{schedule.date.strftime('%m월 %d일')} 교대 신청이 거절되었습니다."
            )
            if reason:
                content += f" 사유: {reason}"
            link = "/swap_manage"
...
def user_manage():
    ...
    if request.method == "POST":
        ...
        if User.query.filter_by(username=username).first():
            ...
            db.session.commit()
            flash(
                f"{username} 직원이 등록되었습니다. 임시 비밀번호: {temp_password}",
                "success",
            )
...
def schedule_manage():
    ...
    return render_template(
        "schedule_manage.html", schedules=schedules, employees=employees
    )
...
def clean_manage():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            cleaning = CleaningPlan(
                date=date_obj, plan=plan, team=team, manager_id=manager_id
            )
    ...
    return render_template(
        "clean_manage.html", cleanings=cleanings, employees=employees
    )
...
def swap_manage():
    swaps = (
        Schedule.query.filter_by(category="교대").order_by(Schedule.date.desc()).all()
    )
...
def clean_report():
    ...
    counts = (
        db.session.query(
            CleaningPlan.team, CleaningPlan.date, func.count(CleaningPlan.id)
        )
        .group_by(CleaningPlan.team, CleaningPlan.date)
        .all()
    )
...
def order_approved_notifications():
    ...
    notis = (
        Notification.query.filter_by(
            user_id=current_user.id, category="발주", is_read=False
        )
        .filter(Notification.content.contains("승인"))
...
def admin_system_notifications():
    ...
    notis = (
        Notification.query.filter_by(is_admin_only=True)
        .order_by(Notification.created_at.desc())
        .all()
    )
