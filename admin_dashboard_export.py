from flask import request, flash, redirect, url_for, send_file, render_template, jsonify
from flask_login import login_required, current_user
from datetime import datetime
import pandas as pd
import io
import pdfkit
from app import app, db, Attendance, User, AttendanceReport

@app.route('/admin_dashboard/export_excel')
@login_required
...
@app.route('/admin_dashboard/export_pdf')
@login_required
def export_admin_dashboard_pdf():
    ...
    html = render_template('admin_dashboard_pdf.html', records=records, stats=stats, 
                         team=team, user_id=user_id, date_from=date_from, date_to=date_to)
    pdf = pdfkit.from_string(html, False)
    return send_file(io.BytesIO(pdf), download_name="dashboard_export.pdf", as_attachment=True) 