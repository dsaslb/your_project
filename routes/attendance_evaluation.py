...
from models import (AttendanceEvaluation, AttendanceReport, Excuse,
                    ExcuseResponse, Notification, User, db)
...
def submit_excuse(user_id):
    ...
    if request.method == "POST":
        try:
            ...
            return redirect(
                url_for("evaluation.staff_attendance_report", user_id=user_id)
            )
...
def review_excuse(excuse_id):
    ...
    try:
        ...
        if action not in ["accept", "reject"]:
            ...
            return redirect(
                url_for("evaluation.admin_excuse_detail", excuse_id=excuse_id)
            )
