...
from models import User, Brand, Branch, SystemLog, Suggestion
...
def get_feedbacks():
    ...
    try:
        ...
        for suggestion in pagination.items:
            ...
            # 브랜드 정보
            brand = Brand.query.get(user.brand_id) if user and user.brand_id else None
...
def get_feedback_details(suggestion_id):
    ...
    try:
        ...
        # 매장 정보
        branch = Branch.query.get(user.branch_id) if user and user.branch_id else None
        ...
        related_list = []
        for related in related_feedbacks:
            related_list.append({
