...
from utils.query_optimizer import get_query_optimizer, get_connection_pool_optimizer  # pyright: ignore
from utils.advanced_caching import get_cache, AdvancedCache  # pyright: ignore
...
def test_cache():
    ...
    try:
        ...
        key = data.get('key', 'test_key') if data else 'test_key'
        value = data.get('value', 'test_value') if data else 'test_value'
        ttl = data.get('ttl', 60) if data else 60
        levels = data.get('levels', ['l1_memory', 'l2_redis', 'l3_file']) if data else ['l1_memory', 'l2_redis', 'l3_file']
        ...
        # 값 조회
        retrieved_value = cache.get(key, levels=levels) if cache else None
...
def get_performance_overview():
    ...
    try:
        ...
        # 연결 풀 상태 기반 점수
        if pool_stats:
            total_connections = pool_stats.get('total_connections', 0) if pool_stats else 0
            checked_out = pool_stats.get('checked_out', 0) if pool_stats else 0
...
def run_optimization():
    ...
    try:
        data = request.get_json(silent=True) or {}
        optimization_types = data.get('types', ['cache', 'queries', 'pool']) if data else ['cache', 'queries', 'pool']
