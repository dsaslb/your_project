...
def hide_notice(notice_id):
    ...
    flash(
        f'공지사항이 {"숨김" if notice.is_hidden else "공개"} 처리되었습니다.',
        "success",
    )
...
def add_comment(notice_id):
    ...
    if content:
        ...
        db.session.commit()
        log_action(
            current_user.id, "COMMENT_CREATE", f"Comment created on notice {notice_id}"
        )
...
def hide_comment(comment_id):
    ...
    flash(
        f'댓글이 {"숨김" if comment.is_hidden else "공개"} 처리되었습니다.', "success"
    )
    return redirect(url_for("notice.view_notice", notice_id=comment.notice_id))
