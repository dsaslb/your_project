...
def create_project():
    ...
    try:
        ...
        # 프로젝트 디렉토리 생성
        project_dir = f"dev_projects/{data['name']}"
...
\"\"\"
{data['display_name']} - 기본 모듈
\"\"\"

def main():
    print("Hello from {data['display_name']}!")
...
@app.route('/api/{data["name"]}/health')
def health_check():
    return jsonify({{"status": "healthy", "module": "{data['name']}"}})

@app.route('/api/{data["name"]}/info')
def module_info():
    return jsonify({{
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">{data['display_name']}</h1>
        <div class="bg-white rounded-lg shadow p-6">
            <p class="text-gray-600">{data.get('description', '')}</p>
        </div>
    </div>
</body>
</html>
...
class {data['name'].title().replace('_', '')}Model(Base):
