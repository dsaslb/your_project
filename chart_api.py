...
def admin_dashboard_stats_data():
    ...
    data = (
        q.with_entities(
            Attendance.date,
            func.sum(func.case([(Attendance.reason.like("%지각%"), 1)], else_=0)).label(
                "late"
            ),
            func.sum(func.case([(Attendance.reason.like("%결근%"), 1)], else_=0)).label(
                "absent"
            ),
...
def admin_dashboard_reason_stats():
    ...
    # 사유별 카운트 (상위 10개)
    data = (
        q.with_entities(Attendance.reason, func.count().label("count"))
