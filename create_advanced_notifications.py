...
def create_advanced_notifications():
    ...
    with app.app_context():
        ...
        print(
            f"📊 총 {len(users)}명의 직원, {len(admins)}명의 관리자에게 알림을 생성합니다."
        )
        ...
        send_admin_only_notification(
            "데이터베이스 백업이 완료되었습니다.", "공지", "/admin/backup/db"
        )
        ...
        print(
            f"   - 발주 승인 필터: http://localhost:5000/notifications/filtered?category=발주&keyword=승인"
        )
        print(
            f"   - 안읽음 필터: http://localhost:5000/notifications/filtered?is_read=false"
        )
        print(
            f"   - 관리자 전용: http://localhost:5000/notifications/filtered?is_admin_only=true"
        )
