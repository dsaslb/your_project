...
from flask_login import (
    AnonymousUserMixin as BaseAnonymousUserMixin,
    UserMixin,
)  # noqa: E402
...
class Brand(db.Model):
    ...
    store_type = db.Column(
        db.String(20), default="individual", index=True
    )  # individual(개인매장), chain(체인점)
    ...
    contract_type = db.Column(
        db.String(50)
    )  # 계약 유형 (franchise, direct, partnership 등)
    ...
    brand_manager = db.relationship(
        "User", backref="managed_brand", foreign_keys="User.brand_id"
    )
...
class Branch(db.Model):
    ...
    )  # franchise, corporate, independent
...
class User(db.Model, UserMixin):
    ...
    brand_id = db.Column(
        db.Integer, db.ForeignKey("brands.id"), index=True
    )  # 브랜드 매니저용
    industry_id = db.Column(db.Integer, db.ForeignKey("industries.id"), nullable=True)
...
class Attendance(db.Model):
    ...
    status = db.Column(
        db.String(20), default="present", index=True
...
class NotificationHistory(db.Model):
    ...
    )  # email, sms, push, slack 등
    status = db.Column(
        db.String(20), default="sent", index=True
    )  # sent, failed, pending
...
class NotificationChannel(db.Model):
    ...
    type = db.Column(
        db.String(50), nullable=False, index=True
    )  # email, sms, push, slack, webhook 등
...
class NotificationRule(db.Model):
    ...
    event_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # attendance_late, order_created 등
...
class SystemLog(db.Model):
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_system_logs")
...
class InventoryItem(db.Model):
    ...
    stock_movements = db.relationship(
        "StockMovement", backref="inventory_item", cascade="all, delete-orphan"
    )
...
class StockMovement(db.Model):
    ...
    reference_type = db.Column(
        db.String(20)
    )  # 참조 타입 (order, your_program_order, manual)
...
class AttendanceReport(db.Model):
    ...
    dispute_type = db.Column(
        db.String(20), default="report", index=True
    )  # report, dispute
    ...
    assignee = db.relationship(
        "User", foreign_keys=[assignee_id], backref="assigned_reports"
    )
...
class Team(db.Model):
    ...
    # 관계
    brand = db.relationship("Brand", backref="brand_teams")
    manager = db.relationship(
        "User", foreign_keys=[manager_id], backref="managed_teams"
    )
    branch = db.relationship("Branch", backref="brand_teams")
...
class PayTransfer(db.Model):
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_pay_transfers")
    from_user = db.relationship(
        "User", foreign_keys=[from_user_id], backref="sent_transfers"
    )
    to_user = db.relationship(
        "User", foreign_keys=[to_user_id], backref="received_transfers"
    )
    ...
    def __repr__(self):
        return (
            f"<PayTransfer {self.from_user_id} -> {self.to_user_id}: {self.amount}원>"
        )
...
class Staff(db.Model):
    ...
    health_certificates = db.relationship(
        "HealthCertificate", back_populates="staff", cascade="all, delete-orphan"
    )
...
class Contract(db.Model):
    ...
    expired_notification_sent = db.Column(
        db.Boolean, default=False, index=True
    )  # 만료 알림 발송 여부
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(
    ...
    __table_args__ = (
        db.Index(
            "idx_contract_expiry_notification", "expiry_date", "notification_sent"
        ),
        db.Index(
    ...
    def is_expiring_soon(self):
        ...
        return (
            self.expiry_date <= today + timedelta(days=30) and self.expiry_date > today
        )
...
class StaffTemplate(db.Model):
    ...
    template_type = db.Column(
        db.String(20), default="position"
    )  # position, store, custom
...
class StaffRegistrationStep(db.Model):
    ...
    # 메타데이터
    template_id = db.Column(
        db.Integer, db.ForeignKey("staff_templates.id"), nullable=True
    )
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class ChatRoom(db.Model):
    ...
    name = db.Column(db.String(100), nullable=False)
    room_type = db.Column(
        db.String(20), nullable=False, default="group"
    ...
    participants = db.relationship(
        "ChatParticipant", backref="room", cascade="all, delete-orphan"
    )
    messages = db.relationship(
        "ChatMessage", backref="room", cascade="all, delete-orphan"
    )
...
class IoTDevice(db.Model):
    ...
    )  # 예: temperature, humidity, inventory, machine
...
class IoTData(db.Model):
    ...
    data_type = db.Column(
        db.String(32), nullable=False
    )  # 예: temperature, humidity, inventory, status
...
class AIDiagnosis(db.Model):
    ...
    status = db.Column(
        db.String(20), default="pending", index=True
    )  # pending, reviewed, implemented, resolved
    priority = db.Column(
        db.String(20), default="normal", index=True
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_ai_diagnoses")
    store = db.relationship("Branch", backref="brand_ai_diagnoses")
    reviewer = db.relationship(
        "User", foreign_keys=[reviewed_by], backref="reviewed_brand_diagnoses"
    )
    implementer = db.relationship(
        "User", foreign_keys=[implemented_by], backref="implemented_brand_diagnoses"
    )
...
class ImprovementRequest(db.Model):
    ...
    category = db.Column(
        db.String(50), nullable=False, index=True
    )  # system, process, equipment, training, other
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text, nullable=False)
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_improvement_requests")
    store = db.relationship("Branch", backref="brand_improvement_requests")
    requester = db.relationship(
        "User", foreign_keys=[requester_id], backref="improvement_requests"
    )
    reviewer = db.relationship(
        "User", foreign_keys=[reviewed_by], backref="reviewed_brand_improvements"
    )
...
class AIImprovementSuggestion(db.Model):
    ...
    diagnosis_id = db.Column(
        db.Integer, db.ForeignKey("ai_diagnoses.id"), nullable=True, index=True
    )
    brand_id = db.Column(
        db.Integer, db.ForeignKey("brands.id"), nullable=True, index=True
    )
    ...
    suggestion_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # automation, optimization, process, training
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text, nullable=False)
    ...
    status = db.Column(
        db.String(20), default="suggested", index=True
    )  # suggested, approved, rejected, implemented
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    )
    ...
    brand = db.relationship("Brand", backref="brand_ai_suggestions")
    store = db.relationship("Branch", backref="brand_ai_suggestions")
    reviewer = db.relationship(
        "User", foreign_keys=[reviewed_by], backref="reviewed_brand_ai_suggestions"
    )
...
class BrandDataCollection(db.Model):
    ...
    data_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # sales, performance, quality, customer_feedback
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_data_collections")
...
class StoreDataCollection(db.Model):
    ...
    # 관계 설정
    store = db.relationship("Branch", backref="brand_data_collections")
...
class SystemHealth(db.Model):
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_system_health")
    store = db.relationship("Branch", backref="brand_system_health")
...
class ApprovalWorkflow(db.Model):
    ...
    workflow_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # user_approval, improvement_approval, ai_suggestion_approval
    target_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # user, improvement_request, ai_suggestion
    target_id = db.Column(db.Integer, nullable=False, index=True)
    requester_id = db.Column(
        db.Integer, db.ForeignKey("users.id"), nullable=False, index=True
    )
    ...
    # 관계 설정
    requester = db.relationship(
        "User", foreign_keys=[requester_id], backref="workflow_requests"
    )
    approver = db.relationship(
        "User", foreign_keys=[approver_id], backref="workflow_approvals"
    )
...
class Industry(db.Model):
    ...
    # 관계
    brands_list = db.relationship(
        "Brand", backref="industry_obj", lazy="dynamic", overlaps="industry"
    )
...
class AIIntegration(db.Model):
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_ai_integrations")
...
class AIAnalysisReport(db.Model):
    ...
    )  # sales, inventory, staff, customer
    ...
    # 관계
    branch = db.relationship("Branch", backref="brand_ai_reports")
...
class Module(db.Model):
    ...
    category = db.Column(
        db.String(50), nullable=False, index=True
    )  # attendance, schedule, inventory, purchase, ai, etc.
    version = db.Column(db.String(20), default="1.0.0")
    ...
    feedbacks = db.relationship(
        "ModuleFeedback", backref="module", cascade="all, delete-orphan"
    )
    usages = db.relationship(
        "ModuleUsage", backref="module", cascade="all, delete-orphan"
    )
    versions = db.relationship(
        "ModuleVersion", backref="module", cascade="all, delete-orphan"
    )
...
class ModuleUsage(db.Model):
    ...
    action = db.Column(
        db.String(50), nullable=False
    )  # download, install, use, customize
    customizations = db.Column(db.JSON)  # 커스터마이징 정보
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)

    # 관계 설정
    brand = db.relationship("Brand", backref="brand_module_usages")
...
class ModuleCustomization(db.Model):
    ...
    # 관계 설정
    brand = db.relationship("Brand", backref="brand_module_customizations")
...
class PluginBugReport(db.Model):
    ...
    comments = db.relationship(
        "PluginBugComment", backref="bug_report", cascade="all, delete-orphan"
    )
...
class PluginBugComment(db.Model):
    ...
    bug_report_id = db.Column(
        db.Integer, db.ForeignKey("plugin_bug_reports.id"), nullable=False
    )
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    content = db.Column(db.Text, nullable=False)
...
class PluginFeatureRequest(db.Model):
    ...
    status = db.Column(
        db.String(20), default="open"
    )  # open, under_review, planned, in_progress, completed, rejected
    priority = db.Column(db.String(20), default="normal")  # low, normal, high, urgent
...
class BrandOnboarding(db.Model):
    ...
    # 관계
    brand = db.relationship("Brand", backref="brand_onboarding")
