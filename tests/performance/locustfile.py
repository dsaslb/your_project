...
class PluginSystemUser(HttpUser):
    ...
    def get_plugin_settings(self):
        ...
        with self.client.get(
            f"/api/plugin-settings/{plugin_name}", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def get_plugin_monitoring(self):
        """플러그인 모니터링 데이터 조회"""
        with self.client.get(
            "/api/plugin-monitoring/metrics", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def get_realtime_alerts(self):
        """실시간 알림 조회"""
        with self.client.get(
            "/api/enhanced-alerts/stream", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def get_marketplace_plugins(self):
        """마켓플레이스 플러그인 조회"""
        with self.client.get(
            "/api/marketplace/plugins", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
...
class PluginLoadTester(HttpUser):
    ...
    def load_plugin(self):
        ...
        with self.client.post(
            f"/api/plugins/{plugin_name}/reload", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def enable_plugin(self):
        ...
        with self.client.post(
            f"/api/plugins/{plugin_name}/enable", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def disable_plugin(self):
        ...
        with self.client.post(
            f"/api/plugins/{plugin_name}/disable", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def complete_plugin_workflow(self):
        ...
        try:
            ...
            response = self.client.post(
                f"/api/plugin-settings/{plugin_name}", json=settings_data
            )
            if response.status_code != 200:
