    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'disk_io': {
                ...
                'total_write_mb': sum(disk_writes) / (1024 * 1024) if disk_writes else 0,
                'read_rate_mbps': (max(disk_reads) - min(disk_reads)) / (1024 * 1024) if len(disk_reads) > 1 else 0,
                'write_rate_mbps': (max(disk_writes) - min(disk_writes)) / (1024 * 1024) if len(disk_writes) > 1 else 0
            },
            'network_io': {
                'total_sent_mb': sum(net_bytes_sent) / (1024 * 1024) if net_bytes_sent else 0,
                'total_recv_mb': sum(net_bytes_recv) / (1024 * 1024) if net_bytes_recv else 0,
                'send_rate_mbps': (max(net_bytes_sent) - min(net_bytes_sent)) / (1024 * 1024) if len(net_bytes_sent) > 1 else 0,
                'recv_rate_mbps': (max(net_bytes_recv) - min(net_bytes_recv)) / (1024 * 1024) if len(net_bytes_recv) > 1 else 0
            }
        }
...
class PluginResourceTest:
    ...
    def test_plugin_loading(self, plugin_name: str) -> Dict[str, Any]:
        ...
        try:
            # 플러그인 로드 API 호출
            response = requests.post(f"{self.base_url}/api/plugins/{plugin_name}/load", timeout=30)
    ...
    def test_plugin_operation(self, plugin_name: str, operation: str = "test") -> Dict[str, Any]:
        """플러그인 작업 테스트"""
        ...
        try:
            # 플러그인 작업 API 호출
            response = requests.post(f"{self.base_url}/api/plugins/{plugin_name}/{operation}", timeout=30)
    ...
    def test_concurrent_operations(self, plugin_name: str, num_operations: int = 10) -> Dict[str, Any]:
        """동시 작업 테스트"""
        ...
        def run_operation():
            try:
                response = requests.post(f"{self.base_url}/api/plugins/{plugin_name}/test", timeout=10)
                results.append({
                    'success': response.status_code == 200,
                    'response_time': response.elapsed.total_seconds(),
                    'status_code': response.status_code
    ...
    def test_memory_leak(self, plugin_name: str, duration: int = 60) -> Dict[str, Any]:
        ...
        try:
            while time.time() - start_time < duration:
                # 플러그인 작업 반복 실행
                response = requests.post(f"{self.base_url}/api/plugins/{plugin_name}/test", timeout=5)
            ...
            summary = self.monitor.get_summary()
            summary['memory_leak'] = {
                'initial_memory_mb': memory_samples[0]['memory_used_mb'] if memory_samples else 0,
                'final_memory_mb': memory_samples[-1]['memory_used_mb'] if memory_samples else 0,
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        # 성능 메트릭 집계
        all_metrics = [r.get('metrics', {}) for r in self.results.values() if r.get('metrics')]
        ...
        memory_averages = [m.get('memory', {}).get('average_percent', 0) for m in all_metrics]
        ...
        summary = {
            'overall_cpu_average': sum(cpu_averages) / len(cpu_averages) if cpu_averages else 0,
            'overall_memory_average': sum(memory_averages) / len(memory_averages) if memory_averages else 0,
