...
class PerformanceTest:
    ...
    def test_metrics_endpoint(self, plugin_id: str = "test_plugin") -> Dict[str, Any]:
        ...
        return self.test_api_endpoint(
            f"/api/advanced-monitoring/metrics/{plugin_id}?hours=24"
        )
    ...
    def test_analytics_endpoint(self, plugin_id: str = "test_plugin") -> Dict[str, Any]:
        """분석 엔드포인트 테스트"""
        return self.test_api_endpoint(
            f"/api/advanced-monitoring/analytics/{plugin_id}?hours=24"
        )
    ...
    def test_plugin_registration(self) -> Dict[str, Any]:
        ...
        return self.test_api_endpoint(
            "/api/advanced-monitoring/register-plugin", "POST", data
        )
    ...
    def test_metrics_update(self, plugin_id: str = "test_plugin") -> Dict[str, Any]:
        ...
        return self.test_api_endpoint(
            "/api/advanced-monitoring/update-metrics", "POST", data
        )

    def run_concurrent_tests(
        self, test_func, num_threads: int = 10, num_requests: int = 100
    ) -> List[Dict[str, Any]]:
        """동시 테스트 실행"""
        results = []
    ...
    def run_load_test(
        self, test_type: str, num_threads: int = 10, num_requests: int = 100
    ) -> Dict[str, Any]:
        """부하 테스트 실행"""
        logger.info(
            f"Starting {test_type} load test with {num_threads} threads, {num_requests} requests"
        )

        self.start_time = time.time()

        if test_type == "metrics":
            results = self.run_concurrent_tests(
                self.test_metrics_endpoint, num_threads, num_requests
            )
        elif test_type == "trends":
            results = self.run_concurrent_tests(
                self.test_trends_endpoint, num_threads, num_requests
            )
        elif test_type == "analytics":
            results = self.run_concurrent_tests(
                self.test_analytics_endpoint, num_threads, num_requests
            )
        elif test_type == "registration":
            results = self.run_concurrent_tests(
                self.test_plugin_registration, num_threads, num_requests
            )
        elif test_type == "update":
            results = self.run_concurrent_tests(
                self.test_metrics_update, num_threads, num_requests
            )
        else:
    ...
    def analyze_results(
        self, results: List[Dict[str, Any]], test_type: str
    ) -> Dict[str, Any]:
        """결과 분석"""
        ...
        analysis = {
            ...
            "requests_per_second": (
                len(results) / (self.end_time - self.start_time)
                if self.end_time and self.start_time
                else 0
            ),
        }
    ...
    def run_stress_test(
        self, test_type: str, max_threads: int = 50, max_requests: int = 1000
    ) -> Dict[str, Any]:
        """스트레스 테스트 실행"""
        ...
        for threads in [5, 10, 20, 30, 40, 50]:
            ...
            logger.info(
                f"Testing with {threads} threads, {requests_per_thread} requests per thread"
            )
    ...
    def find_max_sustainable_load(
        ...
        if not sustainable_results:
            return {"threads": 0, "requests_per_second": 0, "avg_response_time": 0}
    ...
    def run_websocket_test(self, num_connections: int = 100) -> Dict[str, Any]:
        ...
        def on_open(ws):
            results.append({"type": "open", "timestamp": datetime.now().isoformat()})

            # 인증 메시지 전송
            ws.send(
                json.dumps({"type": "auth", "user_id": "test_user", "role": "admin"})
            )
        ...
        for i, ws in enumerate(connections[:10]):  # 처음 10개 연결만 테스트
            try:
                ws.send(json.dumps({"type": "get_all_metrics"}))
            except Exception as e:
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        for endpoint, result in endpoint_results.items():  # pyright: ignore
            if isinstance(result, dict):
                ...
                print(
                    f"    Avg Response Time: {result.get('avg_response_time', 0):.1f}ms"
                )
                print(
                    f"    Throughput: {result.get('requests_per_second', 0):.1f} req/s"
                )
...
def main():
    ...
    parser.add_argument(
        "--base-url", default="http://localhost:5000", help="Base URL for testing"
    )
    parser.add_argument(
        "--test-type",
        choices=["load", "stress", "websocket", "comprehensive"],
        default="comprehensive",
        help="Type of test to run",
    )
    parser.add_argument(
        "--threads", type=int, default=10, help="Number of threads for load test"
    )
    parser.add_argument(
        "--requests", type=int, default=100, help="Number of requests per test"
    )
