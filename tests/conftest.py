...
def admin_token(client, admin_user, session):
    ...
    login_payload = {"username": admin_user.username, "password": "a-very-secure-admin-password-123"}
    resp = client.post("/api/auth/login", json=login_payload)
    
    if resp.status_code != 200:
        pytest.fail(f"Admin login failed with status {resp.status_code}: {resp.data.decode()}")
...
def logged_in_admin_client(client, admin_user, session):
    ...
    with client:
        ...
        session.add(admin_user)
        session.commit()
        client.post('/auth/login', data={'username': admin_user.username, 'password': 'a-very-secure-admin-password-123'}, follow_redirects=True)
        yield client
