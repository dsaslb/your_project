...
from core.backend.module_integration_system import ModuleIntegrationSystem, IntegrationType, DataSource
...
class TestModuleMarketplaceSystem(unittest.TestCase):
    ...
    def test_marketplace_initialization(self):
        ...
        # 출퇴근 관리 모듈 존재 확인
        attendance_module = next((m for m in modules if m['id'] == 'attendance_management'), None)
    ...
    def test_module_statistics(self):
        """모듈 통계 테스트"""
        # 여러 모듈 설치
        modules_to_install = ['attendance_management', 'sales_management', 'payroll_management']
    ...
    def test_module_search_and_filter(self):
        ...
        for module in search_results:
            self.assertTrue(
                '출퇴근' in module['name'] or 
                '출퇴근' in module['description'] or
                '출퇴근' in module.get('tags', [])
            )
...
class TestModuleMarketplaceAPI(unittest.TestCase):
    ...
    def test_get_module_reviews(self):
        """모듈 리뷰 목록 조회 테스트"""
        response = self.app.get('/api/module-marketplace/modules/attendance_management/reviews')
        data = json.loads(response.data)
