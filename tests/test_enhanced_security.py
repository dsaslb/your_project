...
class EnhancedSecurityTester:
    ...
    def test_security_summary(self) -> bool:
        """보안 요약 정보 테스트"""
        try:
            response = self.session.get(f"{self.base_url}/api/enhanced-security/summary")
            
            if response.status_code == 200:
                data = response.json()
                if data.get('success') and 'data' in data:
                    summary = data['data']
                    required_fields = ['open_vulnerabilities', 'active_malware', 'unresolved_events', 'critical_plugins', 'high_risk_plugins']
                    
                    for field in required_fields:
                        if field not in summary:
                            self.log_test("보안 요약", False, f"필수 필드 누락: {field}")
                            return False
                    
                    self.log_test("보안 요약", True, f"보안 요약 조회 성공 - 취약점: {summary['open_vulnerabilities']}, 악성코드: {summary['active_malware']}")
                    return True
                else:
    ...
    def test_vulnerabilities_list(self) -> bool:
        """취약점 목록 조회 테스트"""
        try:
            response = self.session.get(f"{self.base_url}/api/enhanced-security/vulnerabilities?limit=10")
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and 'data' in data:
                    ...
                    if isinstance(vulnerabilities, list):
                        self.log_test("취약점 목록", True, f"취약점 {len(vulnerabilities)}개 조회 성공")
                        return True
                    else:
                ...
                self.log_test("취약점 목록", False, f"HTTP 오류: {response.status_code}")
                return False
    ...
    def test_malware_detections(self) -> bool:
        """악성코드 감지 목록 조회 테스트"""
        try:
            response = self.session.get(f"{self.base_url}/api/enhanced-security/malware?limit=10")
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and 'data' in data:
                    ...
                    if isinstance(malware_detections, list):
                        self.log_test("악성코드 감지", True, f"악성코드 감지 {len(malware_detections)}개 조회 성공")
                        return True
                    else:
                        self.log_test("악성코드 감지", False, "악성코드 감지 목록이 배열이 아님")
                        return False
                else:
                    self.log_test("악성코드 감지", False, "응답 형식 오류", data)
                    return False
            else:
                self.log_test("악성코드 감지", False, f"HTTP 오류: {response.status_code}")
                return False
    ...
    def test_security_events(self) -> bool:
        """보안 이벤트 목록 조회 테스트"""
        try:
            response = self.session.get(f"{self.base_url}/api/enhanced-security/events?limit=10")
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and 'data' in data:
                    ...
                    if isinstance(security_events, list):
                        self.log_test("보안 이벤트", True, f"보안 이벤트 {len(security_events)}개 조회 성공")
                        return True
                    else:
                        self.log_test("보안 이벤트", False, "보안 이벤트 목록이 배열이 아님")
                        return False
                else:
                    self.log_test("보안 이벤트", False, "응답 형식 오류", data)
                    return False
            else:
                self.log_test("보안 이벤트", False, f"HTTP 오류: {response.status_code}")
                return False
    ...
    def test_security_profiles(self) -> bool:
        """보안 프로필 목록 조회 테스트"""
        try:
            response = self.session.get(f"{self.base_url}/api/enhanced-security/profiles?limit=10")
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and 'data' in data:
                    ...
                    if isinstance(security_profiles, list):
                        self.log_test("보안 프로필", True, f"보안 프로필 {len(security_profiles)}개 조회 성공")
                        return True
                    else:
                        self.log_test("보안 프로필", False, "보안 프로필 목록이 배열이 아님")
                        return False
                else:
                    self.log_test("보안 프로필", False, "응답 형식 오류", data)
                    return False
            else:
                self.log_test("보안 프로필", False, f"HTTP 오류: {response.status_code}")
                return False
    ...
    def test_scan_history(self) -> bool:
        """스캔 이력 조회 테스트"""
        try:
            response = self.session.get(f"{self.base_url}/api/enhanced-security/scans?limit=10")
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and 'data' in data:
                    ...
                    if isinstance(scan_history, list):
                        self.log_test("스캔 이력", True, f"스캔 이력 {len(scan_history)}개 조회 성공")
                        return True
                    else:
                        self.log_test("스캔 이력", False, "스캔 이력 목록이 배열이 아님")
                        return False
                else:
        ...
        try:
            ...
            if response.status_code in [401, 403]:
                ...
                if scan_data.get('success'):
                    ...
                    self.log_test("보안 스캔 시작", False, "보안 스캔 시작 실패", scan_data)
                    return False
            else:
                self.log_test("보안 스캔 시작", False, f"예상치 못한 HTTP 상태: {response.status_code}")
                return False
    ...
    def test_monitoring_control(self) -> bool:
        ...
        try:
            # 모니터링 시작 시도
            start_response = self.session.post(f"{self.base_url}/api/enhanced-security/monitoring/start")
            ...
            if start_response.status_code in [401, 403]:
                ...
                if start_data.get('success'):
                    # 모니터링 중지 시도
                    stop_response = self.session.post(f"{self.base_url}/api/enhanced-security/monitoring/stop")
                    if stop_response.status_code == 200:
                        stop_data = stop_response.json()
                        if stop_data.get('success'):
                            self.log_test("모니터링 제어", True, "모니터링 시작/중지 성공")
                            return True
                        else:
                            self.log_test("모니터링 제어", False, "모니터링 중지 실패", stop_data)
                            return False
                    else:
                        self.log_test("모니터링 제어", False, f"모니터링 중지 HTTP 오류: {stop_response.status_code}")
                        return False
                else:
                    self.log_test("모니터링 제어", False, "모니터링 시작 실패", start_data)
                    return False
            else:
                self.log_test("모니터링 제어", False, f"모니터링 시작 HTTP 오류: {start_response.status_code}")
                return False
    ...
    def test_vulnerability_status_update(self) -> bool:
        ...
        try:
            ...
            list_response = self.session.get(f"{self.base_url}/api/enhanced-security/vulnerabilities?limit=1")
            ...
            if list_response.status_code == 200:
                ...
                if list_data.get('success') and list_data['data']:
                    ...
                    if update_response.status_code in [401, 403]:
                        self.log_test("취약점 상태 업데이트", True, "인증 필요 (예상된 동작)")
                        return True
                    elif update_response.status_code == 200:
                        update_data = update_response.json()
                        if update_data.get('success'):
                            self.log_test("취약점 상태 업데이트", True, "취약점 상태 업데이트 성공")
                            return True
                        else:
                            self.log_test("취약점 상태 업데이트", False, "취약점 상태 업데이트 실패", update_data)
                            return False
                    else:
                        self.log_test("취약점 상태 업데이트", False, f"HTTP 오류: {update_response.status_code}")
                        return False
                else:
                    self.log_test("취약점 상태 업데이트", False, "테스트할 취약점이 없음")
                    return False
            else:
                self.log_test("취약점 상태 업데이트", False, f"취약점 목록 조회 실패: {list_response.status_code}")
                return False
    ...
    def test_malware_quarantine(self) -> bool:
        ...
        try:
            ...
            list_response = self.session.get(f"{self.base_url}/api/enhanced-security/malware?limit=1")
                ...
                if list_data.get('success') and list_data['data']:
                    ...
                    if quarantine_response.status_code in [401, 403]:
                        ...
                        if quarantine_data.get('success'):
                            ...
                            self.log_test("악성코드 격리", False, "악성코드 격리 실패", quarantine_data)
                            return False
                    else:
                        self.log_test("악성코드 격리", False, f"HTTP 오류: {quarantine_response.status_code}")
                        return False
                else:
                    self.log_test("악성코드 격리", False, "테스트할 악성코드 감지가 없음")
                    return False
            else:
                self.log_test("악성코드 격리", False, f"악성코드 감지 목록 조회 실패: {list_response.status_code}")
                return False
    ...
    def test_security_event_resolution(self) -> bool:
        ...
        try:
            ...
            list_response = self.session.get(f"{self.base_url}/api/enhanced-security/events?limit=1")
            ...
            if list_response.status_code == 200:
                ...
                if list_data.get('success') and list_data['data']:
                    ...
                    if resolve_response.status_code in [401, 403]:
                        self.log_test("보안 이벤트 해결", True, "인증 필요 (예상된 동작)")
                        return True
                    elif resolve_response.status_code == 200:
                        resolve_data = resolve_response.json()
                        if resolve_data.get('success'):
                            self.log_test("보안 이벤트 해결", True, "보안 이벤트 해결 성공")
                            return True
                        else:
                            self.log_test("보안 이벤트 해결", False, "보안 이벤트 해결 실패", resolve_data)
                            return False
                    else:
                        self.log_test("보안 이벤트 해결", False, f"HTTP 오류: {resolve_response.status_code}")
                        return False
                else:
                    self.log_test("보안 이벤트 해결", False, "테스트할 보안 이벤트가 없음")
                    return False
            else:
                self.log_test("보안 이벤트 해결", False, f"보안 이벤트 목록 조회 실패: {list_response.status_code}")
                return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="고도화된 플러그인 보안 모니터링 시스템 테스트")
    parser.add_argument("--url", default="http://localhost:5000", help="테스트할 서버 URL")
