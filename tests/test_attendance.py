...
def test_attendance_api():
    """출퇴근 체크 API 테스트"""
    print('🔍 1단계: 출퇴근 체크 API 테스트 시작...')
    
    # 1. 로그인하여 토큰 획득
    print('📱 모바일 로그인 중...')
    login_response = requests.post('http://localhost:5000/api/mobile/login', 
    ...
    attendance_response = requests.post('http://localhost:5000/api/mobile/attendance/clock',
    ...
    print(f'   - 시간: {result.get("timestamp", result.get("at"))}')
    print(f'   - 위치: {result.get("lat")}, {result.get("lng")}')
    print(f'   - QR 코드: {result.get("qr")}')
    print('🔔 실시간 이벤트가 웹 대시보드로 전송되었습니다!')
    ...
    checkout_response = requests.post('http://localhost:5000/api/mobile/attendance/clock',
                                    json=attendance_data,
                                    headers=headers)
    ...
    if checkout_response.status_code == 200:
        ...
        print(f'   - 타입: {result.get("type")}')
        print(f'   - 시간: {result.get("timestamp", result.get("at"))}')
        print('🔔 퇴근 실시간 이벤트가 웹 대시보드로 전송되었습니다!')
    else:
