...
def groupsort_indexer(index: np.ndarray, ngroups: int) -> tuple[
    np.ndarray,
    np.ndarray,
]: ...
def kth_smallest(arr: np.ndarray, k: int) -> Any: ...
def nancorr(
    mat: npt.NDArray[np.float64], cov: bool = ..., minp: int | None = ...
) -> npt.NDArray[np.float64]: ...
def nancorr_spearman(
    mat: npt.NDArray[np.float64], minp: int = ...
) -> npt.NDArray[np.float64]: ...
def validate_limit(nobs: int | None, limit=...) -> int: ...
def get_fill_indexer(
    mask: npt.NDArray[np.bool_], limit: int | None = ...
) -> npt.NDArray[np.intp]: ...
