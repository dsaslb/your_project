...
from models import User, ReasonTemplate, Attendance, Notification
...
def test_reason_templates():
    ...
    with app.app_context():
        ...
        # 팀별 템플릿
        kitchen_templates = ReasonTemplate.query.filter_by(team="주방", is_active=True).all()
        ...
        # 기존 근태 기록 확인
        attendances = Attendance.query.filter(Attendance.reason.isnot(None)).limit(5).all()
        if attendances:
        ...
        late_notifications = Notification.query.filter(
            Notification.content.like('%지각%'),
            Notification.category == '근무'
        ).all()
...
def test_mobile_api():
    ...
    with app.app_context():
        ...
        if test_attendance:
            print(f"  테스트 근태 기록: ID {test_attendance.id}, 사용자 {test_attendance.user_id}")
