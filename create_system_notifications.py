    ...
    with app.app_context():
        ...
        for noti in system_notifications:
            send_admin_only_notification(
                noti["content"], noti["category"], noti["link"]
            )
        ...
        test_orders = [
            {
                "item": "커피원두",
                "quantity": 10,
                "ordered_by": users[0].id if users else None,
            },
            {
                "item": "청소용품",
                "quantity": 5,
                "ordered_by": users[1].id if len(users) > 1 else None,
            },
            {
                "item": "식재료",
                "quantity": 20,
                "ordered_by": users[2].id if len(users) > 2 else None,
            },
        ]
        ...
        for noti in role_notifications:
            send_notification_to_role(
                noti["role"], noti["content"], noti["category"], noti["link"]
            )
        ...
        for content in general_notifications:
            for user in users[:3]:  # 처음 3명에게만
                send_notification_enhanced(user.id, content, "공지")
            print(f"   ✅ {content[:20]}...")

        # 통계 출력
        total_notifications = Notification.query.count()
        admin_only_notifications = Notification.query.filter_by(
            is_admin_only=True
        ).count()
        ...
        print(f"\n🔗 테스트 링크:")
        print(
            f"   - 발주 승인 알림: http://localhost:5000/notifications/orders/approved"
        )
        print(
            f"   - 관리자 시스템 알림: http://localhost:5000/admin/system_notifications"
        )
        ...
        print(
            f"   3. 발주 승인 필터: /notifications/filtered?category=발주&keyword=승인"
        )
