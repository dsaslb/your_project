...
def upload_plugin():
    ...
    try:
        with zipfile.ZipFile(save_path, 'r') as zipf:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # 필수 파일 검사
                required = [os.path.join(tmpdir, 'config', 'plugin.json'), os.path.join(tmpdir, 'backend')]
                for req in required:
                        ...
                        if fname.endswith('.py'):
                            with open(os.path.join(root, fname), encoding='utf-8') as f:
                                ...
                                if 'eval(' in code or 'os.system' in code:
                                    return reject_plugin_upload(meta, '보안 위험 함수 사용')
                # 자동 테스트 실행(pytest)
                test_dir = os.path.join(tmpdir, 'tests')
                if os.path.exists(test_dir):
                    try:
                        result = subprocess.run(['pytest', test_dir, '--maxfail=1', '--disable-warnings', '-q'], capture_output=True, text=True)
    ...
    return jsonify({'success': True, 'plugin_id': plugin_id, 'status': 'pending', 'meta': meta})
...
def reject_plugin_upload(meta, reason):
    ...
    send_notification(meta.get('author_email', 'dev@example.com'), '플러그인 자동 거절', f"사유: {reason}")
    send_email(meta.get('author_email', 'dev@example.com'), '플러그인 자동 거절', f"사유: {reason}")
    send_push(meta.get('author_email', 'dev@example.com'), f"플러그인 자동 거절: {reason}")
...
def approve_plugin(plugin_id):
    ...
    if not current_user.is_authenticated or getattr(current_user, 'role', None) != 'OWNER':
    ...
    if PLUGINS[plugin_id].get('status') == 'pending':
        ...
        send_notification(PLUGINS[plugin_id]['meta'].get('author_email', 'dev@example.com'),
                         '플러그인 1차 승인', f"{plugin_id} 플러그인이 1차 승인되었습니다.\n사유: {reason or '-'}")
        socketio.emit('plugin_approval', {
            'plugin_id': plugin_id,
            'action': 'first_approve',
            'user': current_user.username
        })
        return jsonify({'success': True, 'plugin_id': plugin_id, 'status': 'waiting_second_approval', 'msg': '1차 승인 완료, 2차 승인 필요'}), 200
        ...
        if PLUGINS[plugin_id].get('first_approved_by') == current_user.username:
            return jsonify({'success': False, 'msg': '2차 승인은 다른 관리자가 해야 합니다.'}), 400
        ...
        send_notification(PLUGINS[plugin_id]['meta'].get('author_email', 'dev@example.com'),
                         '플러그인 최종 승인', f"{plugin_id} 플러그인이 최종 승인되었습니다.\n사유: {reason or '-'}")
        socketio.emit('plugin_approval', {
            'plugin_id': plugin_id,
            'action': 'second_approve',
            'user': current_user.username
        })
        return jsonify({'success': True, 'plugin_id': plugin_id, 'status': 'approved', 'msg': '최종 승인 완료'}), 200
    else:
        return jsonify({'success': False, 'msg': '이미 승인되었거나 승인 대기 상태가 아닙니다.'}), 400
...
def reject_plugin(plugin_id):
    ...
    # 알림 발송 (샘플)
    send_notification(PLUGINS[plugin_id]['meta'].get('author_email', 'dev@example.com'),
                     '플러그인 거절', f"{plugin_id} 플러그인이 거절되었습니다.\n사유: {reason or '-'}")
    # 실시간 알림(WebSocket)
    socketio.emit('plugin_approval', {
        'plugin_id': plugin_id,
        'action': 'reject',
...
def plugin_status(plugin_id):
    ...
    if plugin_id not in PLUGINS:
        return jsonify({'success': False, 'msg': '플러그인 없음'}), 404
    return jsonify({'success': True, 'plugin_id': plugin_id, 'status': PLUGINS[plugin_id]['status'], 'meta': PLUGINS[plugin_id]['meta']})
...
def get_marketplace_stats():
    ...
    try:
        ...
        for r in ratings:
            pid = r['plugin_id']
            plugin_stats.setdefault(pid, {'rating_sum': 0, 'rating_cnt': 0, 'feedback_cnt': 0})
        ...
        for pid in plugin_stats:
            if plugin_stats[pid]['rating_cnt']:
                plugin_stats[pid]['avg_rating'] = round(plugin_stats[pid]['rating_sum'] / plugin_stats[pid]['rating_cnt'], 2)
            else:
...
def list_plugins():
    ...
    def is_accessible(p):
        if brand_code and p.get('brand_code') and p['brand_code'] != brand_code:
            return False
        if industry_code and p.get('industry_code') and p['industry_code'] != industry_code:
            return False
        return True
...
def send_pending_approval_report():
    ...
    with open(meta_path, 'r', encoding='utf-8') as f:
        meta = json.load(f)
    plugins = [p for p in meta.get('plugins', {}).values() if p.get('status') == 'pending']
    if not plugins:
        return
    # 관리자 이메일/ID 예시
...
def delegate_admin():
    ...
    log_approval('system', 'delegate_admin', current_user.username, f'위임 대상: {target}')
    send_email(f'{target}@your_program.com', '관리자 권한 위임', f'OWNER가 {target}에게 권한을 위임했습니다.')
...
def revoke_admin():
    ...
    log_approval('system', 'revoke_admin', current_user.username, f'회수 대상: {target}')
    send_email(f'{target}@your_program.com', '관리자 권한 회수', f'OWNER가 {target}의 권한을 회수했습니다.')
