...
class AttendanceService:
    ...
    def save_attendance_record(self, record: Dict) -> Dict:
        ...
        try:
            ...
            existing_record = next(
                (r for r in data if r.get('employee_id') == record['employee_id'] 
                 and r.get('date', '').startswith(today)), None
            )
    ...
    def calculate_work_hours(self, employee_id: str, check_out_time: datetime) -> float:
        ...
        try:
            ...
            # 오늘 출근 기록 찾기
            record = next(
                (r for r in data if r.get('employee_id') == employee_id 
                 and r.get('date', '').startswith(today)), None
            )
    ...
    def get_attendance_data(self, user_id: str, user_type: str, 
    ...
    def get_attendance_stats(self, user_id: str, user_type: str) -> Dict:
        ...
        try:
            ...
            # 정시 출근률
            on_time_rate = ((total_records - late_count) / total_records * 100) if total_records > 0 else 0
    ...
    def get_today_attendance(self, user_id: str, user_type: str) -> Dict:
        ...
        try:
            ...
            present_today = len([r for r in today_data if r.get('status') == 'checked_out'])
            late_count = len([r for r in today_data if r.get('is_late', False)])
            
            # 평균 근무 시간
            work_hours = [r.get('work_hours', 0) for r in today_data if r.get('work_hours')]
            avg_work_hours = sum(work_hours) / len(work_hours) if work_hours else 0
            ...
            return {
                ...
                'statistics': {
                    ...
                    'on_time_rate': round(((total_employees - late_count) / total_employees * 100) if total_employees > 0 else 0, 1)
                }
            }
    ...
    def export_attendance_data(self, user_id: str, user_type: str, 
                             format_type: str, start_date: Optional[str] = None, 
                             end_date: Optional[str] = None) -> Dict:
        """출근 데이터 내보내기"""
        try:
            # 데이터 조회
            data_result = self.get_attendance_data(user_id, user_type, start_date, end_date)
            ...
            if format_type == 'excel':
                ...
                export_data = {
                    ...
                    'data': records,
                    'columns': ['id', 'employee_id', 'date', 'check_in_time', 'check_out_time', 
                               'work_hours', 'is_late', 'status']
                }
