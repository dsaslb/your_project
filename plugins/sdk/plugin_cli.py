...
사용 예제:
  ...
  %(prog)s publish plugins/my-plugin
        ...
        validate_parser.add_argument(
            "--strict", action="store_true", help="엄격한 검증 모드"
        )
        validate_parser.add_argument(
            "--run-tests", action="store_true", help="테스트 코드 자동 실행"
        )
        ...
        test_parser.add_argument(
            "--coverage", action="store_true", help="코드 커버리지 측정"
        )
        ...
        docs_parser.add_argument(
            "--format", choices=["html", "pdf", "md"], default="html", help="문서 형식"
        )
        ...
        if args.marketplace:
            ...
            try:
                with open(package_path, "rb") as f:
                    files = {
                        "file": (os.path.basename(package_path), f, "application/zip")
                    }
                    headers = (
                        {"Authorization": f"Bearer {args.token}"} if args.token else {}
                    )
                    response = requests.post(
                        args.marketplace, files=files, headers=headers
                    )
